var rr = Object.defineProperty;
var ar = (t, e, n) => e in t ? rr(t, e, { enumerable: !0, configurable: !0, writable: !0, value: n }) : t[e] = n;
var En = (t, e, n) => ar(t, typeof e != "symbol" ? e + "" : e, n);
import { reactive as St, watch as Oe, ref as O, shallowRef as lr, onMounted as Ce, onUnmounted as Qn, onUpdated as Bs, nextTick as vt, computed as ze, inject as re, createElementBlock as g, openBlock as _, withKeys as $t, unref as r, createElementVNode as a, withModifiers as tt, renderSlot as Lt, normalizeClass as le, toDisplayString as b, createBlock as K, resolveDynamicComponent as Us, withCtx as J, createVNode as j, createCommentVNode as N, Fragment as ye, renderList as $e, withDirectives as _e, vModelCheckbox as Xt, createTextVNode as Z, vModelSelect as gs, vModelText as Ct, onBeforeUnmount as Ns, customRef as ir, vShow as je, isRef as cr, TransitionGroup as dr, normalizeStyle as fn, mergeModels as ur, useModel as Ps, resolveComponent as fr, provide as vr, Transition as _r } from "vue";
import mr from "mitt";
import pr from "dragselect";
import hr from "@uppy/core";
import gr from "@uppy/xhr-upload";
import br from "vanilla-lazyload";
import "cropperjs/dist/cropper.css";
import wr from "cropperjs";
var Rs;
const Tn = (Rs = document.querySelector('meta[name="csrf-token"]')) == null ? void 0 : Rs.getAttribute("content");
class yr {
  /** @param {RequestConfig} config */
  constructor(e) {
    /** @type {RequestConfig} */
    En(this, "config");
    En(this, "customFetch", async (...e) => {
      let [n, o] = e;
      this.config.fetchRequestInterceptor && (o = this.config.fetchRequestInterceptor(o));
      let s = await fetch(n, o);
      return this.config.fetchResponseInterceptor && (s = await this.config.fetchResponseInterceptor(s)), s;
    });
    this.config = e;
  }
  /** @type {RequestConfig} */
  get config() {
    return this.config;
  }
  /**
   * Transform request params
   * @param {Object} input
   * @param {String} input.url
   * @param {'get'|'post'|'put'|'patch'|'delete'} input.method
   * @param {Record<String,String>=} input.headers
   * @param {Record<String,?String>=} input.params
   * @param {Record<String,?String>|FormData=} input.body
   * @return {RequestTransformResultInternal}
   */
  transformRequestParams(e) {
    const n = this.config, o = {};
    Tn != null && Tn !== "" && (o[n.xsrfHeaderName] = Tn);
    const s = Object.assign({}, n.headers, o, e.headers), i = Object.assign({}, n.params, e.params), c = e.body, u = n.baseUrl + e.url, l = e.method;
    let d;
    l !== "get" && (c instanceof FormData ? (d = c, n.body != null && Object.entries(this.config.body).forEach(([f, m]) => {
      d.append(f, m);
    })) : (d = { ...c }, n.body != null && Object.assign(d, this.config.body)));
    const v = {
      url: u,
      method: l,
      headers: s,
      params: i,
      body: d
    };
    if (n.transformRequest != null) {
      const f = n.transformRequest({
        url: u,
        method: l,
        headers: s,
        params: i,
        body: d
      });
      f.url != null && (v.url = f.url), f.method != null && (v.method = f.method), f.params != null && (v.params = f.params ?? {}), f.headers != null && (v.headers = f.headers ?? {}), f.body != null && (v.body = f.body);
    }
    return v;
  }
  /**
   * Get download url
   * @param {String} adapter
   * @param {String} node
   * @param {String} node.path
   * @param {String=} node.url
   * @return {String}
   */
  getDownloadUrl(e, n) {
    if (n.url != null)
      return n.url;
    const o = this.transformRequestParams({
      url: "",
      method: "get",
      params: { q: "download", adapter: e, path: n.path }
    });
    return o.url + "?" + new URLSearchParams(o.params).toString();
  }
  /**
   * Get preview url
   * @param {String} adapter
   * @param {String} node
   * @param {String} node.path
   * @param {String=} node.url
   * @return {String}
   */
  getPreviewUrl(e, n) {
    if (n.url != null)
      return n.url;
    const o = this.transformRequestParams({
      url: "",
      method: "get",
      params: { q: "preview", adapter: e, path: n.path }
    });
    return o.url + "?" + new URLSearchParams(o.params).toString();
  }
  /**
   * Send request
   * @param {Object} input
   * @param {String} input.url
   * @param {'get'|'post'|'put'|'patch'|'delete'} input.method
   * @param {Record<String,String>=} input.headers
   * @param {Record<String,?String>=} input.params
   * @param {(Record<String,?String>|FormData|null)=} input.body
   * @param {'arrayBuffer'|'blob'|'json'|'text'=} input.responseType
   * @param {AbortSignal=} input.abortSignal
   * @returns {Promise<(ArrayBuffer|Blob|Record<String,?String>|String|null)>}
   * @throws {Record<String,?String>|null} resp json error
   */
  async send(e) {
    const n = this.transformRequestParams(e), o = e.responseType || "json", s = {
      method: e.method,
      headers: n.headers,
      signal: e.abortSignal
    }, i = n.url + "?" + new URLSearchParams(n.params);
    if (n.method !== "get" && n.body != null) {
      let u;
      n.body instanceof FormData ? u = e.body : (u = JSON.stringify(n.body), s.headers["Content-Type"] = "application/json"), s.body = u;
    }
    this.config.fetchParams && Object.assign(s, this.config.fetchParams);
    const c = await this.customFetch(i, s);
    if (c.ok)
      return await c[o]();
    throw await c.json();
  }
}
function kr(t) {
  const e = {
    baseUrl: "",
    headers: {},
    params: {},
    body: {},
    xsrfHeaderName: "X-CSRF-Token",
    fetchParams: {}
  };
  return typeof t == "string" ? Object.assign(e, { baseUrl: t }) : Object.assign(e, t), new yr(e);
}
function xr(t) {
  let e = localStorage.getItem(t + "_storage");
  const n = St(JSON.parse(e ?? "{}"));
  Oe(n, o);
  function o() {
    Object.keys(n).length ? localStorage.setItem(t + "_storage", JSON.stringify(n)) : localStorage.removeItem(t + "_storage");
  }
  function s(l, d) {
    n[l] = d;
  }
  function i(l) {
    delete n[l];
  }
  function c() {
    Object.keys(n).map((l) => i(l));
  }
  return { getStore: (l, d = null) => n.hasOwnProperty(l) ? n[l] : d, setStore: s, removeStore: i, clearStore: c };
}
async function Sr(t, e) {
  const n = e[t];
  return typeof n == "function" ? (await n()).default : n;
}
function $r(t, e, n, o) {
  const { getStore: s, setStore: i } = t, c = O({}), u = O(s("locale", e)), l = (f, m = e) => {
    Sr(f, o).then((p) => {
      c.value = p, i("locale", f), u.value = f, i("translations", p), Object.values(o).length > 1 && (n.emit("vf-toast-push", { label: "The language is set to " + f }), n.emit("vf-language-saved"));
    }).catch((p) => {
      m ? (n.emit("vf-toast-push", { label: "The selected locale is not yet supported!", type: "error" }), l(m, null)) : n.emit("vf-toast-push", { label: "Locale cannot be loaded!", type: "error" });
    });
  };
  Oe(u, (f) => {
    l(f);
  }), !s("locale") && !o.length ? l(e) : c.value = s("translations");
  const d = (f, ...m) => m.length ? d(f = f.replace("%s", m.shift()), ...m) : f;
  function v(f, ...m) {
    return c.value && c.value.hasOwnProperty(f) ? d(c.value[f], ...m) : d(f, ...m);
  }
  return St({ t: v, locale: u });
}
const ue = {
  EDIT: "edit",
  NEW_FILE: "newfile",
  NEW_FOLDER: "newfolder",
  PREVIEW: "preview",
  ARCHIVE: "archive",
  UNARCHIVE: "unarchive",
  SEARCH: "search",
  RENAME: "rename",
  UPLOAD: "upload",
  DELETE: "delete",
  FULL_SCREEN: "fullscreen",
  DOWNLOAD: "download",
  LANGUAGE: "language"
}, Cr = Object.values(ue), Er = "2.8.0";
function qs(t, e, n, o, s) {
  return (e = Math, n = e.log, o = 1024, s = n(t) / n(o) | 0, t / e.pow(o, s)).toFixed(0) + " " + (s ? "KMGTPEZY"[--s] + "iB" : "B");
}
function zs(t, e, n, o, s) {
  return (e = Math, n = e.log, o = 1e3, s = n(t) / n(o) | 0, t / e.pow(o, s)).toFixed(0) + " " + (s ? "KMGTPEZY"[--s] + "B" : "B");
}
function Tr(t) {
  const e = { k: 1, m: 2, g: 3, t: 4 }, o = /(\d+(?:\.\d+)?)\s?(k|m|g|t)?b?/i.exec(t);
  return o[1] * Math.pow(1024, e[o[2].toLowerCase()]);
}
const rt = {
  SYSTEM: "system",
  LIGHT: "light",
  DARK: "dark"
};
function Ar(t, e) {
  const n = O(rt.SYSTEM), o = O(rt.LIGHT);
  n.value = t.getStore("theme", e ?? rt.SYSTEM);
  const s = window.matchMedia("(prefers-color-scheme: dark)"), i = (c) => {
    n.value === rt.DARK || n.value === rt.SYSTEM && c.matches ? o.value = rt.DARK : o.value = rt.LIGHT;
  };
  return i(s), s.addEventListener("change", i), {
    /**
     * @type {import('vue').Ref<Theme>}
     */
    value: n,
    /**
     * @type {import('vue').Ref<Theme>}
     */
    actualValue: o,
    /**
     * @param {Theme} value
     */
    set(c) {
      n.value = c, c !== rt.SYSTEM ? t.setStore("theme", c) : t.removeStore("theme"), i(s);
    }
  };
}
function Mr() {
  const t = lr(null), e = O(!1), n = O();
  return { visible: e, type: t, data: n, open: (i, c = null) => {
    document.querySelector("body").style.overflow = "hidden", e.value = !0, t.value = i, n.value = c;
  }, close: () => {
    document.querySelector("body").style.overflow = "", e.value = !1, t.value = null;
  } };
}
/*!
 * OverlayScrollbars
 * Version: 2.11.1
 *
 * Copyright (c) Rene Haas | KingSora.
 * https://github.com/KingSora
 *
 * Released under the MIT license.
 */
const Ve = (t, e) => {
  const { o: n, i: o, u: s } = t;
  let i = n, c;
  const u = (v, f) => {
    const m = i, p = v, h = f || (o ? !o(m, p) : m !== p);
    return (h || s) && (i = p, c = m), [i, h, c];
  };
  return [e ? (v) => u(e(i, c), v) : u, (v) => [i, !!v, c]];
}, Dr = typeof window < "u" && typeof HTMLElement < "u" && !!window.document, Le = Dr ? window : {}, js = Math.max, Or = Math.min, In = Math.round, sn = Math.abs, bs = Math.sign, Gs = Le.cancelAnimationFrame, Jn = Le.requestAnimationFrame, on = Le.setTimeout, Hn = Le.clearTimeout, vn = (t) => typeof Le[t] < "u" ? Le[t] : void 0, Lr = vn("MutationObserver"), ws = vn("IntersectionObserver"), pt = vn("ResizeObserver"), Ot = vn("ScrollTimeline"), es = (t) => t === void 0, _n = (t) => t === null, We = (t) => typeof t == "number", Ht = (t) => typeof t == "string", mn = (t) => typeof t == "boolean", Re = (t) => typeof t == "function", Ke = (t) => Array.isArray(t), rn = (t) => typeof t == "object" && !Ke(t) && !_n(t), ts = (t) => {
  const e = !!t && t.length, n = We(e) && e > -1 && e % 1 == 0;
  return Ke(t) || !Re(t) && n ? e > 0 && rn(t) ? e - 1 in t : !0 : !1;
}, an = (t) => !!t && t.constructor === Object, ln = (t) => t instanceof HTMLElement, pn = (t) => t instanceof Element;
function ie(t, e) {
  if (ts(t))
    for (let n = 0; n < t.length && e(t[n], n, t) !== !1; n++)
      ;
  else t && ie(Object.keys(t), (n) => e(t[n], n, t));
  return t;
}
const Ws = (t, e) => t.indexOf(e) >= 0, Vt = (t, e) => t.concat(e), he = (t, e, n) => (!Ht(e) && ts(e) ? Array.prototype.push.apply(t, e) : t.push(e), t), ct = (t) => Array.from(t || []), ns = (t) => Ke(t) ? t : !Ht(t) && ts(t) ? ct(t) : [t], Rn = (t) => !!t && !t.length, Bn = (t) => ct(new Set(t)), Ie = (t, e, n) => {
  ie(t, (s) => s ? s.apply(void 0, e || []) : !0), n || (t.length = 0);
}, Ks = "paddingTop", Ys = "paddingRight", Xs = "paddingLeft", Zs = "paddingBottom", Qs = "marginLeft", Js = "marginRight", eo = "marginBottom", to = "overflowX", no = "overflowY", hn = "width", gn = "height", at = "visible", ft = "hidden", yt = "scroll", Vr = (t) => {
  const e = String(t || "");
  return e ? e[0].toUpperCase() + e.slice(1) : "";
}, bn = (t, e, n, o) => {
  if (t && e) {
    let s = !0;
    return ie(n, (i) => {
      const c = t[i], u = e[i];
      c !== u && (s = !1);
    }), s;
  }
  return !1;
}, so = (t, e) => bn(t, e, ["w", "h"]), en = (t, e) => bn(t, e, ["x", "y"]), Fr = (t, e) => bn(t, e, ["t", "r", "b", "l"]), lt = () => {
}, X = (t, ...e) => t.bind(0, ...e), ht = (t) => {
  let e;
  const n = t ? on : Jn, o = t ? Hn : Gs;
  return [(s) => {
    o(e), e = n(() => s(), Re(t) ? t() : t);
  }, () => o(e)];
}, cn = (t, e) => {
  const { _: n, p: o, v: s, S: i } = e || {};
  let c, u, l, d, v = lt;
  const f = function(x) {
    v(), Hn(c), d = c = u = void 0, v = lt, t.apply(this, x);
  }, m = (w) => i && u ? i(u, w) : w, p = () => {
    v !== lt && f(m(l) || l);
  }, h = function() {
    const x = ct(arguments), M = Re(n) ? n() : n;
    if (We(M) && M >= 0) {
      const L = Re(o) ? o() : o, y = We(L) && L >= 0, V = M > 0 ? on : Jn, H = M > 0 ? Hn : Gs, T = m(x) || x, $ = f.bind(0, T);
      let C;
      v(), s && !d ? ($(), d = !0, C = V(() => d = void 0, M)) : (C = V($, M), y && !c && (c = on(p, L))), v = () => H(C), u = l = T;
    } else
      f(x);
  };
  return h.m = p, h;
}, oo = (t, e) => Object.prototype.hasOwnProperty.call(t, e), Ue = (t) => t ? Object.keys(t) : [], oe = (t, e, n, o, s, i, c) => {
  const u = [e, n, o, s, i, c];
  return (typeof t != "object" || _n(t)) && !Re(t) && (t = {}), ie(u, (l) => {
    ie(l, (d, v) => {
      const f = l[v];
      if (t === f)
        return !0;
      const m = Ke(f);
      if (f && an(f)) {
        const p = t[v];
        let h = p;
        m && !Ke(p) ? h = [] : !m && !an(p) && (h = {}), t[v] = oe(h, f);
      } else
        t[v] = m ? f.slice() : f;
    });
  }), t;
}, ro = (t, e) => ie(oe({}, t), (n, o, s) => {
  n === void 0 ? delete s[o] : n && an(n) && (s[o] = ro(n));
}), ss = (t) => !Ue(t).length, ao = (t, e, n) => js(t, Or(e, n)), _t = (t) => Bn((Ke(t) ? t : (t || "").split(" ")).filter((e) => e)), os = (t, e) => t && t.getAttribute(e), ys = (t, e) => t && t.hasAttribute(e), Je = (t, e, n) => {
  ie(_t(e), (o) => {
    t && t.setAttribute(o, String(n || ""));
  });
}, qe = (t, e) => {
  ie(_t(e), (n) => t && t.removeAttribute(n));
}, wn = (t, e) => {
  const n = _t(os(t, e)), o = X(Je, t, e), s = (i, c) => {
    const u = new Set(n);
    return ie(_t(i), (l) => {
      u[c](l);
    }), ct(u).join(" ");
  };
  return {
    O: (i) => o(s(i, "delete")),
    $: (i) => o(s(i, "add")),
    C: (i) => {
      const c = _t(i);
      return c.reduce((u, l) => u && n.includes(l), c.length > 0);
    }
  };
}, lo = (t, e, n) => (wn(t, e).O(n), X(rs, t, e, n)), rs = (t, e, n) => (wn(t, e).$(n), X(lo, t, e, n)), dn = (t, e, n, o) => (o ? rs : lo)(t, e, n), as = (t, e, n) => wn(t, e).C(n), io = (t) => wn(t, "class"), co = (t, e) => {
  io(t).O(e);
}, ls = (t, e) => (io(t).$(e), X(co, t, e)), uo = (t, e) => {
  const n = e ? pn(e) && e : document;
  return n ? ct(n.querySelectorAll(t)) : [];
}, Ir = (t, e) => {
  const n = e ? pn(e) && e : document;
  return n && n.querySelector(t);
}, Un = (t, e) => pn(t) && t.matches(e), fo = (t) => Un(t, "body"), Nn = (t) => t ? ct(t.childNodes) : [], Ft = (t) => t && t.parentElement, gt = (t, e) => pn(t) && t.closest(e), Pn = (t) => document.activeElement, Hr = (t, e, n) => {
  const o = gt(t, e), s = t && Ir(n, o), i = gt(s, e) === o;
  return o && s ? o === t || s === t || i && gt(gt(t, n), e) !== o : !1;
}, kt = (t) => {
  ie(ns(t), (e) => {
    const n = Ft(e);
    e && n && n.removeChild(e);
  });
}, De = (t, e) => X(kt, t && e && ie(ns(e), (n) => {
  n && t.appendChild(n);
}));
let vo;
const Rr = () => vo, Br = (t) => {
  vo = t;
}, bt = (t) => {
  const e = document.createElement("div");
  return Je(e, "class", t), e;
}, _o = (t) => {
  const e = bt(), n = Rr(), o = t.trim();
  return e.innerHTML = n ? n.createHTML(o) : o, ie(Nn(e), (s) => kt(s));
}, ks = (t, e) => t.getPropertyValue(e) || t[e] || "", mo = (t) => {
  const e = t || 0;
  return isFinite(e) ? e : 0;
}, Zt = (t) => mo(parseFloat(t || "")), qn = (t) => Math.round(t * 1e4) / 1e4, po = (t) => `${qn(mo(t))}px`;
function It(t, e) {
  t && e && ie(e, (n, o) => {
    try {
      const s = t.style, i = _n(n) || mn(n) ? "" : We(n) ? po(n) : n;
      o.indexOf("--") === 0 ? s.setProperty(o, i) : s[o] = i;
    } catch {
    }
  });
}
function nt(t, e, n) {
  const o = Ht(e);
  let s = o ? "" : {};
  if (t) {
    const i = Le.getComputedStyle(t, n) || t.style;
    s = o ? ks(i, e) : ct(e).reduce((c, u) => (c[u] = ks(i, u), c), s);
  }
  return s;
}
const xs = (t, e, n) => {
  const o = e ? `${e}-` : "", s = n ? `-${n}` : "", i = `${o}top${s}`, c = `${o}right${s}`, u = `${o}bottom${s}`, l = `${o}left${s}`, d = nt(t, [i, c, u, l]);
  return {
    t: Zt(d[i]),
    r: Zt(d[c]),
    b: Zt(d[u]),
    l: Zt(d[l])
  };
}, An = (t, e) => `translate${rn(t) ? `(${t.x},${t.y})` : `${e ? "X" : "Y"}(${t})`}`, Ur = (t) => !!(t.offsetWidth || t.offsetHeight || t.getClientRects().length), Nr = {
  w: 0,
  h: 0
}, yn = (t, e) => e ? {
  w: e[`${t}Width`],
  h: e[`${t}Height`]
} : Nr, Pr = (t) => yn("inner", t || Le), wt = X(yn, "offset"), ho = X(yn, "client"), un = X(yn, "scroll"), is = (t) => {
  const e = parseFloat(nt(t, hn)) || 0, n = parseFloat(nt(t, gn)) || 0;
  return {
    w: e - In(e),
    h: n - In(n)
  };
}, Mn = (t) => t.getBoundingClientRect(), qr = (t) => !!t && Ur(t), zn = (t) => !!(t && (t[gn] || t[hn])), go = (t, e) => {
  const n = zn(t);
  return !zn(e) && n;
}, Ss = (t, e, n, o) => {
  ie(_t(e), (s) => {
    t && t.removeEventListener(s, n, o);
  });
}, ve = (t, e, n, o) => {
  var s;
  const i = (s = o && o.H) != null ? s : !0, c = o && o.I || !1, u = o && o.A || !1, l = {
    passive: i,
    capture: c
  };
  return X(Ie, _t(e).map((d) => {
    const v = u ? (f) => {
      Ss(t, d, v, c), n && n(f);
    } : n;
    return t && t.addEventListener(d, v, l), X(Ss, t, d, v, c);
  }));
}, bo = (t) => t.stopPropagation(), jn = (t) => t.preventDefault(), wo = (t) => bo(t) || jn(t), Ge = (t, e) => {
  const { x: n, y: o } = We(e) ? {
    x: e,
    y: e
  } : e || {};
  We(n) && (t.scrollLeft = n), We(o) && (t.scrollTop = o);
}, Fe = (t) => ({
  x: t.scrollLeft,
  y: t.scrollTop
}), yo = () => ({
  D: {
    x: 0,
    y: 0
  },
  M: {
    x: 0,
    y: 0
  }
}), zr = (t, e) => {
  const { D: n, M: o } = t, { w: s, h: i } = e, c = (f, m, p) => {
    let h = bs(f) * p, w = bs(m) * p;
    if (h === w) {
      const x = sn(f), M = sn(m);
      w = x > M ? 0 : w, h = x < M ? 0 : h;
    }
    return h = h === w ? 0 : h, [h + 0, w + 0];
  }, [u, l] = c(n.x, o.x, s), [d, v] = c(n.y, o.y, i);
  return {
    D: {
      x: u,
      y: d
    },
    M: {
      x: l,
      y: v
    }
  };
}, Dn = ({ D: t, M: e }) => {
  const n = (o, s) => o === 0 && o <= s;
  return {
    x: n(t.x, e.x),
    y: n(t.y, e.y)
  };
}, $s = ({ D: t, M: e }, n) => {
  const o = (s, i, c) => ao(0, 1, (s - c) / (s - i) || 0);
  return {
    x: o(t.x, e.x, n.x),
    y: o(t.y, e.y, n.y)
  };
}, Gn = (t) => {
  t && t.focus && t.focus({
    preventScroll: !0
  });
}, Cs = (t, e) => {
  ie(ns(e), t);
}, Wn = (t) => {
  const e = /* @__PURE__ */ new Map(), n = (i, c) => {
    if (i) {
      const u = e.get(i);
      Cs((l) => {
        u && u[l ? "delete" : "clear"](l);
      }, c);
    } else
      e.forEach((u) => {
        u.clear();
      }), e.clear();
  }, o = (i, c) => {
    if (Ht(i)) {
      const d = e.get(i) || /* @__PURE__ */ new Set();
      return e.set(i, d), Cs((v) => {
        Re(v) && d.add(v);
      }, c), X(n, i, c);
    }
    mn(c) && c && n();
    const u = Ue(i), l = [];
    return ie(u, (d) => {
      const v = i[d];
      v && he(l, o(d, v));
    }), X(Ie, l);
  }, s = (i, c) => {
    ie(ct(e.get(i)), (u) => {
      c && !Rn(c) ? u.apply(0, c) : u();
    });
  };
  return o(t || {}), [o, n, s];
}, ko = {}, xo = {}, jr = (t) => {
  ie(t, (e) => ie(e, (n, o) => {
    ko[o] = e[o];
  }));
}, So = (t, e, n) => Ue(t).map((o) => {
  const { static: s, instance: i } = t[o], [c, u, l] = n || [], d = n ? i : s;
  if (d) {
    const v = n ? d(c, u, e) : d(e);
    return (l || xo)[o] = v;
  }
}), Rt = (t) => xo[t], Gr = "__osOptionsValidationPlugin", Et = "data-overlayscrollbars", tn = "os-environment", Qt = `${tn}-scrollbar-hidden`, On = `${Et}-initialize`, nn = "noClipping", Es = `${Et}-body`, it = Et, Wr = "host", et = `${Et}-viewport`, Kr = to, Yr = no, Xr = "arrange", $o = "measuring", Zr = "scrolling", Co = "scrollbarHidden", Qr = "noContent", Kn = `${Et}-padding`, Ts = `${Et}-content`, cs = "os-size-observer", Jr = `${cs}-appear`, ea = `${cs}-listener`, ta = "os-trinsic-observer", na = "os-theme-none", He = "os-scrollbar", sa = `${He}-rtl`, oa = `${He}-horizontal`, ra = `${He}-vertical`, Eo = `${He}-track`, ds = `${He}-handle`, aa = `${He}-visible`, la = `${He}-cornerless`, As = `${He}-interaction`, Ms = `${He}-unusable`, Yn = `${He}-auto-hide`, Ds = `${Yn}-hidden`, Os = `${He}-wheel`, ia = `${Eo}-interactive`, ca = `${ds}-interactive`, da = "__osSizeObserverPlugin", ua = (t, e) => {
  const { T: n } = e, [o, s] = t("showNativeOverlaidScrollbars");
  return [o && n.x && n.y, s];
}, xt = (t) => t.indexOf(at) === 0, fa = (t, e) => {
  const n = (s, i, c, u) => {
    const l = s === at ? ft : s.replace(`${at}-`, ""), d = xt(s), v = xt(c);
    return !i && !u ? ft : d && v ? at : d ? i && u ? l : i ? at : ft : i ? l : v && u ? at : ft;
  }, o = {
    x: n(e.x, t.x, e.y, t.y),
    y: n(e.y, t.y, e.x, t.x)
  };
  return {
    k: o,
    R: {
      x: o.x === yt,
      y: o.y === yt
    }
  };
}, To = "__osScrollbarsHidingPlugin", va = "__osClickScrollPlugin", Ls = (t) => JSON.stringify(t, (e, n) => {
  if (Re(n))
    throw 0;
  return n;
}), Vs = (t, e) => t ? `${e}`.split(".").reduce((n, o) => n && oo(n, o) ? n[o] : void 0, t) : void 0, _a = {
  paddingAbsolute: !1,
  showNativeOverlaidScrollbars: !1,
  update: {
    elementEvents: [["img", "load"]],
    debounce: [0, 33],
    attributes: null,
    ignoreMutation: null
  },
  overflow: {
    x: "scroll",
    y: "scroll"
  },
  scrollbars: {
    theme: "os-theme-dark",
    visibility: "auto",
    autoHide: "never",
    autoHideDelay: 1300,
    autoHideSuspend: !1,
    dragScroll: !0,
    clickScroll: !1,
    pointers: ["mouse", "touch", "pen"]
  }
}, Ao = (t, e) => {
  const n = {}, o = Vt(Ue(e), Ue(t));
  return ie(o, (s) => {
    const i = t[s], c = e[s];
    if (rn(i) && rn(c))
      oe(n[s] = {}, Ao(i, c)), ss(n[s]) && delete n[s];
    else if (oo(e, s) && c !== i) {
      let u = !0;
      if (Ke(i) || Ke(c))
        try {
          Ls(i) === Ls(c) && (u = !1);
        } catch {
        }
      u && (n[s] = c);
    }
  }), n;
}, Fs = (t, e, n) => (o) => [Vs(t, o), n || Vs(e, o) !== void 0];
let Mo;
const ma = () => Mo, pa = (t) => {
  Mo = t;
};
let Ln;
const ha = () => {
  const t = (y, V, H) => {
    De(document.body, y), De(document.body, y);
    const E = ho(y), T = wt(y), $ = is(V);
    return H && kt(y), {
      x: T.h - E.h + $.h,
      y: T.w - E.w + $.w
    };
  }, e = (y) => {
    let V = !1;
    const H = ls(y, Qt);
    try {
      V = nt(y, "scrollbar-width") === "none" || nt(y, "display", "::-webkit-scrollbar") === "none";
    } catch {
    }
    return H(), V;
  }, n = `.${tn}{scroll-behavior:auto!important;position:fixed;opacity:0;visibility:hidden;overflow:scroll;height:200px;width:200px;z-index:-1}.${tn} div{width:200%;height:200%;margin:10px 0}.${Qt}{scrollbar-width:none!important}.${Qt}::-webkit-scrollbar,.${Qt}::-webkit-scrollbar-corner{appearance:none!important;display:none!important;width:0!important;height:0!important}`, s = _o(`<div class="${tn}"><div></div><style>${n}</style></div>`)[0], i = s.firstChild, c = s.lastChild, u = ma();
  u && (c.nonce = u);
  const [l, , d] = Wn(), [v, f] = Ve({
    o: t(s, i),
    i: en
  }, X(t, s, i, !0)), [m] = f(), p = e(s), h = {
    x: m.x === 0,
    y: m.y === 0
  }, w = {
    elements: {
      host: null,
      padding: !p,
      viewport: (y) => p && fo(y) && y,
      content: !1
    },
    scrollbars: {
      slot: !0
    },
    cancel: {
      nativeScrollbarsOverlaid: !1,
      body: null
    }
  }, x = oe({}, _a), M = X(oe, {}, x), I = X(oe, {}, w), L = {
    N: m,
    T: h,
    P: p,
    G: !!Ot,
    K: X(l, "r"),
    Z: I,
    tt: (y) => oe(w, y) && I(),
    nt: M,
    ot: (y) => oe(x, y) && M(),
    st: oe({}, w),
    et: oe({}, x)
  };
  if (qe(s, "style"), kt(s), ve(Le, "resize", () => {
    d("r", []);
  }), Re(Le.matchMedia) && !p && (!h.x || !h.y)) {
    const y = (V) => {
      const H = Le.matchMedia(`(resolution: ${Le.devicePixelRatio}dppx)`);
      ve(H, "change", () => {
        V(), y(V);
      }, {
        A: !0
      });
    };
    y(() => {
      const [V, H] = v();
      oe(L.N, V), d("r", [H]);
    });
  }
  return L;
}, Ye = () => (Ln || (Ln = ha()), Ln), ga = (t, e, n) => {
  let o = !1;
  const s = n ? /* @__PURE__ */ new WeakMap() : !1, i = () => {
    o = !0;
  }, c = (u) => {
    if (s && n) {
      const l = n.map((d) => {
        const [v, f] = d || [];
        return [f && v ? (u || uo)(v, t) : [], f];
      });
      ie(l, (d) => ie(d[0], (v) => {
        const f = d[1], m = s.get(v) || [];
        if (t.contains(v) && f) {
          const h = ve(v, f, (w) => {
            o ? (h(), s.delete(v)) : e(w);
          });
          s.set(v, he(m, h));
        } else
          Ie(m), s.delete(v);
      }));
    }
  };
  return c(), [i, c];
}, Is = (t, e, n, o) => {
  let s = !1;
  const { ct: i, rt: c, lt: u, it: l, ut: d, ft: v } = o || {}, f = cn(() => s && n(!0), {
    _: 33,
    p: 99
  }), [m, p] = ga(t, f, u), h = i || [], w = c || [], x = Vt(h, w), M = (L, y) => {
    if (!Rn(y)) {
      const V = d || lt, H = v || lt, E = [], T = [];
      let $ = !1, C = !1;
      if (ie(y, (F) => {
        const { attributeName: D, target: R, type: k, oldValue: U, addedNodes: P, removedNodes: ee } = F, se = k === "attributes", ne = k === "childList", me = t === R, Y = se && D, S = Y && os(R, D || ""), B = Ht(S) ? S : null, q = Y && U !== B, A = Ws(w, D) && q;
        if (e && (ne || !me)) {
          const G = se && q, z = G && l && Un(R, l), Q = (z ? !V(R, D, U, B) : !se || G) && !H(F, !!z, t, o);
          ie(P, (ae) => he(E, ae)), ie(ee, (ae) => he(E, ae)), C = C || Q;
        }
        !e && me && q && !V(R, D, U, B) && (he(T, D), $ = $ || A);
      }), p((F) => Bn(E).reduce((D, R) => (he(D, uo(F, R)), Un(R, F) ? he(D, R) : D), [])), e)
        return !L && C && n(!1), [!1];
      if (!Rn(T) || $) {
        const F = [Bn(T), $];
        return L || n.apply(0, F), F;
      }
    }
  }, I = new Lr(X(M, !1));
  return [() => (I.observe(t, {
    attributes: !0,
    attributeOldValue: !0,
    attributeFilter: x,
    subtree: e,
    childList: e,
    characterData: e
  }), s = !0, () => {
    s && (m(), I.disconnect(), s = !1);
  }), () => {
    if (s)
      return f.m(), M(!0, I.takeRecords());
  }];
};
let ut = null;
const Do = (t, e, n) => {
  const { _t: o } = n || {}, s = Rt(da), [i] = Ve({
    o: !1,
    u: !0
  });
  return () => {
    const c = [], l = _o(`<div class="${cs}"><div class="${ea}"></div></div>`)[0], d = l.firstChild, v = (f) => {
      const m = f instanceof ResizeObserverEntry;
      let p = !1, h = !1;
      if (m) {
        const [w, , x] = i(f.contentRect), M = zn(w);
        h = go(w, x), p = !h && !M;
      } else
        h = f === !0;
      p || e({
        dt: !0,
        _t: h
      });
    };
    if (pt) {
      if (!mn(ut)) {
        const h = new pt(lt);
        h.observe(t, {
          get box() {
            ut = !0;
          }
        }), ut = ut || !1, h.disconnect();
      }
      const f = cn(v, {
        _: 0,
        p: 0
      }), m = (h) => f(h.pop()), p = new pt(m);
      if (p.observe(ut ? t : d), he(c, [() => p.disconnect(), !ut && De(t, l)]), ut) {
        const h = new pt(m);
        h.observe(t, {
          box: "border-box"
        }), he(c, () => h.disconnect());
      }
    } else if (s) {
      const [f, m] = s(d, v, o);
      he(c, Vt([ls(l, Jr), ve(l, "animationstart", f), De(t, l)], m));
    } else
      return lt;
    return X(Ie, c);
  };
}, ba = (t, e) => {
  let n;
  const o = (l) => l.h === 0 || l.isIntersecting || l.intersectionRatio > 0, s = bt(ta), [i] = Ve({
    o: !1
  }), c = (l, d) => {
    if (l) {
      const v = i(o(l)), [, f] = v;
      return f && !d && e(v) && [v];
    }
  }, u = (l, d) => c(d.pop(), l);
  return [() => {
    const l = [];
    if (ws)
      n = new ws(X(u, !1), {
        root: t
      }), n.observe(s), he(l, () => {
        n.disconnect();
      });
    else {
      const d = () => {
        const v = wt(s);
        c(v);
      };
      he(l, Do(s, d)()), d();
    }
    return X(Ie, he(l, De(t, s)));
  }, () => n && u(!0, n.takeRecords())];
}, wa = (t, e, n, o) => {
  let s, i, c, u, l, d;
  const v = `[${it}]`, f = `[${et}]`, m = ["id", "class", "style", "open", "wrap", "cols", "rows"], { vt: p, ht: h, U: w, gt: x, bt: M, L: I, wt: L, yt: y, St: V, Ot: H } = t, E = (A) => nt(A, "direction") === "rtl", T = {
    $t: !1,
    F: E(p)
  }, $ = Ye(), C = Rt(To), [F] = Ve({
    i: so,
    o: {
      w: 0,
      h: 0
    }
  }, () => {
    const A = C && C.V(t, e, T, $, n).W, z = !(L && I) && as(h, it, nn), W = !I && y(Xr), Q = W && Fe(x), ae = Q && H(), fe = V($o, z), ce = W && A && A()[0], Ee = un(w), te = is(w);
    return ce && ce(), Ge(x, Q), ae && ae(), z && fe(), {
      w: Ee.w + te.w,
      h: Ee.h + te.h
    };
  }), D = cn(o, {
    _: () => s,
    p: () => i,
    S(A, G) {
      const [z] = A, [W] = G;
      return [Vt(Ue(z), Ue(W)).reduce((Q, ae) => (Q[ae] = z[ae] || W[ae], Q), {})];
    }
  }), R = (A) => {
    const G = E(p);
    oe(A, {
      Ct: d !== G
    }), oe(T, {
      F: G
    }), d = G;
  }, k = (A, G) => {
    const [z, W] = A, Q = {
      xt: W
    };
    return oe(T, {
      $t: z
    }), G || o(Q), Q;
  }, U = ({ dt: A, _t: G }) => {
    const W = !(A && !G) && $.P ? D : o, Q = {
      dt: A || G,
      _t: G
    };
    R(Q), W(Q);
  }, P = (A, G) => {
    const [, z] = F(), W = {
      Ht: z
    };
    return R(W), z && !G && (A ? o : D)(W), W;
  }, ee = (A, G, z) => {
    const W = {
      Et: G
    };
    return R(W), G && !z && D(W), W;
  }, [se, ne] = M ? ba(h, k) : [], me = !I && Do(h, U, {
    _t: !0
  }), [Y, S] = Is(h, !1, ee, {
    rt: m,
    ct: m
  }), B = I && pt && new pt((A) => {
    const G = A[A.length - 1].contentRect;
    U({
      dt: !0,
      _t: go(G, l)
    }), l = G;
  }), q = cn(() => {
    const [, A] = F();
    o({
      Ht: A
    });
  }, {
    _: 222,
    v: !0
  });
  return [() => {
    B && B.observe(h);
    const A = me && me(), G = se && se(), z = Y(), W = $.K((Q) => {
      Q ? D({
        zt: Q
      }) : q();
    });
    return () => {
      B && B.disconnect(), A && A(), G && G(), u && u(), z(), W();
    };
  }, ({ It: A, At: G, Dt: z }) => {
    const W = {}, [Q] = A("update.ignoreMutation"), [ae, fe] = A("update.attributes"), [ce, Ee] = A("update.elementEvents"), [te, we] = A("update.debounce"), Me = Ee || fe, ke = G || z, xe = (ge) => Re(Q) && Q(ge);
    if (Me) {
      c && c(), u && u();
      const [ge, be] = Is(M || w, !0, P, {
        ct: Vt(m, ae || []),
        lt: ce,
        it: v,
        ft: (pe, de) => {
          const { target: Se, attributeName: Ae } = pe;
          return (!de && Ae && !I ? Hr(Se, v, f) : !1) || !!gt(Se, `.${He}`) || !!xe(pe);
        }
      });
      u = ge(), c = be;
    }
    if (we)
      if (D.m(), Ke(te)) {
        const ge = te[0], be = te[1];
        s = We(ge) && ge, i = We(be) && be;
      } else We(te) ? (s = te, i = !1) : (s = !1, i = !1);
    if (ke) {
      const ge = S(), be = ne && ne(), pe = c && c();
      ge && oe(W, ee(ge[0], ge[1], ke)), be && oe(W, k(be[0], ke)), pe && oe(W, P(pe[0], ke));
    }
    return R(W), W;
  }, T];
}, Oo = (t, e) => Re(e) ? e.apply(0, t) : e, ya = (t, e, n, o) => {
  const s = es(o) ? n : o;
  return Oo(t, s) || e.apply(0, t);
}, Lo = (t, e, n, o) => {
  const s = es(o) ? n : o, i = Oo(t, s);
  return !!i && (ln(i) ? i : e.apply(0, t));
}, ka = (t, e) => {
  const { nativeScrollbarsOverlaid: n, body: o } = e || {}, { T: s, P: i, Z: c } = Ye(), { nativeScrollbarsOverlaid: u, body: l } = c().cancel, d = n ?? u, v = es(o) ? l : o, f = (s.x || s.y) && d, m = t && (_n(v) ? !i : v);
  return !!f || !!m;
}, xa = (t, e, n, o) => {
  const s = "--os-viewport-percent", i = "--os-scroll-percent", c = "--os-scroll-direction", { Z: u } = Ye(), { scrollbars: l } = u(), { slot: d } = l, { vt: v, ht: f, U: m, Mt: p, gt: h, wt: w, L: x } = e, { scrollbars: M } = p ? {} : t, { slot: I } = M || {}, L = [], y = [], V = [], H = Lo([v, f, m], () => x && w ? v : f, d, I), E = (Y) => {
    if (Ot) {
      let S = null, B = [];
      const q = new Ot({
        source: h,
        axis: Y
      }), A = () => {
        S && S.cancel(), S = null;
      };
      return {
        Rt: (z) => {
          const { Tt: W } = n, Q = Dn(W)[Y], ae = Y === "x", fe = [An(0, ae), An(`calc(100cq${ae ? "w" : "h"} + -100%)`, ae)], ce = Q ? fe : fe.reverse();
          return B[0] === ce[0] && B[1] === ce[1] || (A(), B = ce, S = z.kt.animate({
            clear: ["left"],
            transform: ce
          }, {
            timeline: q
          })), A;
        }
      };
    }
  }, T = {
    x: E("x"),
    y: E("y")
  }, $ = () => {
    const { Vt: Y, Lt: S } = n, B = (q, A) => ao(0, 1, q / (q + A) || 0);
    return {
      x: B(S.x, Y.x),
      y: B(S.y, Y.y)
    };
  }, C = (Y, S, B) => {
    const q = B ? ls : co;
    ie(Y, (A) => {
      q(A.Ut, S);
    });
  }, F = (Y, S) => {
    ie(Y, (B) => {
      const [q, A] = S(B);
      It(q, A);
    });
  }, D = (Y, S, B) => {
    const q = mn(B), A = q ? B : !0, G = q ? !B : !0;
    A && C(y, Y, S), G && C(V, Y, S);
  }, R = () => {
    const Y = $(), S = (B) => (q) => [q.Ut, {
      [s]: qn(B) + ""
    }];
    F(y, S(Y.x)), F(V, S(Y.y));
  }, k = () => {
    if (!Ot) {
      const { Tt: Y } = n, S = $s(Y, Fe(h)), B = (q) => (A) => [A.Ut, {
        [i]: qn(q) + ""
      }];
      F(y, B(S.x)), F(V, B(S.y));
    }
  }, U = () => {
    const { Tt: Y } = n, S = Dn(Y), B = (q) => (A) => [A.Ut, {
      [c]: q ? "0" : "1"
    }];
    F(y, B(S.x)), F(V, B(S.y)), Ot && (y.forEach(T.x.Rt), V.forEach(T.y.Rt));
  }, P = () => {
    if (x && !w) {
      const { Vt: Y, Tt: S } = n, B = Dn(S), q = $s(S, Fe(h)), A = (G) => {
        const { Ut: z } = G, W = Ft(z) === m && z, Q = (ae, fe, ce) => {
          const Ee = fe * ae;
          return po(ce ? Ee : -Ee);
        };
        return [W, W && {
          transform: An({
            x: Q(q.x, Y.x, B.x),
            y: Q(q.y, Y.y, B.y)
          })
        }];
      };
      F(y, A), F(V, A);
    }
  }, ee = (Y) => {
    const S = Y ? "x" : "y", q = bt(`${He} ${Y ? oa : ra}`), A = bt(Eo), G = bt(ds), z = {
      Ut: q,
      Pt: A,
      kt: G
    }, W = T[S];
    return he(Y ? y : V, z), he(L, [De(q, A), De(A, G), X(kt, q), W && W.Rt(z), o(z, D, Y)]), z;
  }, se = X(ee, !0), ne = X(ee, !1), me = () => (De(H, y[0].Ut), De(H, V[0].Ut), X(Ie, L));
  return se(), ne(), [{
    Nt: R,
    qt: k,
    Bt: U,
    Ft: P,
    jt: D,
    Xt: {
      Yt: y,
      Wt: se,
      Jt: X(F, y)
    },
    Gt: {
      Yt: V,
      Wt: ne,
      Jt: X(F, V)
    }
  }, me];
}, Sa = (t, e, n, o) => (s, i, c) => {
  const { ht: u, U: l, L: d, gt: v, Kt: f, Ot: m } = e, { Ut: p, Pt: h, kt: w } = s, [x, M] = ht(333), [I, L] = ht(444), y = (E) => {
    Re(v.scrollBy) && v.scrollBy({
      behavior: "smooth",
      left: E.x,
      top: E.y
    });
  }, V = () => {
    const E = "pointerup pointercancel lostpointercapture", T = `client${c ? "X" : "Y"}`, $ = c ? hn : gn, C = c ? "left" : "top", F = c ? "w" : "h", D = c ? "x" : "y", R = (U, P) => (ee) => {
      const { Vt: se } = n, ne = wt(h)[F] - wt(w)[F], Y = P * ee / ne * se[D];
      Ge(v, {
        [D]: U + Y
      });
    }, k = [];
    return ve(h, "pointerdown", (U) => {
      const P = gt(U.target, `.${ds}`) === w, ee = P ? w : h, se = t.scrollbars, ne = se[P ? "dragScroll" : "clickScroll"], { button: me, isPrimary: Y, pointerType: S } = U, { pointers: B } = se;
      if (me === 0 && Y && ne && (B || []).includes(S)) {
        Ie(k), L();
        const A = !P && (U.shiftKey || ne === "instant"), G = X(Mn, w), z = X(Mn, h), W = (de, Se) => (de || G())[C] - (Se || z())[C], Q = In(Mn(v)[$]) / wt(v)[F] || 1, ae = R(Fe(v)[D], 1 / Q), fe = U[T], ce = G(), Ee = z(), te = ce[$], we = W(ce, Ee) + te / 2, Me = fe - Ee[C], ke = P ? 0 : Me - we, xe = (de) => {
          Ie(pe), ee.releasePointerCapture(de.pointerId);
        }, ge = P || A, be = m(), pe = [ve(f, E, xe), ve(f, "selectstart", (de) => jn(de), {
          H: !1
        }), ve(h, E, xe), ge && ve(h, "pointermove", (de) => ae(ke + (de[T] - fe))), ge && (() => {
          const de = Fe(v);
          be();
          const Se = Fe(v), Ae = {
            x: Se.x - de.x,
            y: Se.y - de.y
          };
          (sn(Ae.x) > 3 || sn(Ae.y) > 3) && (m(), Ge(v, de), y(Ae), I(be));
        })];
        if (ee.setPointerCapture(U.pointerId), A)
          ae(ke);
        else if (!P) {
          const de = Rt(va);
          if (de) {
            const Se = de(ae, ke, te, (Ae) => {
              Ae ? be() : he(pe, be);
            });
            he(pe, Se), he(k, X(Se, !0));
          }
        }
      }
    });
  };
  let H = !0;
  return X(Ie, [ve(w, "pointermove pointerleave", o), ve(p, "pointerenter", () => {
    i(As, !0);
  }), ve(p, "pointerleave pointercancel", () => {
    i(As, !1);
  }), !d && ve(p, "mousedown", () => {
    const E = Pn();
    (ys(E, et) || ys(E, it) || E === document.body) && on(X(Gn, l), 25);
  }), ve(p, "wheel", (E) => {
    const { deltaX: T, deltaY: $, deltaMode: C } = E;
    H && C === 0 && Ft(p) === u && y({
      x: T,
      y: $
    }), H = !1, i(Os, !0), x(() => {
      H = !0, i(Os);
    }), jn(E);
  }, {
    H: !1,
    I: !0
  }), ve(p, "pointerdown", X(ve, f, "click", wo, {
    A: !0,
    I: !0,
    H: !1
  }), {
    I: !0
  }), V(), M, L]);
}, $a = (t, e, n, o, s, i) => {
  let c, u, l, d, v, f = lt, m = 0;
  const p = ["mouse", "pen"], h = (S) => p.includes(S.pointerType), [w, x] = ht(), [M, I] = ht(100), [L, y] = ht(100), [V, H] = ht(() => m), [E, T] = xa(t, s, o, Sa(e, s, o, (S) => h(S) && se())), { ht: $, Qt: C, wt: F } = s, { jt: D, Nt: R, qt: k, Bt: U, Ft: P } = E, ee = (S, B) => {
    if (H(), S)
      D(Ds);
    else {
      const q = X(D, Ds, !0);
      m > 0 && !B ? V(q) : q();
    }
  }, se = () => {
    (l ? !c : !d) && (ee(!0), M(() => {
      ee(!1);
    }));
  }, ne = (S) => {
    D(Yn, S, !0), D(Yn, S, !1);
  }, me = (S) => {
    h(S) && (c = l, l && ee(!0));
  }, Y = [H, I, y, x, () => f(), ve($, "pointerover", me, {
    A: !0
  }), ve($, "pointerenter", me), ve($, "pointerleave", (S) => {
    h(S) && (c = !1, l && ee(!1));
  }), ve($, "pointermove", (S) => {
    h(S) && u && se();
  }), ve(C, "scroll", (S) => {
    w(() => {
      k(), se();
    }), i(S), P();
  })];
  return [() => X(Ie, he(Y, T())), ({ It: S, Dt: B, Zt: q, tn: A }) => {
    const { nn: G, sn: z, en: W, cn: Q } = A || {}, { Ct: ae, _t: fe } = q || {}, { F: ce } = n, { T: Ee } = Ye(), { k: te, rn: we } = o, [Me, ke] = S("showNativeOverlaidScrollbars"), [xe, ge] = S("scrollbars.theme"), [be, pe] = S("scrollbars.visibility"), [de, Se] = S("scrollbars.autoHide"), [Ae, Tt] = S("scrollbars.autoHideSuspend"), [Bt] = S("scrollbars.autoHideDelay"), [Ut, Nt] = S("scrollbars.dragScroll"), [dt, At] = S("scrollbars.clickScroll"), [Pt, xn] = S("overflow"), Sn = fe && !B, $n = we.x || we.y, Pe = G || z || Q || ae || B, Cn = W || pe || xn, qt = Me && Ee.x && Ee.y, zt = (ot, Mt, Dt) => {
      const jt = ot.includes(yt) && (be === at || be === "auto" && Mt === yt);
      return D(aa, jt, Dt), jt;
    };
    if (m = Bt, Sn && (Ae && $n ? (ne(!1), f(), L(() => {
      f = ve(C, "scroll", X(ne, !0), {
        A: !0
      });
    })) : ne(!0)), ke && D(na, qt), ge && (D(v), D(xe, !0), v = xe), Tt && !Ae && ne(!0), Se && (u = de === "move", l = de === "leave", d = de === "never", ee(d, !0)), Nt && D(ca, Ut), At && D(ia, !!dt), Cn) {
      const ot = zt(Pt.x, te.x, !0), Mt = zt(Pt.y, te.y, !1);
      D(la, !(ot && Mt));
    }
    Pe && (k(), R(), P(), Q && U(), D(Ms, !we.x, !0), D(Ms, !we.y, !1), D(sa, ce && !F));
  }, {}, E];
}, Ca = (t) => {
  const e = Ye(), { Z: n, P: o } = e, { elements: s } = n(), { padding: i, viewport: c, content: u } = s, l = ln(t), d = l ? {} : t, { elements: v } = d, { padding: f, viewport: m, content: p } = v || {}, h = l ? t : d.target, w = fo(h), x = h.ownerDocument, M = x.documentElement, I = () => x.defaultView || Le, L = X(ya, [h]), y = X(Lo, [h]), V = X(bt, ""), H = X(L, V, c), E = X(y, V, u), T = (te) => {
    const we = wt(te), Me = un(te), ke = nt(te, to), xe = nt(te, no);
    return Me.w - we.w > 0 && !xt(ke) || Me.h - we.h > 0 && !xt(xe);
  }, $ = H(m), C = $ === h, F = C && w, D = !C && E(p), R = !C && $ === D, k = F ? M : $, U = F ? k : h, P = !C && y(V, i, f), ee = !R && D, se = [ee, k, P, U].map((te) => ln(te) && !Ft(te) && te), ne = (te) => te && Ws(se, te), me = !ne(k) && T(k) ? k : h, Y = F ? M : k, B = {
    vt: h,
    ht: U,
    U: k,
    ln: P,
    bt: ee,
    gt: Y,
    Qt: F ? x : k,
    an: w ? M : me,
    Kt: x,
    wt: w,
    Mt: l,
    L: C,
    un: I,
    yt: (te) => as(k, et, te),
    St: (te, we) => dn(k, et, te, we),
    Ot: () => dn(Y, et, Zr, !0)
  }, { vt: q, ht: A, ln: G, U: z, bt: W } = B, Q = [() => {
    qe(A, [it, On]), qe(q, On), w && qe(M, [On, it]);
  }];
  let ae = Nn([W, z, G, A, q].find((te) => te && !ne(te)));
  const fe = F ? q : W || z, ce = X(Ie, Q);
  return [B, () => {
    const te = I(), we = Pn(), Me = (pe) => {
      De(Ft(pe), Nn(pe)), kt(pe);
    }, ke = (pe) => ve(pe, "focusin focusout focus blur", wo, {
      I: !0,
      H: !1
    }), xe = "tabindex", ge = os(z, xe), be = ke(we);
    return Je(A, it, C ? "" : Wr), Je(G, Kn, ""), Je(z, et, ""), Je(W, Ts, ""), C || (Je(z, xe, ge || "-1"), w && Je(M, Es, "")), De(fe, ae), De(A, G), De(G || A, !C && z), De(z, W), he(Q, [be, () => {
      const pe = Pn(), de = ne(z), Se = de && pe === z ? q : pe, Ae = ke(Se);
      qe(G, Kn), qe(W, Ts), qe(z, et), w && qe(M, Es), ge ? Je(z, xe, ge) : qe(z, xe), ne(W) && Me(W), de && Me(z), ne(G) && Me(G), Gn(Se), Ae();
    }]), o && !C && (rs(z, et, Co), he(Q, X(qe, z, et))), Gn(!C && w && we === q && te.top === te ? z : we), be(), ae = 0, ce;
  }, ce];
}, Ea = ({ bt: t }) => ({ Zt: e, fn: n, Dt: o }) => {
  const { xt: s } = e || {}, { $t: i } = n;
  t && (s || o) && It(t, {
    [gn]: i && "100%"
  });
}, Ta = ({ ht: t, ln: e, U: n, L: o }, s) => {
  const [i, c] = Ve({
    i: Fr,
    o: xs()
  }, X(xs, t, "padding", ""));
  return ({ It: u, Zt: l, fn: d, Dt: v }) => {
    let [f, m] = c(v);
    const { P: p } = Ye(), { dt: h, Ht: w, Ct: x } = l || {}, { F: M } = d, [I, L] = u("paddingAbsolute");
    (h || m || (v || w)) && ([f, m] = i(v));
    const V = !o && (L || x || m);
    if (V) {
      const H = !I || !e && !p, E = f.r + f.l, T = f.t + f.b, $ = {
        [Js]: H && !M ? -E : 0,
        [eo]: H ? -T : 0,
        [Qs]: H && M ? -E : 0,
        top: H ? -f.t : 0,
        right: H ? M ? -f.r : "auto" : 0,
        left: H ? M ? "auto" : -f.l : 0,
        [hn]: H && `calc(100% + ${E}px)`
      }, C = {
        [Ks]: H ? f.t : 0,
        [Ys]: H ? f.r : 0,
        [Zs]: H ? f.b : 0,
        [Xs]: H ? f.l : 0
      };
      It(e || n, $), It(n, C), oe(s, {
        ln: f,
        _n: !H,
        j: e ? C : oe({}, $, C)
      });
    }
    return {
      dn: V
    };
  };
}, Aa = (t, e) => {
  const n = Ye(), { ht: o, ln: s, U: i, L: c, Qt: u, gt: l, wt: d, St: v, un: f } = t, { P: m } = n, p = d && c, h = X(js, 0), w = {
    display: () => !1,
    direction: (S) => S !== "ltr",
    flexDirection: (S) => S.endsWith("-reverse"),
    writingMode: (S) => S !== "horizontal-tb"
  }, x = Ue(w), M = {
    i: so,
    o: {
      w: 0,
      h: 0
    }
  }, I = {
    i: en,
    o: {}
  }, L = (S) => {
    v($o, !p && S);
  }, y = (S) => {
    if (!x.some((fe) => {
      const ce = S[fe];
      return ce && w[fe](ce);
    }))
      return {
        D: {
          x: 0,
          y: 0
        },
        M: {
          x: 1,
          y: 1
        }
      };
    L(!0);
    const q = Fe(l), A = v(Qr, !0), G = ve(u, yt, (fe) => {
      const ce = Fe(l);
      fe.isTrusted && ce.x === q.x && ce.y === q.y && bo(fe);
    }, {
      I: !0,
      A: !0
    });
    Ge(l, {
      x: 0,
      y: 0
    }), A();
    const z = Fe(l), W = un(l);
    Ge(l, {
      x: W.w,
      y: W.h
    });
    const Q = Fe(l);
    Ge(l, {
      x: Q.x - z.x < 1 && -W.w,
      y: Q.y - z.y < 1 && -W.h
    });
    const ae = Fe(l);
    return Ge(l, q), Jn(() => G()), {
      D: z,
      M: ae
    };
  }, V = (S, B) => {
    const q = Le.devicePixelRatio % 1 !== 0 ? 1 : 0, A = {
      w: h(S.w - B.w),
      h: h(S.h - B.h)
    };
    return {
      w: A.w > q ? A.w : 0,
      h: A.h > q ? A.h : 0
    };
  }, [H, E] = Ve(M, X(is, i)), [T, $] = Ve(M, X(un, i)), [C, F] = Ve(M), [D] = Ve(I), [R, k] = Ve(M), [U] = Ve(I), [P] = Ve({
    i: (S, B) => bn(S, B, x),
    o: {}
  }, () => qr(i) ? nt(i, x) : {}), [ee, se] = Ve({
    i: (S, B) => en(S.D, B.D) && en(S.M, B.M),
    o: yo()
  }), ne = Rt(To), me = (S, B) => `${B ? Kr : Yr}${Vr(S)}`, Y = (S) => {
    const B = (A) => [at, ft, yt].map((G) => me(G, A)), q = B(!0).concat(B()).join(" ");
    v(q), v(Ue(S).map((A) => me(S[A], A === "x")).join(" "), !0);
  };
  return ({ It: S, Zt: B, fn: q, Dt: A }, { dn: G }) => {
    const { dt: z, Ht: W, Ct: Q, _t: ae, zt: fe } = B || {}, ce = ne && ne.V(t, e, q, n, S), { Y: Ee, W: te, J: we } = ce || {}, [Me, ke] = ua(S, n), [xe, ge] = S("overflow"), be = xt(xe.x), pe = xt(xe.y), de = z || G || W || Q || fe || ke;
    let Se = E(A), Ae = $(A), Tt = F(A), Bt = k(A);
    if (ke && m && v(Co, !Me), de) {
      as(o, it, nn) && L(!0);
      const [ps] = te ? te() : [], [Gt] = Se = H(A), [Wt] = Ae = T(A), Kt = ho(i), Yt = p && Pr(f()), or = {
        w: h(Wt.w + Gt.w),
        h: h(Wt.h + Gt.h)
      }, hs = {
        w: h((Yt ? Yt.w : Kt.w + h(Kt.w - Wt.w)) + Gt.w),
        h: h((Yt ? Yt.h : Kt.h + h(Kt.h - Wt.h)) + Gt.h)
      };
      ps && ps(), Bt = R(hs), Tt = C(V(or, hs), A);
    }
    const [Ut, Nt] = Bt, [dt, At] = Tt, [Pt, xn] = Ae, [Sn, $n] = Se, [Pe, Cn] = D({
      x: dt.w > 0,
      y: dt.h > 0
    }), qt = be && pe && (Pe.x || Pe.y) || be && Pe.x && !Pe.y || pe && Pe.y && !Pe.x, zt = G || Q || fe || $n || xn || Nt || At || ge || ke || de, ot = fa(Pe, xe), [Mt, Dt] = U(ot.k), [jt, tr] = P(A), ms = Q || ae || tr || Cn || A, [nr, sr] = ms ? ee(y(jt), A) : se();
    return zt && (Dt && Y(ot.k), we && Ee && It(i, we(ot, q, Ee(ot, Pt, Sn)))), L(!1), dn(o, it, nn, qt), dn(s, Kn, nn, qt), oe(e, {
      k: Mt,
      Lt: {
        x: Ut.w,
        y: Ut.h
      },
      Vt: {
        x: dt.w,
        y: dt.h
      },
      rn: Pe,
      Tt: zr(nr, dt)
    }), {
      en: Dt,
      nn: Nt,
      sn: At,
      cn: sr || At,
      pn: ms
    };
  };
}, Ma = (t) => {
  const [e, n, o] = Ca(t), s = {
    ln: {
      t: 0,
      r: 0,
      b: 0,
      l: 0
    },
    _n: !1,
    j: {
      [Js]: 0,
      [eo]: 0,
      [Qs]: 0,
      [Ks]: 0,
      [Ys]: 0,
      [Zs]: 0,
      [Xs]: 0
    },
    Lt: {
      x: 0,
      y: 0
    },
    Vt: {
      x: 0,
      y: 0
    },
    k: {
      x: ft,
      y: ft
    },
    rn: {
      x: !1,
      y: !1
    },
    Tt: yo()
  }, { vt: i, gt: c, L: u, Ot: l } = e, { P: d, T: v } = Ye(), f = !d && (v.x || v.y), m = [Ea(e), Ta(e, s), Aa(e, s)];
  return [n, (p) => {
    const h = {}, x = f && Fe(c), M = x && l();
    return ie(m, (I) => {
      oe(h, I(p, h) || {});
    }), Ge(c, x), M && M(), u || Ge(i, 0), h;
  }, s, e, o];
}, Da = (t, e, n, o, s) => {
  let i = !1;
  const c = Fs(e, {}), [u, l, d, v, f] = Ma(t), [m, p, h] = wa(v, d, c, (y) => {
    L({}, y);
  }), [w, x, , M] = $a(t, e, h, d, v, s), I = (y) => Ue(y).some((V) => !!y[V]), L = (y, V) => {
    if (n())
      return !1;
    const { vn: H, Dt: E, At: T, hn: $ } = y, C = H || {}, F = !!E || !i, D = {
      It: Fs(e, C, F),
      vn: C,
      Dt: F
    };
    if ($)
      return x(D), !1;
    const R = V || p(oe({}, D, {
      At: T
    })), k = l(oe({}, D, {
      fn: h,
      Zt: R
    }));
    x(oe({}, D, {
      Zt: R,
      tn: k
    }));
    const U = I(R), P = I(k), ee = U || P || !ss(C) || F;
    return i = !0, ee && o(y, {
      Zt: R,
      tn: k
    }), ee;
  };
  return [() => {
    const { an: y, gt: V, Ot: H } = v, E = Fe(y), T = [m(), u(), w()], $ = H();
    return Ge(V, E), $(), X(Ie, T);
  }, L, () => ({
    gn: h,
    bn: d
  }), {
    wn: v,
    yn: M
  }, f];
}, us = /* @__PURE__ */ new WeakMap(), Oa = (t, e) => {
  us.set(t, e);
}, La = (t) => {
  us.delete(t);
}, Vo = (t) => us.get(t), Ne = (t, e, n) => {
  const { nt: o } = Ye(), s = ln(t), i = s ? t : t.target, c = Vo(i);
  if (e && !c) {
    let u = !1;
    const l = [], d = {}, v = (C) => {
      const F = ro(C), D = Rt(Gr);
      return D ? D(F, !0) : F;
    }, f = oe({}, o(), v(e)), [m, p, h] = Wn(), [w, x, M] = Wn(n), I = (C, F) => {
      M(C, F), h(C, F);
    }, [L, y, V, H, E] = Da(t, f, () => u, ({ vn: C, Dt: F }, { Zt: D, tn: R }) => {
      const { dt: k, Ct: U, xt: P, Ht: ee, Et: se, _t: ne } = D, { nn: me, sn: Y, en: S, cn: B } = R;
      I("updated", [$, {
        updateHints: {
          sizeChanged: !!k,
          directionChanged: !!U,
          heightIntrinsicChanged: !!P,
          overflowEdgeChanged: !!me,
          overflowAmountChanged: !!Y,
          overflowStyleChanged: !!S,
          scrollCoordinatesChanged: !!B,
          contentMutation: !!ee,
          hostMutation: !!se,
          appear: !!ne
        },
        changedOptions: C || {},
        force: !!F
      }]);
    }, (C) => I("scroll", [$, C])), T = (C) => {
      La(i), Ie(l), u = !0, I("destroyed", [$, C]), p(), x();
    }, $ = {
      options(C, F) {
        if (C) {
          const D = F ? o() : {}, R = Ao(f, oe(D, v(C)));
          ss(R) || (oe(f, R), y({
            vn: R
          }));
        }
        return oe({}, f);
      },
      on: w,
      off: (C, F) => {
        C && F && x(C, F);
      },
      state() {
        const { gn: C, bn: F } = V(), { F: D } = C, { Lt: R, Vt: k, k: U, rn: P, ln: ee, _n: se, Tt: ne } = F;
        return oe({}, {
          overflowEdge: R,
          overflowAmount: k,
          overflowStyle: U,
          hasOverflow: P,
          scrollCoordinates: {
            start: ne.D,
            end: ne.M
          },
          padding: ee,
          paddingAbsolute: se,
          directionRTL: D,
          destroyed: u
        });
      },
      elements() {
        const { vt: C, ht: F, ln: D, U: R, bt: k, gt: U, Qt: P } = H.wn, { Xt: ee, Gt: se } = H.yn, ne = (Y) => {
          const { kt: S, Pt: B, Ut: q } = Y;
          return {
            scrollbar: q,
            track: B,
            handle: S
          };
        }, me = (Y) => {
          const { Yt: S, Wt: B } = Y, q = ne(S[0]);
          return oe({}, q, {
            clone: () => {
              const A = ne(B());
              return y({
                hn: !0
              }), A;
            }
          });
        };
        return oe({}, {
          target: C,
          host: F,
          padding: D || R,
          viewport: R,
          content: k || R,
          scrollOffsetElement: U,
          scrollEventElement: P,
          scrollbarHorizontal: me(ee),
          scrollbarVertical: me(se)
        });
      },
      update: (C) => y({
        Dt: C,
        At: !0
      }),
      destroy: X(T, !1),
      plugin: (C) => d[Ue(C)[0]]
    };
    return he(l, [E]), Oa(i, $), So(ko, Ne, [$, m, d]), ka(H.wn.wt, !s && t.cancel) ? (T(!0), $) : (he(l, L()), I("initialized", [$]), $.update(), $);
  }
  return c;
};
Ne.plugin = (t) => {
  const e = Ke(t), n = e ? t : [t], o = n.map((s) => So(s, Ne)[0]);
  return jr(n), e ? o : o[0];
};
Ne.valid = (t) => {
  const e = t && t.elements, n = Re(e) && e();
  return an(n) && !!Vo(n.target);
};
Ne.env = () => {
  const { N: t, T: e, P: n, G: o, st: s, et: i, Z: c, tt: u, nt: l, ot: d } = Ye();
  return oe({}, {
    scrollbarsSize: t,
    scrollbarsOverlaid: e,
    scrollbarsHiding: n,
    scrollTimeline: o,
    staticDefaultInitialization: s,
    staticDefaultOptions: i,
    getDefaultInitialization: c,
    setDefaultInitialization: u,
    getDefaultOptions: l,
    setDefaultOptions: d
  });
};
Ne.nonce = pa;
Ne.trustedTypePolicy = Br;
function Va() {
  let t;
  const e = O(null), n = Math.floor(Math.random() * 2 ** 32), o = O(!1), s = O([]), i = () => s.value, c = () => t.getSelection(), u = () => s.value.length, l = () => t.clearSelection(!0), d = O(), v = O(null), f = O(null), m = O(null), p = O(null);
  function h() {
    t = new pr({
      area: e.value,
      keyboardDrag: !1,
      selectedClass: "vf-explorer-selected",
      selectorClass: "vf-explorer-selector"
    }), t.subscribe("DS:start:pre", ({ items: V, event: H, isDragging: E }) => {
      if (E)
        t.Interaction._reset(H);
      else {
        o.value = !1;
        const T = e.value.offsetWidth - H.offsetX, $ = e.value.offsetHeight - H.offsetY;
        T < 15 && $ < 15 && t.Interaction._reset(H), H.target.classList.contains("os-scrollbar-handle") && t.Interaction._reset(H);
      }
    }), document.addEventListener("dragleave", (V) => {
      !V.buttons && o.value && (o.value = !1);
    });
  }
  const w = () => vt(() => {
    t.addSelection(
      t.getSelectables()
    ), x();
  }), x = () => {
    s.value = t.getSelection().map((V) => JSON.parse(V.dataset.item)), d.value(s.value);
  }, M = () => vt(() => {
    const V = i().map((H) => H.path);
    l(), t.setSettings({
      selectables: document.getElementsByClassName("vf-item-" + n)
    }), t.addSelection(
      t.getSelectables().filter((H) => V.includes(JSON.parse(H.dataset.item).path))
    ), x(), L();
  }), I = (V) => {
    d.value = V, t.subscribe("DS:end", ({ items: H, event: E, isDragging: T }) => {
      s.value = H.map(($) => JSON.parse($.dataset.item)), V(H.map(($) => JSON.parse($.dataset.item)));
    });
  }, L = () => {
    v.value && (e.value.getBoundingClientRect().height < e.value.scrollHeight ? (f.value.style.height = e.value.scrollHeight + "px", f.value.style.display = "block") : (f.value.style.height = "100%", f.value.style.display = "none"));
  }, y = (V) => {
    if (!v.value)
      return;
    const { scrollOffsetElement: H } = v.value.elements();
    H.scrollTo(
      {
        top: e.value.scrollTop,
        left: 0
      }
    );
  };
  return Ce(() => {
    Ne(m.value, {
      scrollbars: {
        theme: "vf-theme-dark dark:vf-theme-light"
      },
      plugins: {
        OverlayScrollbars: Ne
        // ScrollbarsHidingPlugin,
        // SizeObserverPlugin,
        // ClickScrollPlugin
      }
    }, {
      initialized: (V) => {
        v.value = V;
      },
      scroll: (V, H) => {
        const { scrollOffsetElement: E } = V.elements();
        e.value.scrollTo({
          top: E.scrollTop,
          left: 0
        });
      }
    }), h(), L(), p.value = new ResizeObserver(L), p.value.observe(e.value), e.value.addEventListener("scroll", y), t.subscribe("DS:scroll", ({ isDragging: V }) => V || y());
  }), Qn(() => {
    t && t.stop(), p.value && p.value.disconnect();
  }), Bs(() => {
    t && t.Area.reset();
  }), {
    area: e,
    explorerId: n,
    isDraggingRef: o,
    scrollBar: f,
    scrollBarContainer: m,
    getSelected: i,
    getSelection: c,
    selectAll: w,
    clearSelection: l,
    refreshSelection: M,
    getCount: u,
    onSelect: I
  };
}
function Fa(t, e) {
  const n = O(t), o = O(e), s = O([]), i = O([]), c = O([]), u = O(!1), l = O(5);
  let d = !1, v = !1;
  const f = St({
    adapter: n,
    storages: [],
    dirname: o,
    files: []
  });
  function m() {
    let I = [], L = [], y = o.value ?? n.value + "://";
    y.length === 0 && (s.value = []), y.replace(n.value + "://", "").split("/").filter(Boolean).forEach(function(E) {
      I.push(E), I.join("/") !== "" && L.push({
        basename: E,
        name: E,
        path: n.value + "://" + I.join("/") + "/",
        type: "dir"
      });
    }), i.value = L;
    const [V, H] = h(
      L,
      l.value
    );
    c.value = H, s.value = V;
  }
  function p(I) {
    l.value = I, m();
  }
  function h(I, L) {
    return I.length > L ? [I.slice(-L), I.slice(0, -L)] : [I, []];
  }
  function w(I = null) {
    u.value = I ?? !u.value;
  }
  function x() {
    return s.value && s.value.length && !0;
  }
  const M = ze(() => {
    var I;
    return ((I = s.value[s.value.length - 2]) == null ? void 0 : I.path) ?? n.value + "://";
  });
  return Ce(() => {
  }), Oe(o, m), Ce(m), {
    adapter: n,
    path: o,
    loading: d,
    searchMode: v,
    data: f,
    breadcrumbs: s,
    breadcrumbItems: i,
    limitBreadcrumbItems: p,
    hiddenBreadcrumbs: c,
    showHiddenBreadcrumbs: u,
    toggleHiddenBreadcrumbs: w,
    isGoUpAvailable: x,
    parentFolderPath: M
  };
}
const Ia = (t, e) => {
  const n = xr(t.id), o = mr(), s = n.getStore("metricUnits", !1), i = Ar(n, t.theme), c = e.i18n, u = t.locale ?? e.locale, l = (p) => Array.isArray(p) ? p : Cr, d = n.getStore("persist-path", t.persist), v = d ? n.getStore("path", t.path) : t.path, f = d ? n.getStore("adapter") : null, m = Va();
  return St({
    /** 
    * Core properties
    * */
    // app version
    version: Er,
    // root element
    root: null,
    // app id
    debug: t.debug,
    // Event Bus
    emitter: o,
    // storage
    storage: n,
    // localization object
    i18n: $r(n, u, o, c),
    // modal state
    modal: Mr(),
    // dragSelect object, it is responsible for selecting items
    dragSelect: ze(() => m),
    // http object
    requester: kr(t.request),
    // active features
    features: l(t.features),
    // view state
    view: n.getStore("viewport", "grid"),
    // fullscreen state
    fullScreen: n.getStore("full-screen", t.fullScreen),
    // show tree view
    showTreeView: n.getStore("show-tree-view", t.showTreeView),
    // pinnedFolders
    pinnedFolders: n.getStore("pinned-folders", t.pinnedFolders),
    // treeViewData
    treeViewData: [],
    // selectButton state
    selectButton: t.selectButton,
    // max file size
    maxFileSize: t.maxFileSize,
    /**
    * Settings
    * */
    // theme state
    theme: i,
    // unit state - for example: GB or GiB
    metricUnits: s,
    // human readable file sizes
    filesize: s ? zs : qs,
    // show large icons in list view
    compactListView: n.getStore("compact-list-view", !0),
    // persist state
    persist: d,
    // show thumbnails
    showThumbnails: n.getStore("show-thumbnails", t.showThumbnails),
    // type of progress indicator
    loadingIndicator: t.loadingIndicator,
    // possible items of the context menu
    contextMenuItems: t.contextMenuItems,
    // file system
    fs: Fa(f, v)
  });
}, Ha = { class: "vuefinder__modal-layout__container" }, Ra = { class: "vuefinder__modal-layout__content" }, Ba = { class: "vuefinder__modal-layout__footer" }, Xe = {
  __name: "ModalLayout",
  setup(t) {
    const e = O(null), n = re("ServiceContainer");
    return Ce(() => {
      const o = document.querySelector(".v-f-modal input");
      o && o.focus(), vt(() => {
        if (document.querySelector(".v-f-modal input") && window.innerWidth < 768) {
          const s = e.value.getBoundingClientRect().bottom + 16;
          window.scrollTo({
            top: s,
            left: 0,
            behavior: "smooth"
          });
        }
      });
    }), (o, s) => (_(), g("div", {
      class: "vuefinder__modal-layout",
      "aria-labelledby": "modal-title",
      role: "dialog",
      "aria-modal": "true",
      onKeyup: s[1] || (s[1] = $t((i) => r(n).modal.close(), ["esc"])),
      tabindex: "0"
    }, [
      s[2] || (s[2] = a("div", { class: "vuefinder__modal-layout__overlay" }, null, -1)),
      a("div", Ha, [
        a("div", {
          class: "vuefinder__modal-layout__wrapper",
          onMousedown: s[0] || (s[0] = tt((i) => r(n).modal.close(), ["self"]))
        }, [
          a("div", {
            ref_key: "modalBody",
            ref: e,
            class: "vuefinder__modal-layout__body"
          }, [
            a("div", Ra, [
              Lt(o.$slots, "default")
            ]),
            a("div", Ba, [
              Lt(o.$slots, "buttons")
            ])
          ], 512)
        ], 32)
      ])
    ], 32));
  }
}, Ua = (t, e) => {
  const n = t.__vccOpts || t;
  for (const [o, s] of e)
    n[o] = s;
  return n;
}, Na = {
  props: {
    on: { type: String, required: !0 }
  },
  setup(t, { emit: e, slots: n }) {
    const o = re("ServiceContainer"), s = O(!1), { t: i } = o.i18n;
    let c = null;
    const u = () => {
      clearTimeout(c), s.value = !0, c = setTimeout(() => {
        s.value = !1;
      }, 2e3);
    };
    return Ce(() => {
      o.emitter.on(t.on, u);
    }), Qn(() => {
      clearTimeout(c);
    }), {
      shown: s,
      t: i
    };
  }
}, Pa = { key: 1 };
function qa(t, e, n, o, s, i) {
  return _(), g("div", {
    class: le(["vuefinder__action-message", { "vuefinder__action-message--hidden": !o.shown }])
  }, [
    t.$slots.default ? Lt(t.$slots, "default", { key: 0 }) : (_(), g("span", Pa, b(o.t("Saved.")), 1))
  ], 2);
}
const mt = /* @__PURE__ */ Ua(Na, [["render", qa]]), za = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  "stroke-width": "1.5",
  class: "h-6 w-6 stroke-blue-600 dark:stroke-blue-100",
  viewBox: "0 0 24 24"
};
function ja(t, e) {
  return _(), g("svg", za, e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87q.11.06.22.127c.325.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 0 1 1.37.49l1.296 2.247a1.125 1.125 0 0 1-.26 1.431l-1.003.827c-.293.241-.438.613-.43.992a8 8 0 0 1 0 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.955.26 1.43l-1.298 2.247a1.125 1.125 0 0 1-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a7 7 0 0 1-.22.128c-.331.183-.581.495-.644.869l-.213 1.281c-.09.543-.56.94-1.11.94h-2.594c-.55 0-1.019-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a7 7 0 0 1-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 0 1-1.369-.49l-1.297-2.247a1.125 1.125 0 0 1 .26-1.431l1.004-.827c.292-.24.437-.613.43-.991a7 7 0 0 1 0-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 0 1-.26-1.43l1.297-2.247a1.125 1.125 0 0 1 1.37-.491l1.216.456c.356.133.751.072 1.076-.124q.108-.066.22-.128c.332-.183.582-.495.644-.869z"
    }, null, -1),
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0"
    }, null, -1)
  ]));
}
const Ga = { render: ja }, Wa = { class: "vuefinder__modal-header" }, Ka = { class: "vuefinder__modal-header__icon-container" }, Ya = {
  class: "vuefinder__modal-header__title",
  id: "modal-title"
}, st = {
  __name: "ModalHeader",
  props: {
    title: {
      type: String,
      required: !0
    },
    icon: {
      type: Object,
      required: !0
    }
  },
  setup(t) {
    return (e, n) => (_(), g("div", Wa, [
      a("div", Ka, [
        (_(), K(Us(t.icon), { class: "vuefinder__modal-header__icon" }))
      ]),
      a("h3", Ya, b(t.title), 1)
    ]));
  }
}, Xa = { class: "vuefinder__about-modal__content" }, Za = { class: "vuefinder__about-modal__main" }, Qa = {
  class: "vuefinder__about-modal__tabs",
  "aria-label": "Tabs"
}, Ja = ["onClick", "aria-current"], el = {
  key: 0,
  class: "vuefinder__about-modal__tab-content"
}, tl = { class: "vuefinder__about-modal__description" }, nl = {
  href: "https://vuefinder.ozdemir.be",
  class: "vuefinder__about-modal__link",
  target: "_blank"
}, sl = {
  href: "https://github.com/n1crack/vuefinder",
  class: "vuefinder__about-modal__link",
  target: "_blank"
}, ol = {
  key: 1,
  class: "vuefinder__about-modal__tab-content"
}, rl = { class: "vuefinder__about-modal__description" }, al = { class: "vuefinder__about-modal__settings" }, ll = { class: "vuefinder__about-modal__setting flex" }, il = { class: "vuefinder__about-modal__setting-input" }, cl = { class: "vuefinder__about-modal__setting-label" }, dl = {
  for: "metric_unit",
  class: "vuefinder__about-modal__label"
}, ul = { class: "vuefinder__about-modal__setting flex" }, fl = { class: "vuefinder__about-modal__setting-input" }, vl = { class: "vuefinder__about-modal__setting-label" }, _l = {
  for: "large_icons",
  class: "vuefinder__about-modal__label"
}, ml = { class: "vuefinder__about-modal__setting flex" }, pl = { class: "vuefinder__about-modal__setting-input" }, hl = { class: "vuefinder__about-modal__setting-label" }, gl = {
  for: "persist_path",
  class: "vuefinder__about-modal__label"
}, bl = { class: "vuefinder__about-modal__setting flex" }, wl = { class: "vuefinder__about-modal__setting-input" }, yl = { class: "vuefinder__about-modal__setting-label" }, kl = {
  for: "show_thumbnails",
  class: "vuefinder__about-modal__label"
}, xl = { class: "vuefinder__about-modal__setting" }, Sl = { class: "vuefinder__about-modal__setting-input" }, $l = {
  for: "theme",
  class: "vuefinder__about-modal__label"
}, Cl = { class: "vuefinder__about-modal__setting-label" }, El = ["label"], Tl = ["value"], Al = {
  key: 0,
  class: "vuefinder__about-modal__setting"
}, Ml = { class: "vuefinder__about-modal__setting-input" }, Dl = {
  for: "language",
  class: "vuefinder__about-modal__label"
}, Ol = { class: "vuefinder__about-modal__setting-label" }, Ll = ["label"], Vl = ["value"], Fl = {
  key: 2,
  class: "vuefinder__about-modal__tab-content"
}, Il = { class: "vuefinder__about-modal__shortcuts" }, Hl = { class: "vuefinder__about-modal__shortcut" }, Rl = { class: "vuefinder__about-modal__shortcut" }, Bl = { class: "vuefinder__about-modal__shortcut" }, Ul = { class: "vuefinder__about-modal__shortcut" }, Nl = { class: "vuefinder__about-modal__shortcut" }, Pl = { class: "vuefinder__about-modal__shortcut" }, ql = { class: "vuefinder__about-modal__shortcut" }, zl = { class: "vuefinder__about-modal__shortcut" }, jl = { class: "vuefinder__about-modal__shortcut" }, Gl = {
  key: 3,
  class: "vuefinder__about-modal__tab-content"
}, Wl = { class: "vuefinder__about-modal__description" }, Fo = {
  __name: "ModalAbout",
  setup(t) {
    const e = re("ServiceContainer"), { setStore: n, clearStore: o } = e.storage, { t: s } = e.i18n, i = {
      ABOUT: "about",
      SETTINGS: "settings",
      SHORTCUTS: "shortcuts",
      RESET: "reset"
    }, c = ze(() => [
      { name: s("About"), key: i.ABOUT },
      { name: s("Settings"), key: i.SETTINGS },
      { name: s("Shortcuts"), key: i.SHORTCUTS },
      { name: s("Reset"), key: i.RESET }
    ]), u = O("about"), l = async () => {
      o(), location.reload();
    }, d = (I) => {
      e.theme.set(I), e.emitter.emit("vf-theme-saved");
    }, v = () => {
      e.metricUnits = !e.metricUnits, e.filesize = e.metricUnits ? zs : qs, n("metricUnits", e.metricUnits), e.emitter.emit("vf-metric-units-saved");
    }, f = () => {
      e.compactListView = !e.compactListView, n("compactListView", e.compactListView), e.emitter.emit("vf-compact-view-saved");
    }, m = () => {
      e.showThumbnails = !e.showThumbnails, n("show-thumbnails", e.showThumbnails), e.emitter.emit("vf-show-thumbnails-saved");
    }, p = () => {
      e.persist = !e.persist, n("persist-path", e.persist), e.emitter.emit("vf-persist-path-saved");
    }, { i18n: h } = re("VueFinderOptions"), x = Object.fromEntries(
      Object.entries({
        ar: "Arabic (العربيّة)",
        en: "English",
        fr: "French (Français)",
        de: "German (Deutsch)",
        fa: "Persian (فارسی)",
        he: "Hebrew (עִברִית)",
        hi: "Hindi (हिंदी)",
        pl: "Polish (Polski)",
        ru: "Russian (Pусский)",
        sv: "Swedish (Svenska)",
        tr: "Turkish (Türkçe)",
        nl: "Dutch (Nederlands)",
        zhCN: "Simplified Chinese (简体中文)",
        zhTW: "Traditional Chinese (繁體中文)"
      }).filter(([I]) => Object.keys(h).includes(I))
    ), M = ze(() => ({
      system: s("System"),
      light: s("Light"),
      dark: s("Dark")
    }));
    return (I, L) => (_(), K(Xe, null, {
      buttons: J(() => [
        a("button", {
          type: "button",
          onClick: L[7] || (L[7] = (y) => r(e).modal.close()),
          class: "vf-btn vf-btn-secondary"
        }, b(r(s)("Close")), 1)
      ]),
      default: J(() => [
        a("div", Xa, [
          j(st, {
            icon: r(Ga),
            title: "Vuefinder " + r(e).version
          }, null, 8, ["icon", "title"]),
          a("div", Za, [
            a("div", null, [
              a("div", null, [
                a("nav", Qa, [
                  (_(!0), g(ye, null, $e(c.value, (y) => (_(), g("button", {
                    key: y.name,
                    onClick: (V) => u.value = y.key,
                    class: le([y.key === u.value ? "vuefinder__about-modal__tab--active" : "vuefinder__about-modal__tab--inactive", "vuefinder__about-modal__tab"]),
                    "aria-current": y.current ? "page" : void 0
                  }, b(y.name), 11, Ja))), 128))
                ])
              ])
            ]),
            u.value === i.ABOUT ? (_(), g("div", el, [
              a("div", tl, b(r(s)("Vuefinder is a simple, lightweight, and fast file manager library for Vue.js applications")), 1),
              a("a", nl, b(r(s)("Project home")), 1),
              a("a", sl, b(r(s)("Follow on GitHub")), 1)
            ])) : N("", !0),
            u.value === i.SETTINGS ? (_(), g("div", ol, [
              a("div", rl, b(r(s)("Customize your experience with the following settings")), 1),
              a("div", al, [
                a("fieldset", null, [
                  a("div", ll, [
                    a("div", il, [
                      _e(a("input", {
                        id: "metric_unit",
                        name: "metric_unit",
                        type: "checkbox",
                        "onUpdate:modelValue": L[0] || (L[0] = (y) => r(e).metricUnits = y),
                        onClick: v,
                        class: "vuefinder__about-modal__checkbox"
                      }, null, 512), [
                        [Xt, r(e).metricUnits]
                      ])
                    ]),
                    a("div", cl, [
                      a("label", dl, [
                        Z(b(r(s)("Use Metric Units")) + " ", 1),
                        j(mt, {
                          class: "ms-3",
                          on: "vf-metric-units-saved"
                        }, {
                          default: J(() => [
                            Z(b(r(s)("Saved.")), 1)
                          ]),
                          _: 1
                        })
                      ])
                    ])
                  ]),
                  a("div", ul, [
                    a("div", fl, [
                      _e(a("input", {
                        id: "large_icons",
                        name: "large_icons",
                        type: "checkbox",
                        "onUpdate:modelValue": L[1] || (L[1] = (y) => r(e).compactListView = y),
                        onClick: f,
                        class: "vuefinder__about-modal__checkbox"
                      }, null, 512), [
                        [Xt, r(e).compactListView]
                      ])
                    ]),
                    a("div", vl, [
                      a("label", _l, [
                        Z(b(r(s)("Compact list view")) + " ", 1),
                        j(mt, {
                          class: "ms-3",
                          on: "vf-compact-view-saved"
                        }, {
                          default: J(() => [
                            Z(b(r(s)("Saved.")), 1)
                          ]),
                          _: 1
                        })
                      ])
                    ])
                  ]),
                  a("div", ml, [
                    a("div", pl, [
                      _e(a("input", {
                        id: "persist_path",
                        name: "persist_path",
                        type: "checkbox",
                        "onUpdate:modelValue": L[2] || (L[2] = (y) => r(e).persist = y),
                        onClick: p,
                        class: "vuefinder__about-modal__checkbox"
                      }, null, 512), [
                        [Xt, r(e).persist]
                      ])
                    ]),
                    a("div", hl, [
                      a("label", gl, [
                        Z(b(r(s)("Persist path on reload")) + " ", 1),
                        j(mt, {
                          class: "ms-3",
                          on: "vf-persist-path-saved"
                        }, {
                          default: J(() => [
                            Z(b(r(s)("Saved.")), 1)
                          ]),
                          _: 1
                        })
                      ])
                    ])
                  ]),
                  a("div", bl, [
                    a("div", wl, [
                      _e(a("input", {
                        id: "show_thumbnails",
                        name: "show_thumbnails",
                        type: "checkbox",
                        "onUpdate:modelValue": L[3] || (L[3] = (y) => r(e).showThumbnails = y),
                        onClick: m,
                        class: "vuefinder__about-modal__checkbox"
                      }, null, 512), [
                        [Xt, r(e).showThumbnails]
                      ])
                    ]),
                    a("div", yl, [
                      a("label", kl, [
                        Z(b(r(s)("Show thumbnails")) + " ", 1),
                        j(mt, {
                          class: "ms-3",
                          on: "vf-show-thumbnails-saved"
                        }, {
                          default: J(() => [
                            Z(b(r(s)("Saved.")), 1)
                          ]),
                          _: 1
                        })
                      ])
                    ])
                  ]),
                  a("div", xl, [
                    a("div", Sl, [
                      a("label", $l, b(r(s)("Theme")), 1)
                    ]),
                    a("div", Cl, [
                      _e(a("select", {
                        id: "theme",
                        "onUpdate:modelValue": L[4] || (L[4] = (y) => r(e).theme.value = y),
                        onChange: L[5] || (L[5] = (y) => d(y.target.value)),
                        class: "vuefinder__about-modal__select"
                      }, [
                        a("optgroup", {
                          label: r(s)("Theme")
                        }, [
                          (_(!0), g(ye, null, $e(M.value, (y, V) => (_(), g("option", { value: V }, b(y), 9, Tl))), 256))
                        ], 8, El)
                      ], 544), [
                        [gs, r(e).theme.value]
                      ]),
                      j(mt, {
                        class: "ms-3",
                        on: "vf-theme-saved"
                      }, {
                        default: J(() => [
                          Z(b(r(s)("Saved.")), 1)
                        ]),
                        _: 1
                      })
                    ])
                  ]),
                  r(e).features.includes(r(ue).LANGUAGE) && Object.keys(r(x)).length > 1 ? (_(), g("div", Al, [
                    a("div", Ml, [
                      a("label", Dl, b(r(s)("Language")), 1)
                    ]),
                    a("div", Ol, [
                      _e(a("select", {
                        id: "language",
                        "onUpdate:modelValue": L[6] || (L[6] = (y) => r(e).i18n.locale = y),
                        class: "vuefinder__about-modal__select"
                      }, [
                        a("optgroup", {
                          label: r(s)("Language")
                        }, [
                          (_(!0), g(ye, null, $e(r(x), (y, V) => (_(), g("option", { value: V }, b(y), 9, Vl))), 256))
                        ], 8, Ll)
                      ], 512), [
                        [gs, r(e).i18n.locale]
                      ]),
                      j(mt, {
                        class: "ms-3",
                        on: "vf-language-saved"
                      }, {
                        default: J(() => [
                          Z(b(r(s)("Saved.")), 1)
                        ]),
                        _: 1
                      })
                    ])
                  ])) : N("", !0)
                ])
              ])
            ])) : N("", !0),
            u.value === i.SHORTCUTS ? (_(), g("div", Fl, [
              a("div", Il, [
                a("div", Hl, [
                  a("div", null, b(r(s)("Rename")), 1),
                  L[8] || (L[8] = a("kbd", null, "F2", -1))
                ]),
                a("div", Rl, [
                  a("div", null, b(r(s)("Refresh")), 1),
                  L[9] || (L[9] = a("kbd", null, "F5", -1))
                ]),
                a("div", Bl, [
                  Z(b(r(s)("Delete")) + " ", 1),
                  L[10] || (L[10] = a("kbd", null, "Del", -1))
                ]),
                a("div", Ul, [
                  Z(b(r(s)("Escape")) + " ", 1),
                  L[11] || (L[11] = a("div", null, [
                    a("kbd", null, "Esc")
                  ], -1))
                ]),
                a("div", Nl, [
                  Z(b(r(s)("Select All")) + " ", 1),
                  L[12] || (L[12] = a("div", null, [
                    a("kbd", null, "Ctrl"),
                    Z(" + "),
                    a("kbd", null, "A")
                  ], -1))
                ]),
                a("div", Pl, [
                  Z(b(r(s)("Search")) + " ", 1),
                  L[13] || (L[13] = a("div", null, [
                    a("kbd", null, "Ctrl"),
                    Z(" + "),
                    a("kbd", null, "F")
                  ], -1))
                ]),
                a("div", ql, [
                  Z(b(r(s)("Toggle Sidebar")) + " ", 1),
                  L[14] || (L[14] = a("div", null, [
                    a("kbd", null, "Ctrl"),
                    Z(" + "),
                    a("kbd", null, "E")
                  ], -1))
                ]),
                a("div", zl, [
                  Z(b(r(s)("Open Settings")) + " ", 1),
                  L[15] || (L[15] = a("div", null, [
                    a("kbd", null, "Ctrl"),
                    Z(" + "),
                    a("kbd", null, ",")
                  ], -1))
                ]),
                a("div", jl, [
                  Z(b(r(s)("Toggle Full Screen")) + " ", 1),
                  L[16] || (L[16] = a("div", null, [
                    a("kbd", null, "Ctrl"),
                    Z(" + "),
                    a("kbd", null, "Enter")
                  ], -1))
                ])
              ])
            ])) : N("", !0),
            u.value === i.RESET ? (_(), g("div", Gl, [
              a("div", Wl, b(r(s)("Reset all settings to default")), 1),
              a("button", {
                onClick: l,
                type: "button",
                class: "vf-btn vf-btn-secondary"
              }, b(r(s)("Reset Settings")), 1)
            ])) : N("", !0)
          ])
        ])
      ]),
      _: 1
    }));
  }
}, Kl = ["title"], Ze = {
  __name: "Message",
  props: {
    error: {
      type: Boolean,
      default: !1
    }
  },
  emits: ["hidden"],
  setup(t, { emit: e }) {
    var d;
    const n = e, o = re("ServiceContainer"), { t: s } = o.i18n, i = O(!1), c = O(null), u = O((d = c.value) == null ? void 0 : d.strMessage);
    Oe(u, () => i.value = !1);
    const l = () => {
      n("hidden"), i.value = !0;
    };
    return (v, f) => (_(), g("div", null, [
      i.value ? N("", !0) : (_(), g("div", {
        key: 0,
        ref_key: "strMessage",
        ref: c,
        class: le(["vuefinder__message", t.error ? "vuefinder__message--error" : "vuefinder__message--success"])
      }, [
        Lt(v.$slots, "default"),
        a("div", {
          class: "vuefinder__message__close",
          onClick: l,
          title: r(s)("Close")
        }, f[0] || (f[0] = [
          a("svg", {
            xmlns: "http://www.w3.org/2000/svg",
            fill: "none",
            viewBox: "0 0 24 24",
            "stroke-width": "1.5",
            stroke: "currentColor",
            class: "vuefinder__message__icon"
          }, [
            a("path", {
              "stroke-linecap": "round",
              "stroke-linejoin": "round",
              d: "M6 18L18 6M6 6l12 12"
            })
          ], -1)
        ]), 8, Kl)
      ], 2))
    ]));
  }
}, Yl = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  class: "h-6 w-6 md:h-8 md:w-8 m-auto",
  viewBox: "0 0 24 24"
};
function Xl(t, e) {
  return _(), g("svg", Yl, e[0] || (e[0] = [
    a("path", { d: "m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21q.512.078 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48 48 0 0 0-3.478-.397m-12 .562q.51-.089 1.022-.165m0 0a48 48 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a52 52 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a49 49 0 0 0-7.5 0" }, null, -1)
  ]));
}
const Io = { render: Xl }, Zl = { class: "vuefinder__delete-modal__content" }, Ql = { class: "vuefinder__delete-modal__form" }, Jl = { class: "vuefinder__delete-modal__description" }, ei = { class: "vuefinder__delete-modal__files vf-scrollbar" }, ti = { class: "vuefinder__delete-modal__file" }, ni = {
  key: 0,
  class: "vuefinder__delete-modal__icon vuefinder__delete-modal__icon--dir",
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  viewBox: "0 0 24 24",
  stroke: "currentColor",
  "stroke-width": "1"
}, si = {
  key: 1,
  class: "vuefinder__delete-modal__icon",
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  viewBox: "0 0 24 24",
  stroke: "currentColor",
  "stroke-width": "1"
}, oi = { class: "vuefinder__delete-modal__file-name" }, ri = { class: "vuefinder__delete-modal__warning" }, fs = {
  __name: "ModalDelete",
  setup(t) {
    const e = re("ServiceContainer"), { t: n } = e.i18n, o = O(e.modal.data.items), s = O(""), i = () => {
      o.value.length && e.emitter.emit("vf-fetch", {
        params: {
          q: "delete",
          m: "post",
          adapter: e.fs.adapter,
          path: e.fs.data.dirname
        },
        body: {
          items: o.value.map(({ path: c, type: u }) => ({ path: c, type: u }))
        },
        onSuccess: () => {
          e.emitter.emit("vf-toast-push", { label: n("Files deleted.") }), e.emitter.emit("vf-fetch", {
            params: {
              q: "index",
              adapter: e.fs.adapter,
              path: e.fs.data.dirname
            }
          });
        },
        onError: (c) => {
          s.value = n(c.message), e.emitter.emit("vf-fetch", {
            params: {
              q: "index",
              adapter: e.fs.adapter,
              path: e.fs.data.dirname
            }
          });
        }
      });
    };
    return (c, u) => (_(), K(Xe, null, {
      buttons: J(() => [
        a("button", {
          type: "button",
          onClick: i,
          class: "vf-btn vf-btn-danger"
        }, b(r(n)("Yes, Delete!")), 1),
        a("button", {
          type: "button",
          onClick: u[1] || (u[1] = (l) => r(e).modal.close()),
          class: "vf-btn vf-btn-secondary"
        }, b(r(n)("Cancel")), 1),
        a("div", ri, b(r(n)("This action cannot be undone.")), 1)
      ]),
      default: J(() => [
        a("div", null, [
          j(st, {
            icon: r(Io),
            title: r(n)("Delete files")
          }, null, 8, ["icon", "title"]),
          a("div", Zl, [
            a("div", Ql, [
              a("p", Jl, b(r(n)("Are you sure you want to delete these files?")), 1),
              a("div", ei, [
                (_(!0), g(ye, null, $e(o.value, (l) => (_(), g("p", ti, [
                  l.type === "dir" ? (_(), g("svg", ni, u[2] || (u[2] = [
                    a("path", {
                      "stroke-linecap": "round",
                      "stroke-linejoin": "round",
                      d: "M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"
                    }, null, -1)
                  ]))) : (_(), g("svg", si, u[3] || (u[3] = [
                    a("path", {
                      "stroke-linecap": "round",
                      "stroke-linejoin": "round",
                      d: "M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"
                    }, null, -1)
                  ]))),
                  a("span", oi, b(l.basename), 1)
                ]))), 256))
              ]),
              s.value.length ? (_(), K(Ze, {
                key: 0,
                onHidden: u[0] || (u[0] = (l) => s.value = ""),
                error: ""
              }, {
                default: J(() => [
                  Z(b(s.value), 1)
                ]),
                _: 1
              })) : N("", !0)
            ])
          ])
        ])
      ]),
      _: 1
    }));
  }
}, ai = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  class: "h-6 w-6 md:h-8 md:w-8 m-auto",
  viewBox: "0 0 24 24"
};
function li(t, e) {
  return _(), g("svg", ai, e[0] || (e[0] = [
    a("path", { d: "m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" }, null, -1)
  ]));
}
const Ho = { render: li }, ii = { class: "vuefinder__rename-modal__content" }, ci = { class: "vuefinder__rename-modal__item" }, di = { class: "vuefinder__rename-modal__item-info" }, ui = {
  key: 0,
  class: "vuefinder__rename-modal__icon vuefinder__rename-modal__icon--dir",
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  viewBox: "0 0 24 24",
  stroke: "currentColor",
  "stroke-width": "1"
}, fi = {
  key: 1,
  class: "vuefinder__rename-modal__icon",
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  viewBox: "0 0 24 24",
  stroke: "currentColor",
  "stroke-width": "1"
}, vi = { class: "vuefinder__rename-modal__item-name" }, vs = {
  __name: "ModalRename",
  setup(t) {
    const e = re("ServiceContainer"), { t: n } = e.i18n, o = O(e.modal.data.items[0]), s = O(e.modal.data.items[0].basename), i = O(""), c = () => {
      s.value != "" && e.emitter.emit("vf-fetch", {
        params: {
          q: "rename",
          m: "post",
          adapter: e.fs.adapter,
          path: e.fs.data.dirname
        },
        body: {
          item: o.value.path,
          name: s.value
        },
        onSuccess: () => {
          e.emitter.emit("vf-toast-push", {
            label: n("%s is renamed.", s.value)
          }), e.emitter.emit("vf-fetch", {
            params: {
              q: "index",
              adapter: e.fs.adapter,
              path: e.fs.data.dirname
            }
          });
        },
        onError: (u) => {
          i.value = n(u.message);
        }
      });
    };
    return (u, l) => (_(), K(Xe, null, {
      buttons: J(() => [
        a("button", {
          type: "button",
          onClick: c,
          class: "vf-btn vf-btn-primary"
        }, b(r(n)("Rename")), 1),
        a("button", {
          type: "button",
          onClick: l[2] || (l[2] = (d) => r(e).modal.close()),
          class: "vf-btn vf-btn-secondary"
        }, b(r(n)("Cancel")), 1)
      ]),
      default: J(() => [
        a("div", null, [
          j(st, {
            icon: r(Ho),
            title: r(n)("Rename")
          }, null, 8, ["icon", "title"]),
          a("div", ii, [
            a("div", ci, [
              a("p", di, [
                o.value.type === "dir" ? (_(), g("svg", ui, l[3] || (l[3] = [
                  a("path", {
                    "stroke-linecap": "round",
                    "stroke-linejoin": "round",
                    d: "M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"
                  }, null, -1)
                ]))) : (_(), g("svg", fi, l[4] || (l[4] = [
                  a("path", {
                    "stroke-linecap": "round",
                    "stroke-linejoin": "round",
                    d: "M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"
                  }, null, -1)
                ]))),
                a("span", vi, b(o.value.basename), 1)
              ]),
              _e(a("input", {
                "onUpdate:modelValue": l[0] || (l[0] = (d) => s.value = d),
                onKeyup: $t(c, ["enter"]),
                class: "vuefinder__rename-modal__input",
                placeholder: "Name",
                type: "text"
              }, null, 544), [
                [Ct, s.value]
              ]),
              i.value.length ? (_(), K(Ze, {
                key: 0,
                onHidden: l[1] || (l[1] = (d) => i.value = ""),
                error: ""
              }, {
                default: J(() => [
                  Z(b(i.value), 1)
                ]),
                _: 1
              })) : N("", !0)
            ])
          ])
        ])
      ]),
      _: 1
    }));
  }
}, Qe = {
  ESCAPE: "Escape",
  F2: "F2",
  F5: "F5",
  DELETE: "Delete",
  ENTER: "Enter",
  BACKSLASH: "Backslash",
  KEY_A: "KeyA",
  KEY_E: "KeyE",
  KEY_F: "KeyF"
};
function _i(t) {
  const e = (n) => {
    n.code === Qe.ESCAPE && (t.modal.close(), t.root.focus()), !t.modal.visible && (t.fs.searchMode || (n.code === Qe.F2 && t.features.includes(ue.RENAME) && (t.dragSelect.getCount() !== 1 || t.modal.open(vs, { items: t.dragSelect.getSelected() })), n.code === Qe.F5 && t.emitter.emit("vf-fetch", { params: { q: "index", adapter: t.fs.adapter, path: t.fs.data.dirname } }), n.code === Qe.DELETE && (!t.dragSelect.getCount() || t.modal.open(fs, { items: t.dragSelect.getSelected() })), n.metaKey && n.code === Qe.BACKSLASH && t.modal.open(Fo), n.metaKey && n.code === Qe.KEY_F && t.features.includes(ue.SEARCH) && (t.fs.searchMode = !0, n.preventDefault()), n.metaKey && n.code === Qe.KEY_E && (t.showTreeView = !t.showTreeView, t.storage.setStore("show-tree-view", t.showTreeView)), n.metaKey && n.code === Qe.ENTER && (t.fullScreen = !t.fullScreen, t.root.focus()), n.metaKey && n.code === Qe.KEY_A && (t.dragSelect.selectAll(), n.preventDefault())));
  };
  Ce(() => {
    t.root.addEventListener("keydown", e);
  });
}
const mi = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  class: "h-6 w-6 md:h-8 md:w-8 m-auto vf-toolbar-icon",
  viewBox: "0 0 24 24"
};
function pi(t, e) {
  return _(), g("svg", mi, e[0] || (e[0] = [
    a("path", { d: "M12 10.5v6m3-3H9m4.06-7.19-2.12-2.12a1.5 1.5 0 0 0-1.061-.44H4.5A2.25 2.25 0 0 0 2.25 6v12a2.25 2.25 0 0 0 2.25 2.25h15A2.25 2.25 0 0 0 21.75 18V9a2.25 2.25 0 0 0-2.25-2.25h-5.379a1.5 1.5 0 0 1-1.06-.44z" }, null, -1)
  ]));
}
const Ro = { render: pi }, hi = { class: "vuefinder__new-folder-modal__content" }, gi = { class: "vuefinder__new-folder-modal__form" }, bi = { class: "vuefinder__new-folder-modal__description" }, wi = ["placeholder"], Bo = {
  __name: "ModalNewFolder",
  setup(t) {
    const e = re("ServiceContainer"), { getStore: n } = e.storage, { t: o } = e.i18n, s = O(""), i = O(""), c = () => {
      s.value !== "" && e.emitter.emit("vf-fetch", {
        params: {
          q: "newfolder",
          m: "post",
          adapter: e.fs.adapter,
          path: e.fs.data.dirname
        },
        body: {
          name: s.value
        },
        onSuccess: () => {
          e.emitter.emit("vf-toast-push", {
            label: o("%s is created.", s.value)
          }), e.emitter.emit("vf-fetch", {
            params: {
              q: "index",
              adapter: e.fs.adapter,
              path: e.fs.data.dirname
            }
          });
        },
        onError: (u) => {
          i.value = o(u.message);
        }
      });
    };
    return (u, l) => (_(), K(Xe, null, {
      buttons: J(() => [
        a("button", {
          type: "button",
          onClick: c,
          class: "vf-btn vf-btn-primary"
        }, b(r(o)("Create")), 1),
        a("button", {
          type: "button",
          onClick: l[2] || (l[2] = (d) => r(e).modal.close()),
          class: "vf-btn vf-btn-secondary"
        }, b(r(o)("Cancel")), 1)
      ]),
      default: J(() => [
        a("div", null, [
          j(st, {
            icon: r(Ro),
            title: r(o)("New Folder")
          }, null, 8, ["icon", "title"]),
          a("div", hi, [
            a("div", gi, [
              a("p", bi, b(r(o)("Create a new folder")), 1),
              _e(a("input", {
                "onUpdate:modelValue": l[0] || (l[0] = (d) => s.value = d),
                onKeyup: $t(c, ["enter"]),
                class: "vuefinder__new-folder-modal__input",
                placeholder: r(o)("Folder Name"),
                type: "text"
              }, null, 40, wi), [
                [Ct, s.value]
              ]),
              i.value.length ? (_(), K(Ze, {
                key: 0,
                onHidden: l[1] || (l[1] = (d) => i.value = ""),
                error: ""
              }, {
                default: J(() => [
                  Z(b(i.value), 1)
                ]),
                _: 1
              })) : N("", !0)
            ])
          ])
        ])
      ]),
      _: 1
    }));
  }
}, yi = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  class: "h-6 w-6 md:h-8 md:w-8 m-auto vf-toolbar-icon",
  viewBox: "0 0 24 24"
};
function ki(t, e) {
  return _(), g("svg", yi, e[0] || (e[0] = [
    a("path", { d: "M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m3.75 9v6m3-3H9m1.5-12H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9" }, null, -1)
  ]));
}
const Uo = { render: ki }, xi = { class: "vuefinder__new-file-modal__content" }, Si = { class: "vuefinder__new-file-modal__form" }, $i = { class: "vuefinder__new-file-modal__description" }, Ci = ["placeholder"], Ei = {
  __name: "ModalNewFile",
  setup(t) {
    const e = re("ServiceContainer"), { getStore: n } = e.storage, { t: o } = e.i18n, s = O(""), i = O(""), c = () => {
      s.value !== "" && e.emitter.emit("vf-fetch", {
        params: {
          q: "newfile",
          m: "post",
          adapter: e.fs.adapter,
          path: e.fs.data.dirname
        },
        body: {
          name: s.value
        },
        onSuccess: () => {
          e.emitter.emit("vf-toast-push", {
            label: o("%s is created.", s.value)
          }), e.emitter.emit("vf-fetch", {
            params: {
              q: "index",
              adapter: e.fs.adapter,
              path: e.fs.data.dirname
            }
          });
        },
        onError: (u) => {
          i.value = o(u.message);
        }
      });
    };
    return (u, l) => (_(), K(Xe, null, {
      buttons: J(() => [
        a("button", {
          type: "button",
          onClick: c,
          class: "vf-btn vf-btn-primary"
        }, b(r(o)("Create")), 1),
        a("button", {
          type: "button",
          onClick: l[2] || (l[2] = (d) => r(e).modal.close()),
          class: "vf-btn vf-btn-secondary"
        }, b(r(o)("Cancel")), 1)
      ]),
      default: J(() => [
        a("div", null, [
          j(st, {
            icon: r(Uo),
            title: r(o)("New File")
          }, null, 8, ["icon", "title"]),
          a("div", xi, [
            a("div", Si, [
              a("p", $i, b(r(o)("Create a new file")), 1),
              _e(a("input", {
                "onUpdate:modelValue": l[0] || (l[0] = (d) => s.value = d),
                onKeyup: $t(c, ["enter"]),
                class: "vuefinder__new-file-modal__input",
                placeholder: r(o)("File Name"),
                type: "text"
              }, null, 40, Ci), [
                [Ct, s.value]
              ]),
              i.value.length ? (_(), K(Ze, {
                key: 0,
                onHidden: l[1] || (l[1] = (d) => i.value = ""),
                error: ""
              }, {
                default: J(() => [
                  Z(b(i.value), 1)
                ]),
                _: 1
              })) : N("", !0)
            ])
          ])
        ])
      ]),
      _: 1
    }));
  }
};
function Xn(t, e = 14) {
  let n = `((?=([\\w\\W]{0,${e}}))([\\w\\W]{${e + 1},})([\\w\\W]{8,}))`;
  return t.replace(new RegExp(n), "$2..$4");
}
const Ti = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  class: "h-6 w-6 md:h-8 md:w-8 m-auto vf-toolbar-icon",
  viewBox: "0 0 24 24"
};
function Ai(t, e) {
  return _(), g("svg", Ti, e[0] || (e[0] = [
    a("path", { d: "M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5m-13.5-9L12 3m0 0 4.5 4.5M12 3v13.5" }, null, -1)
  ]));
}
const No = { render: Ai }, Mi = { class: "vuefinder__upload-modal__content" }, Di = {
  key: 0,
  class: "pointer-events-none"
}, Oi = {
  key: 1,
  class: "pointer-events-none"
}, Li = ["disabled"], Vi = ["disabled"], Fi = { class: "vuefinder__upload-modal__file-list vf-scrollbar" }, Ii = ["textContent"], Hi = { class: "vuefinder__upload-modal__file-info" }, Ri = { class: "vuefinder__upload-modal__file-name hidden md:block" }, Bi = { class: "vuefinder__upload-modal__file-name md:hidden" }, Ui = {
  key: 0,
  class: "ml-auto"
}, Ni = ["title", "disabled", "onClick"], Pi = {
  key: 0,
  class: "py-2"
}, qi = ["disabled"], zi = {
  __name: "ModalUpload",
  setup(t) {
    const e = re("ServiceContainer"), { t: n } = e.i18n, o = n("uppy"), s = {
      PENDING: 0,
      CANCELED: 1,
      UPLOADING: 2,
      ERROR: 3,
      DONE: 10
    }, i = O({ QUEUE_ENTRY_STATUS: s }), c = O(null), u = O(null), l = O(null), d = O(null), v = O(null), f = O(null), m = O([]), p = O(""), h = O(!1), w = O(!1);
    let x;
    function M(D) {
      return m.value.findIndex((R) => R.id === D);
    }
    function I(D, R = null) {
      R = R ?? (D.webkitRelativePath || D.name), x.addFile({
        name: R,
        type: D.type,
        data: D,
        source: "Local"
      });
    }
    function L(D) {
      switch (D.status) {
        case s.DONE:
          return "text-green-600";
        case s.ERROR:
          return "text-red-600";
        case s.CANCELED:
          return "text-red-600";
        case s.PENDING:
        default:
          return "";
      }
    }
    const y = (D) => {
      switch (D.status) {
        case s.DONE:
          return "✓";
        case s.ERROR:
        case s.CANCELED:
          return "!";
        case s.PENDING:
        default:
          return "...";
      }
    };
    function V() {
      d.value.click();
    }
    function H() {
      if (!h.value) {
        if (!m.value.filter((D) => D.status !== s.DONE).length) {
          p.value = n("Please select file to upload first.");
          return;
        }
        p.value = "", x.retryAll(), x.upload();
      }
    }
    function E() {
      x.cancelAll({ reason: "user" }), m.value.forEach((D) => {
        D.status !== s.DONE && (D.status = s.CANCELED, D.statusName = n("Canceled"));
      }), h.value = !1;
    }
    function T(D) {
      h.value || (x.removeFile(D.id, "removed-by-user"), m.value.splice(M(D.id), 1));
    }
    function $(D) {
      if (!h.value) {
        if (x.cancelAll({ reason: "user" }), D) {
          const R = [];
          m.value.forEach((k) => {
            k.status !== s.DONE && R.push(k);
          }), m.value = [], R.forEach((k) => {
            I(k.originalFile, k.name);
          });
          return;
        }
        m.value.splice(0);
      }
    }
    function C() {
      e.modal.close();
    }
    function F() {
      return e.requester.transformRequestParams({
        url: "",
        method: "post",
        params: { q: "upload", adapter: e.fs.adapter, path: e.fs.data.dirname }
      });
    }
    return Ce(async () => {
      x = new hr({
        debug: e.debug,
        restrictions: {
          maxFileSize: Tr(e.maxFileSize)
          //maxNumberOfFiles
          //allowedFileTypes
        },
        locale: o,
        onBeforeFileAdded(k, U) {
          if (U[k.id] != null) {
            const ee = M(k.id);
            m.value[ee].status === s.PENDING && (p.value = x.i18n("noDuplicates", { fileName: k.name })), m.value = m.value.filter((se) => se.id !== k.id);
          }
          return m.value.push({
            id: k.id,
            name: k.name,
            size: e.filesize(k.size),
            status: s.PENDING,
            statusName: n("Pending upload"),
            percent: null,
            originalFile: k.data
          }), !0;
        }
      }), x.use(gr, {
        endpoint: "WILL_BE_REPLACED_BEFORE_UPLOAD",
        limit: 5,
        timeout: 0,
        getResponseError(k, U) {
          let P;
          try {
            P = JSON.parse(k).message;
          } catch {
            P = n("Cannot parse server response.");
          }
          return new Error(P);
        }
      }), x.on("restriction-failed", (k, U) => {
        const P = m.value[M(k.id)];
        T(P), p.value = U.message;
      }), x.on("upload", () => {
        const k = F();
        x.setMeta({ ...k.body });
        const U = x.getPlugin("XHRUpload");
        U.opts.method = k.method, U.opts.endpoint = k.url + "?" + new URLSearchParams(k.params), U.opts.headers = k.headers, delete k.headers["Content-Type"], h.value = !0, m.value.forEach((P) => {
          P.status !== s.DONE && (P.percent = null, P.status = s.UPLOADING, P.statusName = n("Pending upload"));
        });
      }), x.on("upload-progress", (k, U) => {
        const P = Math.floor(U.bytesUploaded / U.bytesTotal * 100);
        m.value[M(k.id)].percent = `${P}%`;
      }), x.on("upload-success", (k) => {
        const U = m.value[M(k.id)];
        U.status = s.DONE, U.statusName = n("Done"), e.emitter.emit("vf-fetch", {
          params: {
            q: "index",
            adapter: e.fs.adapter,
            path: e.fs.data.dirname
          }
        });
      }), x.on("upload-error", (k, U) => {
        const P = m.value[M(k.id)];
        P.percent = null, P.status = s.ERROR, U.isNetworkError ? P.statusName = n(
          "Network Error, Unable establish connection to the server or interrupted."
        ) : P.statusName = U ? U.message : n("Unknown Error");
      }), x.on("error", (k) => {
        p.value = k.message, h.value = !1, e.emitter.emit("vf-fetch", {
          params: {
            q: "index",
            adapter: e.fs.adapter,
            path: e.fs.data.dirname
          },
          noCloseModal: !0
        });
      }), x.on("complete", () => {
        h.value = !1, e.emitter.emit("vf-fetch", {
          params: {
            q: "index",
            adapter: e.fs.adapter,
            path: e.fs.data.dirname
          },
          noCloseModal: !0
        });
      }), d.value.addEventListener("click", () => {
        u.value.click();
      }), v.value.addEventListener("click", () => {
        l.value.click();
      }), f.value.addEventListener("dragover", (k) => {
        k.preventDefault(), w.value = !0;
      }), f.value.addEventListener("dragleave", (k) => {
        k.preventDefault(), w.value = !1;
      });
      function D(k, U) {
        U.isFile && U.file((P) => k(U, P)), U.isDirectory && U.createReader().readEntries((P) => {
          P.forEach((ee) => {
            D(k, ee);
          });
        });
      }
      f.value.addEventListener("drop", (k) => {
        k.preventDefault(), w.value = !1;
        const U = /^[/\\](.+)/;
        [...k.dataTransfer.items].forEach((P) => {
          P.kind === "file" && D((ee, se) => {
            const ne = U.exec(ee.fullPath);
            I(se, ne[1]);
          }, P.webkitGetAsEntry());
        });
      });
      const R = ({ target: k }) => {
        const U = k.files;
        for (const P of U)
          I(P);
        k.value = "";
      };
      u.value.addEventListener("change", R), l.value.addEventListener("change", R);
    }), Ns(() => {
      x == null || x.close({ reason: "unmount" });
    }), (D, R) => (_(), K(Xe, null, {
      buttons: J(() => [
        a("button", {
          type: "button",
          class: "vf-btn vf-btn-primary",
          disabled: h.value,
          onClick: tt(H, ["prevent"])
        }, b(r(n)("Upload")), 9, qi),
        h.value ? (_(), g("button", {
          key: 0,
          type: "button",
          class: "vf-btn vf-btn-secondary",
          onClick: tt(E, ["prevent"])
        }, b(r(n)("Cancel")), 1)) : (_(), g("button", {
          key: 1,
          type: "button",
          class: "vf-btn vf-btn-secondary",
          onClick: tt(C, ["prevent"])
        }, b(r(n)("Close")), 1))
      ]),
      default: J(() => [
        a("div", null, [
          j(st, {
            icon: r(No),
            title: r(n)("Upload Files")
          }, null, 8, ["icon", "title"]),
          a("div", Mi, [
            a("div", {
              class: "vuefinder__upload-modal__drop-area",
              ref_key: "dropArea",
              ref: f,
              onClick: V
            }, [
              w.value ? (_(), g("div", Di, b(r(n)("Release to drop these files.")), 1)) : (_(), g("div", Oi, b(r(n)("Drag and drop the files/folders to here or click here.")), 1))
            ], 512),
            a("div", {
              ref_key: "container",
              ref: c,
              class: "vuefinder__upload-modal__buttons"
            }, [
              a("button", {
                ref_key: "pickFiles",
                ref: d,
                type: "button",
                class: "vf-btn vf-btn-secondary"
              }, b(r(n)("Select Files")), 513),
              a("button", {
                ref_key: "pickFolders",
                ref: v,
                type: "button",
                class: "vf-btn vf-btn-secondary"
              }, b(r(n)("Select Folders")), 513),
              a("button", {
                type: "button",
                class: "vf-btn vf-btn-secondary",
                disabled: h.value,
                onClick: R[0] || (R[0] = (k) => $(!1))
              }, b(r(n)("Clear all")), 9, Li),
              a("button", {
                type: "button",
                class: "vf-btn vf-btn-secondary",
                disabled: h.value,
                onClick: R[1] || (R[1] = (k) => $(!0))
              }, b(r(n)("Clear only successful")), 9, Vi)
            ], 512),
            a("div", Fi, [
              (_(!0), g(ye, null, $e(m.value, (k) => (_(), g("div", {
                class: "vuefinder__upload-modal__file-entry",
                key: k.id
              }, [
                a("span", {
                  class: le(["vuefinder__upload-modal__file-icon", L(k)])
                }, [
                  a("span", {
                    class: "vuefinder__upload-modal__file-icon-text",
                    textContent: b(y(k))
                  }, null, 8, Ii)
                ], 2),
                a("div", Hi, [
                  a("div", Ri, b(r(Xn)(k.name, 40)) + " (" + b(k.size) + ") ", 1),
                  a("div", Bi, b(r(Xn)(k.name, 16)) + " (" + b(k.size) + ") ", 1),
                  a("div", {
                    class: le(["vuefinder__upload-modal__file-status", L(k)])
                  }, [
                    Z(b(k.statusName) + " ", 1),
                    k.status === i.value.QUEUE_ENTRY_STATUS.UPLOADING ? (_(), g("b", Ui, b(k.percent), 1)) : N("", !0)
                  ], 2)
                ]),
                a("button", {
                  type: "button",
                  class: le(["vuefinder__upload-modal__file-remove", h.value ? "disabled" : ""]),
                  title: r(n)("Delete"),
                  disabled: h.value,
                  onClick: (U) => T(k)
                }, R[3] || (R[3] = [
                  a("svg", {
                    xmlns: "http://www.w3.org/2000/svg",
                    fill: "none",
                    viewBox: "0 0 24 24",
                    "stroke-width": "1.5",
                    stroke: "currentColor",
                    class: "vuefinder__upload-modal__file-remove-icon"
                  }, [
                    a("path", {
                      "stroke-linecap": "round",
                      "stroke-linejoin": "round",
                      d: "M6 18L18 6M6 6l12 12"
                    })
                  ], -1)
                ]), 10, Ni)
              ]))), 128)),
              m.value.length ? N("", !0) : (_(), g("div", Pi, b(r(n)("No files selected!")), 1))
            ]),
            p.value.length ? (_(), K(Ze, {
              key: 0,
              onHidden: R[2] || (R[2] = (k) => p.value = ""),
              error: ""
            }, {
              default: J(() => [
                Z(b(p.value), 1)
              ]),
              _: 1
            })) : N("", !0)
          ])
        ]),
        a("input", {
          ref_key: "internalFileInput",
          ref: u,
          type: "file",
          multiple: "",
          class: "hidden"
        }, null, 512),
        a("input", {
          ref_key: "internalFolderInput",
          ref: l,
          type: "file",
          multiple: "",
          webkitdirectory: "",
          class: "hidden"
        }, null, 512)
      ]),
      _: 1
    }));
  }
}, ji = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  class: "h-6 w-6 md:h-8 md:w-8 m-auto",
  viewBox: "0 0 24 24"
};
function Gi(t, e) {
  return _(), g("svg", ji, e[0] || (e[0] = [
    a("path", { d: "m20.25 7.5-.625 10.632a2.25 2.25 0 0 1-2.247 2.118H6.622a2.25 2.25 0 0 1-2.247-2.118L3.75 7.5m6 4.125 2.25 2.25m0 0 2.25 2.25M12 13.875l2.25-2.25M12 13.875l-2.25 2.25M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125" }, null, -1)
  ]));
}
const Po = { render: Gi }, Wi = { class: "vuefinder__unarchive-modal__content" }, Ki = { class: "vuefinder__unarchive-modal__items" }, Yi = { class: "vuefinder__unarchive-modal__item" }, Xi = {
  key: 0,
  class: "vuefinder__unarchive-modal__icon vuefinder__unarchive-modal__icon--dir",
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  viewBox: "0 0 24 24",
  stroke: "currentColor",
  "stroke-width": "1"
}, Zi = {
  key: 1,
  class: "vuefinder__unarchive-modal__icon vuefinder__unarchive-modal__icon--file",
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  viewBox: "0 0 24 24",
  stroke: "currentColor",
  "stroke-width": "1"
}, Qi = { class: "vuefinder__unarchive-modal__item-name" }, Ji = { class: "vuefinder__unarchive-modal__info" }, qo = {
  __name: "ModalUnarchive",
  setup(t) {
    const e = re("ServiceContainer"), { t: n } = e.i18n, o = O(e.modal.data.items[0]), s = O(""), i = O([]), c = () => {
      e.emitter.emit("vf-fetch", {
        params: {
          q: "unarchive",
          m: "post",
          adapter: e.fs.adapter,
          path: e.fs.data.dirname
        },
        body: {
          item: o.value.path
        },
        onSuccess: () => {
          e.emitter.emit("vf-toast-push", { label: n("The file unarchived.") });
        },
        onError: (u) => {
          s.value = n(u.message);
        }
      });
    };
    return (u, l) => (_(), K(Xe, null, {
      buttons: J(() => [
        a("button", {
          type: "button",
          onClick: c,
          class: "vf-btn vf-btn-primary"
        }, b(r(n)("Unarchive")), 1),
        a("button", {
          type: "button",
          onClick: l[1] || (l[1] = (d) => r(e).modal.close()),
          class: "vf-btn vf-btn-secondary"
        }, b(r(n)("Cancel")), 1)
      ]),
      default: J(() => [
        a("div", null, [
          j(st, {
            icon: r(Po),
            title: r(n)("Unarchive")
          }, null, 8, ["icon", "title"]),
          a("div", Wi, [
            a("div", Ki, [
              (_(!0), g(ye, null, $e(i.value, (d) => (_(), g("p", Yi, [
                d.type === "dir" ? (_(), g("svg", Xi, l[2] || (l[2] = [
                  a("path", {
                    "stroke-linecap": "round",
                    "stroke-linejoin": "round",
                    d: "M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"
                  }, null, -1)
                ]))) : (_(), g("svg", Zi, l[3] || (l[3] = [
                  a("path", {
                    "stroke-linecap": "round",
                    "stroke-linejoin": "round",
                    d: "M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"
                  }, null, -1)
                ]))),
                a("span", Qi, b(d.basename), 1)
              ]))), 256)),
              a("p", Ji, b(r(n)("The archive will be unarchived at")) + " (" + b(r(e).fs.data.dirname) + ")", 1),
              s.value.length ? (_(), K(Ze, {
                key: 0,
                onHidden: l[0] || (l[0] = (d) => s.value = ""),
                error: ""
              }, {
                default: J(() => [
                  Z(b(s.value), 1)
                ]),
                _: 1
              })) : N("", !0)
            ])
          ])
        ])
      ]),
      _: 1
    }));
  }
}, ec = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  class: "h-6 w-6 md:h-8 md:w-8 m-auto",
  viewBox: "0 0 24 24"
};
function tc(t, e) {
  return _(), g("svg", ec, e[0] || (e[0] = [
    a("path", { d: "m20.25 7.5-.625 10.632a2.25 2.25 0 0 1-2.247 2.118H6.622a2.25 2.25 0 0 1-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125" }, null, -1)
  ]));
}
const zo = { render: tc }, nc = { class: "vuefinder__archive-modal__content" }, sc = { class: "vuefinder__archive-modal__form" }, oc = { class: "vuefinder__archive-modal__files vf-scrollbar" }, rc = { class: "vuefinder__archive-modal__file" }, ac = {
  key: 0,
  class: "vuefinder__archive-modal__icon vuefinder__archive-modal__icon--dir",
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  viewBox: "0 0 24 24",
  stroke: "currentColor",
  "stroke-width": "1"
}, lc = {
  key: 1,
  class: "vuefinder__archive-modal__icon",
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  viewBox: "0 0 24 24",
  stroke: "currentColor",
  "stroke-width": "1"
}, ic = { class: "vuefinder__archive-modal__file-name" }, cc = ["placeholder"], jo = {
  __name: "ModalArchive",
  setup(t) {
    const e = re("ServiceContainer"), { t: n } = e.i18n, o = O(""), s = O(""), i = O(e.modal.data.items), c = () => {
      i.value.length && e.emitter.emit("vf-fetch", {
        params: {
          q: "archive",
          m: "post",
          adapter: e.fs.adapter,
          path: e.fs.data.dirname
        },
        body: {
          items: i.value.map(({ path: u, type: l }) => ({ path: u, type: l })),
          name: o.value
        },
        onSuccess: () => {
          e.emitter.emit("vf-toast-push", { label: n("The file(s) archived.") });
        },
        onError: (u) => {
          s.value = n(u.message);
        }
      });
    };
    return (u, l) => (_(), K(Xe, null, {
      buttons: J(() => [
        a("button", {
          type: "button",
          onClick: c,
          class: "vf-btn vf-btn-primary"
        }, b(r(n)("Archive")), 1),
        a("button", {
          type: "button",
          onClick: l[2] || (l[2] = (d) => r(e).modal.close()),
          class: "vf-btn vf-btn-secondary"
        }, b(r(n)("Cancel")), 1)
      ]),
      default: J(() => [
        a("div", null, [
          j(st, {
            icon: r(zo),
            title: r(n)("Archive the files")
          }, null, 8, ["icon", "title"]),
          a("div", nc, [
            a("div", sc, [
              a("div", oc, [
                (_(!0), g(ye, null, $e(i.value, (d) => (_(), g("p", rc, [
                  d.type === "dir" ? (_(), g("svg", ac, l[3] || (l[3] = [
                    a("path", {
                      "stroke-linecap": "round",
                      "stroke-linejoin": "round",
                      d: "M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"
                    }, null, -1)
                  ]))) : (_(), g("svg", lc, l[4] || (l[4] = [
                    a("path", {
                      "stroke-linecap": "round",
                      "stroke-linejoin": "round",
                      d: "M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"
                    }, null, -1)
                  ]))),
                  a("span", ic, b(d.basename), 1)
                ]))), 256))
              ]),
              _e(a("input", {
                "onUpdate:modelValue": l[0] || (l[0] = (d) => o.value = d),
                onKeyup: $t(c, ["enter"]),
                class: "vuefinder__archive-modal__input",
                placeholder: r(n)("Archive name. (.zip file will be created)"),
                type: "text"
              }, null, 40, cc), [
                [Ct, o.value]
              ]),
              s.value.length ? (_(), K(Ze, {
                key: 0,
                onHidden: l[1] || (l[1] = (d) => s.value = ""),
                error: ""
              }, {
                default: J(() => [
                  Z(b(s.value), 1)
                ]),
                _: 1
              })) : N("", !0)
            ])
          ])
        ])
      ]),
      _: 1
    }));
  }
}, dc = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  class: "animate-spin p-0.5 h-5 w-5 text-white ml-auto",
  viewBox: "0 0 24 24"
};
function uc(t, e) {
  return _(), g("svg", dc, e[0] || (e[0] = [
    a("circle", {
      cx: "12",
      cy: "12",
      r: "10",
      stroke: "currentColor",
      "stroke-width": "4",
      class: "opacity-25 stroke-blue-900 dark:stroke-blue-100"
    }, null, -1),
    a("path", {
      fill: "currentColor",
      d: "M4 12a8 8 0 0 1 8-8V0C5.373 0 0 5.373 0 12zm2 5.291A7.96 7.96 0 0 1 4 12H0c0 3.042 1.135 5.824 3 7.938z",
      class: "opacity-75"
    }, null, -1)
  ]));
}
const _s = { render: uc }, fc = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  class: "h-6 w-6 md:h-8 md:w-8 m-auto vf-toolbar-icon",
  viewBox: "0 0 24 24"
};
function vc(t, e) {
  return _(), g("svg", fc, e[0] || (e[0] = [
    a("path", { d: "M3.75 3.75v4.5m0-4.5h4.5m-4.5 0L9 9M3.75 20.25v-4.5m0 4.5h4.5m-4.5 0L9 15M20.25 3.75h-4.5m4.5 0v4.5m0-4.5L15 9m5.25 11.25h-4.5m4.5 0v-4.5m0 4.5L15 15" }, null, -1)
  ]));
}
const _c = { render: vc }, mc = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  class: "h-6 w-6 md:h-8 md:w-8 m-auto vf-toolbar-icon",
  viewBox: "0 0 24 24"
};
function pc(t, e) {
  return _(), g("svg", mc, e[0] || (e[0] = [
    a("path", { d: "M9 9V4.5M9 9H4.5M9 9 3.75 3.75M9 15v4.5M9 15H4.5M9 15l-5.25 5.25M15 9h4.5M15 9V4.5M15 9l5.25-5.25M15 15h4.5M15 15v4.5m0-4.5 5.25 5.25" }, null, -1)
  ]));
}
const hc = { render: pc }, gc = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  class: "h-6 w-6 md:h-8 md:w-8 m-auto",
  viewBox: "0 0 24 24"
};
function bc(t, e) {
  return _(), g("svg", gc, e[0] || (e[0] = [
    a("path", { d: "M3.75 6A2.25 2.25 0 0 1 6 3.75h2.25A2.25 2.25 0 0 1 10.5 6v2.25a2.25 2.25 0 0 1-2.25 2.25H6a2.25 2.25 0 0 1-2.25-2.25zm0 9.75A2.25 2.25 0 0 1 6 13.5h2.25a2.25 2.25 0 0 1 2.25 2.25V18a2.25 2.25 0 0 1-2.25 2.25H6A2.25 2.25 0 0 1 3.75 18zM13.5 6a2.25 2.25 0 0 1 2.25-2.25H18A2.25 2.25 0 0 1 20.25 6v2.25A2.25 2.25 0 0 1 18 10.5h-2.25a2.25 2.25 0 0 1-2.25-2.25zm0 9.75a2.25 2.25 0 0 1 2.25-2.25H18a2.25 2.25 0 0 1 2.25 2.25V18A2.25 2.25 0 0 1 18 20.25h-2.25A2.25 2.25 0 0 1 13.5 18z" }, null, -1)
  ]));
}
const wc = { render: bc }, yc = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  class: "h-6 w-6 md:h-8 md:w-8 m-auto",
  viewBox: "0 0 24 24"
};
function kc(t, e) {
  return _(), g("svg", yc, e[0] || (e[0] = [
    a("path", { d: "M3.75 12h16.5m-16.5 3.75h16.5M3.75 19.5h16.5M5.625 4.5h12.75a1.875 1.875 0 0 1 0 3.75H5.625a1.875 1.875 0 0 1 0-3.75" }, null, -1)
  ]));
}
const xc = { render: kc }, Sc = { class: "vuefinder__toolbar" }, $c = {
  key: 0,
  class: "vuefinder__toolbar__actions"
}, Cc = ["title"], Ec = ["title"], Tc = ["title"], Ac = ["title"], Mc = ["title"], Dc = ["title"], Oc = ["title"], Lc = {
  key: 1,
  class: "vuefinder__toolbar__search-results"
}, Vc = { class: "pl-2" }, Fc = { class: "dark:bg-gray-700 bg-gray-200 text-xs px-2 py-1 rounded" }, Ic = { class: "vuefinder__toolbar__controls" }, Hc = ["title"], Rc = ["title"], Bc = {
  __name: "Toolbar",
  setup(t) {
    const e = re("ServiceContainer"), { setStore: n } = e.storage, { t: o } = e.i18n, s = e.dragSelect, i = O("");
    e.emitter.on("vf-search-query", ({ newQuery: l }) => {
      i.value = l;
    });
    const c = () => {
      e.fullScreen = !e.fullScreen;
    };
    Oe(() => e.fullScreen, () => {
      e.fullScreen ? document.querySelector("body").style.overflow = "hidden" : document.querySelector("body").style.overflow = "", n("full-screen", e.fullScreen), e.emitter.emit("vf-fullscreen-toggle");
    });
    const u = () => {
      e.view = e.view === "list" ? "grid" : "list", s.refreshSelection(), n("viewport", e.view);
    };
    return (l, d) => (_(), g("div", Sc, [
      i.value.length ? (_(), g("div", Lc, [
        a("div", Vc, [
          Z(b(r(o)("Search results for")) + " ", 1),
          a("span", Fc, b(i.value), 1)
        ]),
        r(e).loadingIndicator === "circular" && r(e).fs.loading ? (_(), K(r(_s), { key: 0 })) : N("", !0)
      ])) : (_(), g("div", $c, [
        r(e).features.includes(r(ue).NEW_FOLDER) ? (_(), g("div", {
          key: 0,
          class: "mx-1.5",
          title: r(o)("New Folder"),
          onClick: d[0] || (d[0] = (v) => r(e).modal.open(Bo, { items: r(s).getSelected() }))
        }, [
          j(r(Ro))
        ], 8, Cc)) : N("", !0),
        r(e).features.includes(r(ue).NEW_FILE) ? (_(), g("div", {
          key: 1,
          class: "mx-1.5",
          title: r(o)("New File"),
          onClick: d[1] || (d[1] = (v) => r(e).modal.open(Ei, { items: r(s).getSelected() }))
        }, [
          j(r(Uo))
        ], 8, Ec)) : N("", !0),
        r(e).features.includes(r(ue).RENAME) ? (_(), g("div", {
          key: 2,
          class: "mx-1.5",
          title: r(o)("Rename"),
          onClick: d[2] || (d[2] = (v) => r(s).getCount() !== 1 || r(e).modal.open(vs, { items: r(s).getSelected() }))
        }, [
          j(r(Ho), {
            class: le(r(s).getCount() === 1 ? "vf-toolbar-icon" : "vf-toolbar-icon-disabled")
          }, null, 8, ["class"])
        ], 8, Tc)) : N("", !0),
        r(e).features.includes(r(ue).DELETE) ? (_(), g("div", {
          key: 3,
          class: "mx-1.5",
          title: r(o)("Delete"),
          onClick: d[3] || (d[3] = (v) => !r(s).getCount() || r(e).modal.open(fs, { items: r(s).getSelected() }))
        }, [
          j(r(Io), {
            class: le(r(s).getCount() ? "vf-toolbar-icon" : "vf-toolbar-icon-disabled")
          }, null, 8, ["class"])
        ], 8, Ac)) : N("", !0),
        r(e).features.includes(r(ue).UPLOAD) ? (_(), g("div", {
          key: 4,
          class: "mx-1.5",
          title: r(o)("Upload"),
          onClick: d[4] || (d[4] = (v) => r(e).modal.open(zi, { items: r(s).getSelected() }))
        }, [
          j(r(No))
        ], 8, Mc)) : N("", !0),
        r(e).features.includes(r(ue).UNARCHIVE) && r(s).getCount() === 1 && r(s).getSelected()[0].mime_type === "application/zip" ? (_(), g("div", {
          key: 5,
          class: "mx-1.5",
          title: r(o)("Unarchive"),
          onClick: d[5] || (d[5] = (v) => !r(s).getCount() || r(e).modal.open(qo, { items: r(s).getSelected() }))
        }, [
          j(r(Po), {
            class: le(r(s).getCount() ? "vf-toolbar-icon" : "vf-toolbar-icon-disabled")
          }, null, 8, ["class"])
        ], 8, Dc)) : N("", !0),
        r(e).features.includes(r(ue).ARCHIVE) ? (_(), g("div", {
          key: 6,
          class: "mx-1.5",
          title: r(o)("Archive"),
          onClick: d[6] || (d[6] = (v) => !r(s).getCount() || r(e).modal.open(jo, { items: r(s).getSelected() }))
        }, [
          j(r(zo), {
            class: le(r(s).getCount() ? "vf-toolbar-icon" : "vf-toolbar-icon-disabled")
          }, null, 8, ["class"])
        ], 8, Oc)) : N("", !0)
      ])),
      a("div", Ic, [
        r(e).features.includes(r(ue).FULL_SCREEN) ? (_(), g("div", {
          key: 0,
          onClick: c,
          class: "mx-1.5",
          title: r(o)("Toggle Full Screen")
        }, [
          r(e).fullScreen ? (_(), K(r(hc), { key: 0 })) : (_(), K(r(_c), { key: 1 }))
        ], 8, Hc)) : N("", !0),
        a("div", {
          class: "mx-1.5",
          title: r(o)("Change View"),
          onClick: d[7] || (d[7] = (v) => i.value.length || u())
        }, [
          r(e).view === "grid" ? (_(), K(r(wc), {
            key: 0,
            class: le(["vf-toolbar-icon", i.value.length ? "vf-toolbar-icon-disabled" : ""])
          }, null, 8, ["class"])) : N("", !0),
          r(e).view === "list" ? (_(), K(r(xc), {
            key: 1,
            class: le(["vf-toolbar-icon", i.value.length ? "vf-toolbar-icon-disabled" : ""])
          }, null, 8, ["class"])) : N("", !0)
        ], 8, Rc)
      ])
    ]));
  }
}, Uc = (t, e = 0, n = !1) => {
  let o;
  return (...s) => {
    n && !o && t(...s), clearTimeout(o), o = setTimeout(() => {
      t(...s);
    }, e);
  };
}, Hs = (t, e, n) => {
  const o = O(t);
  return ir((s, i) => ({
    get() {
      return s(), o.value;
    },
    set: Uc(
      (c) => {
        o.value = c, i();
      },
      e,
      n
    )
  }));
}, Nc = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  "stroke-width": "2",
  "aria-hidden": "true",
  class: "h-6 w-6 stroke-blue-600 dark:stroke-blue-100",
  viewBox: "0 0 24 24"
};
function Pc(t, e) {
  return _(), g("svg", Nc, e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3"
    }, null, -1)
  ]));
}
const qc = { render: Pc }, zc = { class: "vuefinder__move-modal__content" }, jc = { class: "vuefinder__move-modal__description" }, Gc = { class: "vuefinder__move-modal__files vf-scrollbar" }, Wc = { class: "vuefinder__move-modal__file" }, Kc = {
  key: 0,
  class: "vuefinder__move-modal__icon vuefinder__move-modal__icon--dir",
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  viewBox: "0 0 24 24",
  stroke: "currentColor",
  "stroke-width": "1"
}, Yc = {
  key: 1,
  class: "vuefinder__move-modal__icon",
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  viewBox: "0 0 24 24",
  stroke: "currentColor",
  "stroke-width": "1"
}, Xc = { class: "vuefinder__move-modal__file-name" }, Zc = { class: "vuefinder__move-modal__target-title" }, Qc = { class: "vuefinder__move-modal__target-directory" }, Jc = { class: "vuefinder__move-modal__target-path" }, ed = { class: "vuefinder__move-modal__selected-items" }, Zn = {
  __name: "ModalMove",
  setup(t) {
    const e = re("ServiceContainer"), { t: n } = e.i18n, o = O(e.modal.data.items.from), s = O(""), i = () => {
      o.value.length && e.emitter.emit("vf-fetch", {
        params: {
          q: "move",
          m: "post",
          adapter: e.fs.adapter,
          path: e.fs.data.dirname
        },
        body: {
          items: o.value.map(({ path: c, type: u }) => ({ path: c, type: u })),
          item: e.modal.data.items.to.path
        },
        onSuccess: () => {
          e.emitter.emit("vf-toast-push", {
            label: n("Files moved.", e.modal.data.items.to.name)
          }), e.emitter.emit("vf-fetch", {
            params: {
              q: "index",
              adapter: e.fs.adapter,
              path: e.fs.data.dirname
            }
          });
        },
        onError: (c) => {
          s.value = n(c.message), e.emitter.emit("vf-fetch", {
            params: {
              q: "index",
              adapter: e.fs.adapter,
              path: e.fs.data.dirname
            }
          });
        }
      });
    };
    return (c, u) => (_(), K(Xe, null, {
      buttons: J(() => [
        a("button", {
          type: "button",
          onClick: i,
          class: "vf-btn vf-btn-primary"
        }, b(r(n)("Yes, Move!")), 1),
        a("button", {
          type: "button",
          onClick: u[1] || (u[1] = (l) => r(e).modal.close()),
          class: "vf-btn vf-btn-secondary"
        }, b(r(n)("Cancel")), 1),
        a("div", ed, b(r(n)("%s item(s) selected.", o.value.length)), 1)
      ]),
      default: J(() => [
        a("div", null, [
          j(st, {
            icon: r(qc),
            title: r(n)("Move files")
          }, null, 8, ["icon", "title"]),
          a("div", zc, [
            a("p", jc, b(r(n)("Are you sure you want to move these files?")), 1),
            a("div", Gc, [
              (_(!0), g(ye, null, $e(o.value, (l) => (_(), g("div", Wc, [
                a("div", null, [
                  l.type === "dir" ? (_(), g("svg", Kc, u[2] || (u[2] = [
                    a("path", {
                      "stroke-linecap": "round",
                      "stroke-linejoin": "round",
                      d: "M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"
                    }, null, -1)
                  ]))) : (_(), g("svg", Yc, u[3] || (u[3] = [
                    a("path", {
                      "stroke-linecap": "round",
                      "stroke-linejoin": "round",
                      d: "M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"
                    }, null, -1)
                  ])))
                ]),
                a("div", Xc, b(l.path), 1)
              ]))), 256))
            ]),
            a("h4", Zc, b(r(n)("Target Directory")), 1),
            a("p", Qc, [
              u[4] || (u[4] = a("svg", {
                xmlns: "http://www.w3.org/2000/svg",
                class: "vuefinder__move-modal__icon vuefinder__move-modal__icon--dir",
                fill: "none",
                viewBox: "0 0 24 24",
                stroke: "currentColor",
                "stroke-width": "1"
              }, [
                a("path", {
                  "stroke-linecap": "round",
                  "stroke-linejoin": "round",
                  d: "M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"
                })
              ], -1)),
              a("span", Jc, b(r(e).modal.data.items.to.path), 1)
            ]),
            s.value.length ? (_(), K(Ze, {
              key: 0,
              onHidden: u[0] || (u[0] = (l) => s.value = ""),
              error: ""
            }, {
              default: J(() => [
                Z(b(s.value), 1)
              ]),
              _: 1
            })) : N("", !0)
          ])
        ])
      ]),
      _: 1
    }));
  }
}, td = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "currentColor",
  class: "h-6 w-6 p-1 rounded text-slate-700 hover:bg-neutral-300 dark:text-neutral-200 dark:hover:bg-gray-700 cursor-pointer",
  viewBox: "-40 -40 580 580"
};
function nd(t, e) {
  return _(), g("svg", td, e[0] || (e[0] = [
    a("path", { d: "M463.5 224h8.5c13.3 0 24-10.7 24-24V72c0-9.7-5.8-18.5-14.8-22.2S461.9 48.1 455 55l-41.6 41.6c-87.6-86.5-228.7-86.2-315.8 1-87.5 87.5-87.5 229.3 0 316.8s229.3 87.5 316.8 0c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0c-62.5 62.5-163.8 62.5-226.3 0s-62.5-163.8 0-226.3c62.2-62.2 162.7-62.5 225.3-1L327 183c-6.9 6.9-8.9 17.2-5.2 26.2S334.3 224 344 224z" }, null, -1)
  ]));
}
const sd = { render: nd }, od = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "currentColor",
  class: "h-6 w-6 p-0.5 rounded",
  viewBox: "0 0 20 20"
};
function rd(t, e) {
  return _(), g("svg", od, e[0] || (e[0] = [
    a("path", {
      "fill-rule": "evenodd",
      d: "M5.293 9.707a1 1 0 0 1 0-1.414l4-4a1 1 0 0 1 1.414 0l4 4a1 1 0 0 1-1.414 1.414L11 7.414V15a1 1 0 1 1-2 0V7.414L6.707 9.707a1 1 0 0 1-1.414 0",
      class: "pointer-events-none",
      "clip-rule": "evenodd"
    }, null, -1)
  ]));
}
const ad = { render: rd }, ld = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  "stroke-width": "1.5",
  class: "h-6 w-6 p-1 rounded text-slate-700 hover:bg-neutral-300 dark:text-neutral-200 dark:hover:bg-gray-700 cursor-pointer",
  viewBox: "0 0 24 24"
};
function id(t, e) {
  return _(), g("svg", ld, e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M6 18 18 6M6 6l12 12"
    }, null, -1)
  ]));
}
const cd = { render: id }, dd = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "currentColor",
  class: "h-6 w-6 p-1 rounded text-slate-700 hover:bg-neutral-100 dark:text-neutral-300 dark:hover:bg-gray-800 cursor-pointer",
  viewBox: "0 0 20 20"
};
function ud(t, e) {
  return _(), g("svg", dd, e[0] || (e[0] = [
    a("path", {
      d: "M10.707 2.293a1 1 0 0 0-1.414 0l-7 7a1 1 0 0 0 1.414 1.414L4 10.414V17a1 1 0 0 0 1 1h2a1 1 0 0 0 1-1v-2a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v2a1 1 0 0 0 1 1h2a1 1 0 0 0 1-1v-6.586l.293.293a1 1 0 0 0 1.414-1.414z",
      class: "pointer-events-none"
    }, null, -1)
  ]));
}
const fd = { render: ud }, vd = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "currentColor",
  class: "h-6 w-6 p-1 m-auto stroke-gray-400 fill-gray-100 dark:stroke-gray-400 dark:fill-gray-400/20",
  viewBox: "0 0 20 20"
};
function _d(t, e) {
  return _(), g("svg", vd, e[0] || (e[0] = [
    a("path", { d: "m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607" }, null, -1)
  ]));
}
const md = { render: _d }, pd = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  "stroke-width": "1.5",
  class: "w-6 h-6 cursor-pointer",
  viewBox: "0 0 24 24"
};
function hd(t, e) {
  return _(), g("svg", pd, e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M6 18 18 6M6 6l12 12"
    }, null, -1)
  ]));
}
const gd = { render: hd }, bd = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  class: "text-neutral-500 fill-sky-500 stroke-sky-500 dark:fill-slate-500 dark:stroke-slate-500",
  viewBox: "0 0 24 24"
};
function wd(t, e) {
  return _(), g("svg", bd, e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M3 7v10a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-6l-2-2H5a2 2 0 0 0-2 2"
    }, null, -1)
  ]));
}
const kn = { render: wd }, yd = {
  xmlns: "http://www.w3.org/2000/svg",
  class: "h-6 w-6 rounded text-slate-700 hover:bg-neutral-100 dark:fill-neutral-300 dark:hover:bg-gray-800 cursor-pointer",
  viewBox: "0 0 448 512"
};
function kd(t, e) {
  return _(), g("svg", yd, e[0] || (e[0] = [
    a("path", { d: "M8 256a56 56 0 1 1 112 0 56 56 0 1 1-112 0m160 0a56 56 0 1 1 112 0 56 56 0 1 1-112 0m216-56a56 56 0 1 1 0 112 56 56 0 1 1 0-112" }, null, -1)
  ]));
}
const xd = { render: kd }, Sd = { class: "vuefinder__breadcrumb__container" }, $d = ["title"], Cd = ["title"], Ed = ["title"], Td = { class: "vuefinder__breadcrumb__list" }, Ad = {
  key: 0,
  class: "vuefinder__breadcrumb__hidden-list"
}, Md = { class: "relative" }, Dd = ["onDragover", "onDragleave", "onDrop", "title", "onClick"], Od = { class: "vuefinder__breadcrumb__search-mode" }, Ld = ["placeholder"], Vd = { class: "vuefinder__breadcrumb__hidden-dropdown" }, Fd = ["onDrop", "onClick"], Id = { class: "vuefinder__breadcrumb__hidden-item-content" }, Hd = { class: "vuefinder__breadcrumb__hidden-item-text" }, Rd = {
  __name: "Breadcrumb",
  setup(t) {
    const e = re("ServiceContainer"), { t: n } = e.i18n, o = e.dragSelect, { setStore: s } = e.storage, i = O(null), c = Hs(0, 100);
    Oe(c, (E) => {
      const T = i.value.children;
      let $ = 0, C = 0, F = 5, D = 1;
      e.fs.limitBreadcrumbItems(F), vt(() => {
        for (let R = T.length - 1; R >= 0 && !($ + T[R].offsetWidth > c.value - 40); R--)
          $ += parseInt(T[R].offsetWidth, 10), C++;
        C < D && (C = D), C > F && (C = F), e.fs.limitBreadcrumbItems(C);
      });
    });
    const u = () => {
      c.value = i.value.offsetWidth;
    };
    let l = O(null);
    Ce(() => {
      l.value = new ResizeObserver(u), l.value.observe(i.value);
    }), Qn(() => {
      l.value.disconnect();
    });
    const d = (E, T = null) => {
      E.preventDefault(), o.isDraggingRef.value = !1, m(E), T ?? (T = e.fs.hiddenBreadcrumbs.length - 1);
      let $ = JSON.parse(E.dataTransfer.getData("items"));
      if ($.find((C) => C.storage !== e.fs.adapter)) {
        alert("Moving items between different storages is not supported yet.");
        return;
      }
      e.modal.open(Zn, {
        items: {
          from: $,
          to: e.fs.hiddenBreadcrumbs[T] ?? { path: e.fs.adapter + "://" }
        }
      });
    }, v = (E, T = null) => {
      E.preventDefault(), o.isDraggingRef.value = !1, m(E), T ?? (T = e.fs.breadcrumbs.length - 2);
      let $ = JSON.parse(E.dataTransfer.getData("items"));
      if ($.find((C) => C.storage !== e.fs.adapter)) {
        alert("Moving items between different storages is not supported yet.");
        return;
      }
      e.modal.open(Zn, {
        items: {
          from: $,
          to: e.fs.breadcrumbs[T] ?? { path: e.fs.adapter + "://" }
        }
      });
    }, f = (E) => {
      E.preventDefault(), e.fs.isGoUpAvailable() ? (E.dataTransfer.dropEffect = "copy", E.currentTarget.classList.add("bg-blue-200", "dark:bg-slate-600")) : (E.dataTransfer.dropEffect = "none", E.dataTransfer.effectAllowed = "none");
    }, m = (E) => {
      E.preventDefault(), E.currentTarget.classList.remove("bg-blue-200", "dark:bg-slate-600"), e.fs.isGoUpAvailable() && E.currentTarget.classList.remove("bg-blue-200", "dark:bg-slate-600");
    }, p = () => {
      V(), e.emitter.emit("vf-fetch", {
        params: { q: "index", adapter: e.fs.adapter, path: e.fs.data.dirname }
      });
    }, h = () => {
      V(), !e.fs.isGoUpAvailable() || e.emitter.emit("vf-fetch", {
        params: {
          q: "index",
          adapter: e.fs.adapter,
          path: e.fs.parentFolderPath
        }
      });
    }, w = (E) => {
      e.emitter.emit("vf-fetch", {
        params: { q: "index", adapter: e.fs.adapter, path: E.path }
      }), e.fs.toggleHiddenBreadcrumbs(!1);
    }, x = () => {
      e.fs.showHiddenBreadcrumbs && e.fs.toggleHiddenBreadcrumbs(!1);
    }, M = {
      mounted(E, T, $, C) {
        E.clickOutsideEvent = function(F) {
          E === F.target || E.contains(F.target) || T.value();
        }, document.body.addEventListener("click", E.clickOutsideEvent);
      },
      beforeUnmount(E, T, $, C) {
        document.body.removeEventListener("click", E.clickOutsideEvent);
      }
    };
    Oe(
      () => e.showTreeView,
      (E, T) => {
        E !== T && s("show-tree-view", E);
      }
    );
    const I = O(null), L = () => {
      e.features.includes(ue.SEARCH) && (e.fs.searchMode = !0, vt(() => I.value.focus()));
    }, y = Hs("", 400);
    Oe(y, (E) => {
      e.emitter.emit("vf-toast-clear"), e.emitter.emit("vf-search-query", { newQuery: E });
    }), Oe(
      () => e.fs.searchMode,
      (E) => {
        E && vt(() => I.value.focus());
      }
    );
    const V = () => {
      e.fs.searchMode = !1, y.value = "";
    };
    e.emitter.on("vf-search-exit", () => {
      V();
    });
    const H = () => {
      y.value === "" && V();
    };
    return (E, T) => (_(), g("div", Sd, [
      a("span", {
        title: r(n)("Go up a directory")
      }, [
        j(r(ad), {
          onDragover: T[0] || (T[0] = ($) => f($)),
          onDragleave: T[1] || (T[1] = ($) => m($)),
          onDrop: T[2] || (T[2] = ($) => v($)),
          onClick: h,
          class: le(
            r(e).fs.isGoUpAvailable() ? "vuefinder__breadcrumb__go-up--active" : "vuefinder__breadcrumb__go-up--inactive"
          )
        }, null, 8, ["class"])
      ], 8, $d),
      r(e).fs.loading ? (_(), g("span", {
        key: 1,
        title: r(n)("Cancel")
      }, [
        j(r(cd), {
          onClick: T[3] || (T[3] = ($) => r(e).emitter.emit("vf-fetch-abort"))
        })
      ], 8, Ed)) : (_(), g("span", {
        key: 0,
        title: r(n)("Refresh")
      }, [
        j(r(sd), { onClick: p })
      ], 8, Cd)),
      _e(a("div", {
        onClick: tt(L, ["self"]),
        class: "group vuefinder__breadcrumb__search-container"
      }, [
        a("div", null, [
          j(r(fd), {
            onDragover: T[4] || (T[4] = ($) => f($)),
            onDragleave: T[5] || (T[5] = ($) => m($)),
            onDrop: T[6] || (T[6] = ($) => v($, -1)),
            onClick: T[7] || (T[7] = ($) => r(e).emitter.emit("vf-fetch", {
              params: { q: "index", adapter: r(e).fs.adapter }
            }))
          })
        ]),
        a("div", Td, [
          r(e).fs.hiddenBreadcrumbs.length ? _e((_(), g("div", Ad, [
            T[13] || (T[13] = a("div", { class: "vuefinder__breadcrumb__separator" }, "/", -1)),
            a("div", Md, [
              a("span", {
                onDragenter: T[8] || (T[8] = ($) => r(e).fs.toggleHiddenBreadcrumbs(!0)),
                onClick: T[9] || (T[9] = ($) => r(e).fs.toggleHiddenBreadcrumbs()),
                class: "vuefinder__breadcrumb__hidden-toggle"
              }, [
                j(r(xd), { class: "vuefinder__breadcrumb__hidden-toggle-icon" })
              ], 32)
            ])
          ])), [
            [M, x]
          ]) : N("", !0)
        ]),
        a("div", {
          ref_key: "breadcrumbContainer",
          ref: i,
          class: "vuefinder__breadcrumb__visible-list",
          onClick: tt(L, ["self"])
        }, [
          (_(!0), g(ye, null, $e(r(e).fs.breadcrumbs, ($, C) => (_(), g("div", { key: C }, [
            T[14] || (T[14] = a("span", { class: "vuefinder__breadcrumb__separator" }, "/", -1)),
            a("span", {
              onDragover: (F) => C === r(e).fs.breadcrumbs.length - 1 || f(F),
              onDragleave: (F) => C === r(e).fs.breadcrumbs.length - 1 || m(F),
              onDrop: (F) => C === r(e).fs.breadcrumbs.length - 1 || v(F, C),
              class: "vuefinder__breadcrumb__item",
              title: $.basename,
              onClick: (F) => r(e).emitter.emit("vf-fetch", {
                params: {
                  q: "index",
                  adapter: r(e).fs.adapter,
                  path: $.path
                }
              })
            }, b($.name), 41, Dd)
          ]))), 128))
        ], 512),
        r(e).loadingIndicator === "circular" && r(e).fs.loading ? (_(), K(r(_s), { key: 0 })) : N("", !0)
      ], 512), [
        [je, !r(e).fs.searchMode]
      ]),
      _e(a("div", Od, [
        a("div", null, [
          j(r(md))
        ]),
        _e(a("input", {
          ref_key: "searchInput",
          ref: I,
          onKeydown: $t(V, ["esc"]),
          onBlur: H,
          "onUpdate:modelValue": T[10] || (T[10] = ($) => cr(y) ? y.value = $ : null),
          placeholder: r(n)("Search anything.."),
          class: "vuefinder__breadcrumb__search-input",
          type: "text"
        }, null, 40, Ld), [
          [Ct, r(y)]
        ]),
        j(r(gd), { onClick: V })
      ], 512), [
        [je, r(e).fs.searchMode]
      ]),
      _e(a("div", Vd, [
        (_(!0), g(ye, null, $e(r(e).fs.hiddenBreadcrumbs, ($, C) => (_(), g("div", {
          key: C,
          onDragover: T[11] || (T[11] = (F) => f(F)),
          onDragleave: T[12] || (T[12] = (F) => m(F)),
          onDrop: (F) => d(F, C),
          onClick: (F) => w($),
          class: "vuefinder__breadcrumb__hidden-item"
        }, [
          a("div", Id, [
            a("span", null, [
              j(r(kn), { class: "vuefinder__breadcrumb__hidden-item-icon" })
            ]),
            a("span", Hd, b($.name), 1)
          ])
        ], 40, Fd))), 128))
      ], 512), [
        [je, r(e).fs.showHiddenBreadcrumbs]
      ])
    ]));
  }
}, Go = (t, e = null) => new Date(t * 1e3).toLocaleString(e ?? "ru-RU"), Bd = ["onClick"], Ud = {
  __name: "Toast",
  setup(t) {
    const e = re("ServiceContainer"), { getStore: n } = e.storage, o = O(n("full-screen", !1)), s = O([]), i = (l) => l === "error" ? "text-red-400 border-red-400 dark:text-red-300 dark:border-red-300" : "text-lime-600 border-lime-600 dark:text-lime-300 dark:border-lime-1300", c = (l) => {
      s.value.splice(l, 1);
    }, u = (l) => {
      let d = s.value.findIndex((v) => v.id === l);
      d !== -1 && c(d);
    };
    return e.emitter.on("vf-toast-clear", () => {
      s.value = [];
    }), e.emitter.on("vf-toast-push", (l) => {
      let d = (/* @__PURE__ */ new Date()).getTime().toString(36).concat(performance.now().toString(), Math.random().toString()).replace(/\./g, "");
      l.id = d, s.value.push(l), setTimeout(() => {
        u(d);
      }, 5e3);
    }), (l, d) => (_(), g("div", {
      class: le(["vuefinder__toast", o.value.value ? "vuefinder__toast--fixed" : "vuefinder__toast--absolute"])
    }, [
      j(dr, {
        name: "vuefinder__toast-item",
        "enter-active-class": "vuefinder__toast-item--enter-active",
        "leave-active-class": "vuefinder__toast-item--leave-active",
        "leave-to-class": "vuefinder__toast-item--leave-to"
      }, {
        default: J(() => [
          (_(!0), g(ye, null, $e(s.value, (v, f) => (_(), g("div", {
            key: f,
            onClick: (m) => c(f),
            class: le(["vuefinder__toast__message", i(v.type)])
          }, b(v.label), 11, Bd))), 128))
        ]),
        _: 1
      })
    ], 2));
  }
}, Nd = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "currentColor",
  class: "h-5 w-5",
  viewBox: "0 0 20 20"
};
function Pd(t, e) {
  return _(), g("svg", Nd, e[0] || (e[0] = [
    a("path", {
      "fill-rule": "evenodd",
      d: "M5.293 7.293a1 1 0 0 1 1.414 0L10 10.586l3.293-3.293a1 1 0 1 1 1.414 1.414l-4 4a1 1 0 0 1-1.414 0l-4-4a1 1 0 0 1 0-1.414",
      "clip-rule": "evenodd"
    }, null, -1)
  ]));
}
const qd = { render: Pd }, zd = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "currentColor",
  class: "h-5 w-5",
  viewBox: "0 0 20 20"
};
function jd(t, e) {
  return _(), g("svg", zd, e[0] || (e[0] = [
    a("path", {
      "fill-rule": "evenodd",
      d: "M14.707 12.707a1 1 0 0 1-1.414 0L10 9.414l-3.293 3.293a1 1 0 0 1-1.414-1.414l4-4a1 1 0 0 1 1.414 0l4 4a1 1 0 0 1 0 1.414",
      "clip-rule": "evenodd"
    }, null, -1)
  ]));
}
const Gd = { render: jd }, Jt = {
  __name: "SortIcon",
  props: { direction: String },
  setup(t) {
    return (e, n) => (_(), g("div", null, [
      t.direction === "asc" ? (_(), K(r(qd), { key: 0 })) : N("", !0),
      t.direction === "desc" ? (_(), K(r(Gd), { key: 1 })) : N("", !0)
    ]));
  }
}, Wd = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  class: "text-neutral-500",
  viewBox: "0 0 24 24"
};
function Kd(t, e) {
  return _(), g("svg", Wd, e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M7 21h10a2 2 0 0 0 2-2V9.414a1 1 0 0 0-.293-.707l-5.414-5.414A1 1 0 0 0 12.586 3H7a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2"
    }, null, -1)
  ]));
}
const Yd = { render: Kd }, Xd = { class: "vuefinder__item-icon" }, Vn = {
  __name: "ItemIcon",
  props: {
    type: {
      type: String,
      required: !0
    },
    small: {
      type: Boolean,
      default: !1
    }
  },
  setup(t) {
    return (e, n) => (_(), g("span", Xd, [
      t.type === "dir" ? (_(), K(r(kn), {
        key: 0,
        class: le(t.small ? "vuefinder__item-icon--small" : "vuefinder__item-icon--large")
      }, null, 8, ["class"])) : (_(), K(r(Yd), {
        key: 1,
        class: le(t.small ? "vuefinder__item-icon--small" : "vuefinder__item-icon--large")
      }, null, 8, ["class"]))
    ]));
  }
}, Zd = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  class: "absolute h-6 w-6 md:h-12 md:w-12 m-auto stroke-neutral-500 fill-white dark:fill-gray-700 dark:stroke-gray-600 z-10",
  viewBox: "0 0 24 24"
};
function Qd(t, e) {
  return _(), g("svg", Zd, e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M7 21h10a2 2 0 0 0 2-2V9.414a1 1 0 0 0-.293-.707l-5.414-5.414A1 1 0 0 0 12.586 3H7a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2"
    }, null, -1)
  ]));
}
const Jd = { render: Qd }, eu = { class: "vuefinder__drag-item__container" }, tu = { class: "vuefinder__drag-item__count" }, nu = {
  __name: "DragItem",
  props: {
    count: {
      type: Number,
      default: 0
    }
  },
  setup(t) {
    const e = t;
    return (n, o) => (_(), g("div", eu, [
      j(r(Jd)),
      a("div", tu, b(e.count), 1)
    ]));
  }
}, su = { class: "vuefinder__text-preview" }, ou = { class: "vuefinder__text-preview__header" }, ru = ["title"], au = { class: "vuefinder__text-preview__actions" }, lu = {
  key: 0,
  class: "vuefinder__text-preview__content"
}, iu = { key: 1 }, cu = {
  __name: "Text",
  emits: ["success"],
  setup(t, { emit: e }) {
    const n = e, o = O(""), s = O(""), i = O(null), c = O(!1), u = O(""), l = O(!1), d = re("ServiceContainer"), { t: v } = d.i18n;
    Ce(() => {
      d.requester.send({
        url: "",
        method: "get",
        params: { q: "preview", adapter: d.modal.data.adapter, path: d.modal.data.item.path },
        responseType: "text"
      }).then((p) => {
        o.value = p, n("success");
      });
    });
    const f = () => {
      c.value = !c.value, s.value = o.value;
    }, m = () => {
      u.value = "", l.value = !1, d.requester.send({
        url: "",
        method: "post",
        params: {
          q: "save",
          adapter: d.modal.data.adapter,
          path: d.modal.data.item.path
        },
        body: {
          content: s.value
        },
        responseType: "text"
      }).then((p) => {
        u.value = v("Updated."), o.value = p, n("success"), c.value = !c.value;
      }).catch((p) => {
        u.value = v(p.message), l.value = !0;
      });
    };
    return (p, h) => (_(), g("div", su, [
      a("div", ou, [
        a("div", {
          class: "vuefinder__text-preview__title",
          id: "modal-title",
          title: r(d).modal.data.item.path
        }, b(r(d).modal.data.item.basename), 9, ru),
        a("div", au, [
          c.value ? (_(), g("button", {
            key: 0,
            onClick: m,
            class: "vuefinder__text-preview__save-button"
          }, b(r(v)("Save")), 1)) : N("", !0),
          r(d).features.includes(r(ue).EDIT) ? (_(), g("button", {
            key: 1,
            class: "vuefinder__text-preview__edit-button",
            onClick: h[0] || (h[0] = (w) => f())
          }, b(c.value ? r(v)("Cancel") : r(v)("Edit")), 1)) : N("", !0)
        ])
      ]),
      a("div", null, [
        c.value ? (_(), g("div", iu, [
          _e(a("textarea", {
            ref_key: "editInput",
            ref: i,
            "onUpdate:modelValue": h[1] || (h[1] = (w) => s.value = w),
            class: "vuefinder__text-preview__textarea",
            name: "text",
            cols: "30",
            rows: "10"
          }, null, 512), [
            [Ct, s.value]
          ])
        ])) : (_(), g("pre", lu, b(o.value), 1)),
        u.value.length ? (_(), K(Ze, {
          key: 2,
          onHidden: h[2] || (h[2] = (w) => u.value = ""),
          error: l.value
        }, {
          default: J(() => [
            Z(b(u.value), 1)
          ]),
          _: 1
        }, 8, ["error"])) : N("", !0)
      ])
    ]));
  }
}, du = { class: "vuefinder__image-preview" }, uu = { class: "vuefinder__image-preview__header" }, fu = ["title"], vu = { class: "vuefinder__image-preview__actions" }, _u = { class: "vuefinder__image-preview__image-container" }, mu = ["src"], pu = {
  __name: "Image",
  emits: ["success"],
  setup(t, { emit: e }) {
    const n = e, o = re("ServiceContainer"), { t: s } = o.i18n, i = O(null), c = O(null), u = O(!1), l = O(""), d = O(!1), v = () => {
      u.value = !u.value, u.value ? c.value = new wr(i.value, {
        crop(m) {
        }
      }) : c.value.destroy();
    }, f = () => {
      c.value.getCroppedCanvas({
        width: 795,
        height: 341
      }).toBlob(
        (m) => {
          l.value = "", d.value = !1;
          const p = new FormData();
          p.set("file", m), o.requester.send({
            url: "",
            method: "post",
            params: {
              q: "upload",
              adapter: o.modal.data.adapter,
              path: o.modal.data.item.path
            },
            body: p
          }).then((h) => {
            l.value = s("Updated."), i.value.src = o.requester.getPreviewUrl(o.modal.data.adapter, o.modal.data.item), v(), n("success");
          }).catch((h) => {
            l.value = s(h.message), d.value = !0;
          });
        }
      );
    };
    return Ce(() => {
      n("success");
    }), (m, p) => (_(), g("div", du, [
      a("div", uu, [
        a("h3", {
          class: "vuefinder__image-preview__title",
          id: "modal-title",
          title: r(o).modal.data.item.path
        }, b(r(o).modal.data.item.basename), 9, fu),
        a("div", vu, [
          u.value ? (_(), g("button", {
            key: 0,
            onClick: f,
            class: "vuefinder__image-preview__crop-button"
          }, b(r(s)("Crop")), 1)) : N("", !0),
          r(o).features.includes(r(ue).EDIT) ? (_(), g("button", {
            key: 1,
            class: "vuefinder__image-preview__edit-button",
            onClick: p[0] || (p[0] = (h) => v())
          }, b(u.value ? r(s)("Cancel") : r(s)("Edit")), 1)) : N("", !0)
        ])
      ]),
      a("div", _u, [
        a("img", {
          ref_key: "image",
          ref: i,
          class: "vuefinder__image-preview__image",
          src: r(o).requester.getPreviewUrl(r(o).modal.data.adapter, r(o).modal.data.item),
          alt: ""
        }, null, 8, mu)
      ]),
      l.value.length ? (_(), K(Ze, {
        key: 0,
        onHidden: p[1] || (p[1] = (h) => l.value = ""),
        error: d.value
      }, {
        default: J(() => [
          Z(b(l.value), 1)
        ]),
        _: 1
      }, 8, ["error"])) : N("", !0)
    ]));
  }
}, hu = { class: "vuefinder__default-preview" }, gu = { class: "vuefinder__default-preview__header" }, bu = ["title"], wu = {
  __name: "Default",
  emits: ["success"],
  setup(t, { emit: e }) {
    const n = re("ServiceContainer"), o = e;
    return Ce(() => {
      o("success");
    }), (s, i) => (_(), g("div", hu, [
      a("div", gu, [
        a("h3", {
          class: "vuefinder__default-preview__title",
          id: "modal-title",
          title: r(n).modal.data.item.path
        }, b(r(n).modal.data.item.basename), 9, bu)
      ]),
      i[0] || (i[0] = a("div", null, null, -1))
    ]));
  }
}, yu = { class: "vuefinder__video-preview" }, ku = ["title"], xu = {
  class: "vuefinder__video-preview__video",
  preload: "",
  controls: ""
}, Su = ["src"], $u = {
  __name: "Video",
  emits: ["success"],
  setup(t, { emit: e }) {
    const n = re("ServiceContainer"), o = e, s = () => n.requester.getPreviewUrl(n.modal.data.adapter, n.modal.data.item);
    return Ce(() => {
      o("success");
    }), (i, c) => (_(), g("div", yu, [
      a("h3", {
        class: "vuefinder__video-preview__title",
        id: "modal-title",
        title: r(n).modal.data.item.path
      }, b(r(n).modal.data.item.basename), 9, ku),
      a("div", null, [
        a("video", xu, [
          a("source", {
            src: s(),
            type: "video/mp4"
          }, null, 8, Su),
          c[0] || (c[0] = Z(" Your browser does not support the video tag. "))
        ])
      ])
    ]));
  }
}, Cu = { class: "vuefinder__audio-preview" }, Eu = ["title"], Tu = {
  class: "vuefinder__audio-preview__audio",
  controls: ""
}, Au = ["src"], Mu = {
  __name: "Audio",
  emits: ["success"],
  setup(t, { emit: e }) {
    const n = e, o = re("ServiceContainer"), s = () => o.requester.getPreviewUrl(o.modal.data.adapter, o.modal.data.item);
    return Ce(() => {
      n("success");
    }), (i, c) => (_(), g("div", Cu, [
      a("h3", {
        class: "vuefinder__audio-preview__title",
        id: "modal-title",
        title: r(o).modal.data.item.path
      }, b(r(o).modal.data.item.basename), 9, Eu),
      a("div", null, [
        a("audio", Tu, [
          a("source", {
            src: s(),
            type: "audio/mpeg"
          }, null, 8, Au),
          c[0] || (c[0] = Z(" Your browser does not support the audio element. "))
        ])
      ])
    ]));
  }
}, Du = { class: "vuefinder__pdf-preview" }, Ou = ["title"], Lu = ["data"], Vu = ["src"], Fu = {
  __name: "Pdf",
  emits: ["success"],
  setup(t, { emit: e }) {
    const n = re("ServiceContainer"), o = e, s = () => n.requester.getPreviewUrl(n.modal.data.adapter, n.modal.data.item);
    return Ce(() => {
      o("success");
    }), (i, c) => (_(), g("div", Du, [
      a("h3", {
        class: "vuefinder__pdf-preview__title",
        id: "modal-title",
        title: r(n).modal.data.item.path
      }, b(r(n).modal.data.item.basename), 9, Ou),
      a("div", null, [
        a("object", {
          class: "vuefinder__pdf-preview__object",
          data: s(),
          type: "application/pdf",
          width: "100%",
          height: "100%"
        }, [
          a("iframe", {
            class: "vuefinder__pdf-preview__iframe",
            src: s(),
            width: "100%",
            height: "100%"
          }, " Your browser does not support PDFs ", 8, Vu)
        ], 8, Lu)
      ])
    ]));
  }
}, Iu = { class: "vuefinder__preview-modal__content" }, Hu = { key: 0 }, Ru = { class: "vuefinder__preview-modal__loading" }, Bu = {
  key: 0,
  class: "vuefinder__preview-modal__loading-indicator"
}, Uu = { class: "vuefinder__preview-modal__details" }, Nu = { class: "font-bold" }, Pu = { class: "font-bold pl-2" }, qu = {
  key: 0,
  class: "vuefinder__preview-modal__note"
}, zu = ["download", "href"], Wo = {
  __name: "ModalPreview",
  setup(t) {
    const e = re("ServiceContainer"), { t: n } = e.i18n, o = O(!1), s = (c) => (e.modal.data.item.mime_type ?? "").startsWith(c), i = e.features.includes(ue.PREVIEW);
    return i || (o.value = !0), (c, u) => (_(), K(Xe, null, {
      buttons: J(() => [
        a("button", {
          type: "button",
          onClick: u[6] || (u[6] = (l) => r(e).modal.close()),
          class: "vf-btn vf-btn-secondary"
        }, b(r(n)("Close")), 1),
        r(e).features.includes(r(ue).DOWNLOAD) ? (_(), g("a", {
          key: 0,
          target: "_blank",
          class: "vf-btn vf-btn-primary",
          download: r(e).requester.getDownloadUrl(r(e).modal.data.adapter, r(e).modal.data.item),
          href: r(e).requester.getDownloadUrl(r(e).modal.data.adapter, r(e).modal.data.item)
        }, b(r(n)("Download")), 9, zu)) : N("", !0)
      ]),
      default: J(() => [
        a("div", null, [
          a("div", Iu, [
            r(i) ? (_(), g("div", Hu, [
              s("text") ? (_(), K(cu, {
                key: 0,
                onSuccess: u[0] || (u[0] = (l) => o.value = !0)
              })) : s("image") ? (_(), K(pu, {
                key: 1,
                onSuccess: u[1] || (u[1] = (l) => o.value = !0)
              })) : s("video") ? (_(), K($u, {
                key: 2,
                onSuccess: u[2] || (u[2] = (l) => o.value = !0)
              })) : s("audio") ? (_(), K(Mu, {
                key: 3,
                onSuccess: u[3] || (u[3] = (l) => o.value = !0)
              })) : s("application/pdf") ? (_(), K(Fu, {
                key: 4,
                onSuccess: u[4] || (u[4] = (l) => o.value = !0)
              })) : (_(), K(wu, {
                key: 5,
                onSuccess: u[5] || (u[5] = (l) => o.value = !0)
              }))
            ])) : N("", !0),
            a("div", Ru, [
              o.value === !1 ? (_(), g("div", Bu, [
                u[7] || (u[7] = a("svg", {
                  class: "vuefinder__preview-modal__spinner",
                  xmlns: "http://www.w3.org/2000/svg",
                  fill: "none",
                  viewBox: "0 0 24 24"
                }, [
                  a("circle", {
                    class: "vuefinder__preview-modal__spinner-circle",
                    cx: "12",
                    cy: "12",
                    r: "10",
                    stroke: "currentColor",
                    "stroke-width": "4"
                  }),
                  a("path", {
                    class: "vuefinder__preview-modal__spinner-path",
                    fill: "currentColor",
                    d: "M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"
                  })
                ], -1)),
                a("span", null, b(r(n)("Loading")), 1)
              ])) : N("", !0)
            ])
          ])
        ]),
        a("div", Uu, [
          a("div", null, [
            a("span", Nu, b(r(n)("File Size")) + ": ", 1),
            Z(b(r(e).filesize(r(e).modal.data.item.file_size)), 1)
          ]),
          a("div", null, [
            a("span", Pu, b(r(n)("Last Modified")) + ": ", 1),
            Z(" " + b(r(Go)(r(e).modal.data.item.last_modified)), 1)
          ])
        ]),
        r(e).features.includes(r(ue).DOWNLOAD) ? (_(), g("div", qu, [
          a("span", null, b(r(n)(`Download doesn't work? You can try right-click "Download" button, select "Save link as...".`)), 1)
        ])) : N("", !0)
      ]),
      _: 1
    }));
  }
}, ju = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  "stroke-linecap": "round",
  "stroke-linejoin": "round",
  "stroke-width": "2",
  class: "h-5 w-5",
  viewBox: "0 0 24 24"
};
function Gu(t, e) {
  return _(), g("svg", ju, e[0] || (e[0] = [
    a("path", {
      stroke: "none",
      d: "M0 0h24v24H0z"
    }, null, -1),
    a("path", { d: "m15 4.5-4 4L7 10l-1.5 1.5 7 7L14 17l1.5-4 4-4M9 15l-4.5 4.5M14.5 4 20 9.5" }, null, -1)
  ]));
}
const Ko = { render: Gu }, Wu = ["data-type", "data-item", "data-index"], Fn = {
  __name: "Item",
  props: {
    item: { type: Object },
    index: { type: Number },
    dragImage: { type: Object }
  },
  setup(t) {
    const e = re("ServiceContainer"), n = e.dragSelect, o = t, s = (p) => {
      p.type === "dir" ? (e.emitter.emit("vf-search-exit"), e.emitter.emit("vf-fetch", { params: { q: "index", adapter: e.fs.adapter, path: p.path } })) : e.modal.open(Wo, { adapter: e.fs.adapter, item: p });
    }, i = {
      mounted(p, h, w, x) {
        w.props.draggable && (p.addEventListener("dragstart", (M) => c(M, h.value)), p.addEventListener("dragover", (M) => l(M, h.value)), p.addEventListener("drop", (M) => u(M, h.value)));
      },
      beforeUnmount(p, h, w, x) {
        w.props.draggable && (p.removeEventListener("dragstart", c), p.removeEventListener("dragover", l), p.removeEventListener("drop", u));
      }
    }, c = (p, h) => {
      if (p.altKey || p.ctrlKey || p.metaKey)
        return p.preventDefault(), !1;
      n.isDraggingRef.value = !0, p.dataTransfer.setDragImage(o.dragImage.$el, 0, 15), p.dataTransfer.effectAllowed = "all", p.dataTransfer.dropEffect = "copy", p.dataTransfer.setData("items", JSON.stringify(n.getSelected()));
    }, u = (p, h) => {
      p.preventDefault(), n.isDraggingRef.value = !1;
      let w = JSON.parse(p.dataTransfer.getData("items"));
      if (w.find((x) => x.storage !== e.fs.adapter)) {
        alert("Moving items between different storages is not supported yet.");
        return;
      }
      e.modal.open(Zn, { items: { from: w, to: h } });
    }, l = (p, h) => {
      p.preventDefault(), !h || h.type !== "dir" || n.getSelection().find((w) => w === p.currentTarget) ? (p.dataTransfer.dropEffect = "none", p.dataTransfer.effectAllowed = "none") : p.dataTransfer.dropEffect = "copy";
    };
    let d = null, v = !1;
    const f = () => {
      d && clearTimeout(d);
    }, m = (p) => {
      if (!v)
        v = !0, setTimeout(() => v = !1, 300);
      else
        return v = !1, s(o.item), clearTimeout(d), !1;
      d = setTimeout(() => {
        const h = new MouseEvent("contextmenu", {
          bubbles: !0,
          cancelable: !1,
          view: window,
          button: 2,
          buttons: 0,
          clientX: p.target.getBoundingClientRect().x,
          clientY: p.target.getBoundingClientRect().y
        });
        p.target.dispatchEvent(h);
      }, 500);
    };
    return (p, h) => _e((_(), g("div", {
      style: fn({ opacity: r(n).isDraggingRef.value && r(n).getSelection().find((w) => p.$el === w) ? "0.5 !important" : "" }),
      class: le(["vuefinder__item", "vf-item-" + r(n).explorerId]),
      "data-type": t.item.type,
      key: t.item.path,
      "data-item": JSON.stringify(t.item),
      "data-index": t.index,
      onDblclick: h[0] || (h[0] = (w) => s(t.item)),
      onTouchstart: h[1] || (h[1] = (w) => m(w)),
      onTouchend: h[2] || (h[2] = (w) => f()),
      onContextmenu: h[3] || (h[3] = tt((w) => r(e).emitter.emit("vf-contextmenu-show", { event: w, items: r(n).getSelected(), target: t.item }), ["prevent"]))
    }, [
      Lt(p.$slots, "default"),
      r(e).pinnedFolders.find((w) => w.path === t.item.path) ? (_(), K(r(Ko), {
        key: 0,
        class: "vuefinder__item--pinned"
      })) : N("", !0)
    ], 46, Wu)), [
      [i, t.item]
    ]);
  }
}, Ku = { class: "vuefinder__explorer__container" }, Yu = {
  key: 0,
  class: "vuefinder__explorer__header"
}, Xu = { class: "vuefinder__explorer__drag-item" }, Zu = {
  key: 0,
  class: "vuefinder__linear-loader absolute"
}, Qu = { class: "vuefinder__explorer__item-list-content" }, Ju = { class: "vuefinder__explorer__item-list-name" }, ef = { class: "vuefinder__explorer__item-name" }, tf = { class: "vuefinder__explorer__item-path" }, nf = { class: "vuefinder__explorer__item-list-content" }, sf = { class: "vuefinder__explorer__item-list-name" }, of = { class: "vuefinder__explorer__item-name" }, rf = { class: "vuefinder__explorer__item-size" }, af = { class: "vuefinder__explorer__item-date" }, lf = { class: "vuefinder__explorer__item-grid-content" }, cf = ["data-src", "alt"], df = {
  key: 2,
  class: "vuefinder__explorer__item-extension"
}, uf = { class: "vuefinder__explorer__item-title break-all" }, ff = {
  __name: "Explorer",
  setup(t) {
    const e = re("ServiceContainer"), { t: n } = e.i18n, o = (f) => f == null ? void 0 : f.substring(0, 3), s = O(null), i = O(""), c = e.dragSelect;
    let u;
    e.emitter.on("vf-fullscreen-toggle", () => {
      c.area.value.style.height = null;
    }), e.emitter.on("vf-search-query", ({ newQuery: f }) => {
      i.value = f, f ? e.emitter.emit("vf-fetch", {
        params: {
          q: "search",
          adapter: e.fs.adapter,
          path: e.fs.data.dirname,
          filter: f
        },
        onSuccess: (m) => {
          m.files.length || e.emitter.emit("vf-toast-push", { label: n("No search result found.") });
        }
      }) : e.emitter.emit("vf-fetch", { params: { q: "index", adapter: e.fs.adapter, path: e.fs.data.dirname } });
    });
    const l = St({ active: !1, column: "", order: "" }), d = (f = !0) => {
      let m = [...e.fs.data.files], p = l.column, h = l.order === "asc" ? 1 : -1;
      if (!f)
        return m;
      const w = (x, M) => typeof x == "string" && typeof M == "string" ? x.toLowerCase().localeCompare(M.toLowerCase()) : x < M ? -1 : x > M ? 1 : 0;
      return l.active && (m = m.slice().sort((x, M) => w(x[p], M[p]) * h)), m;
    }, v = (f) => {
      l.active && l.column === f ? (l.active = l.order === "asc", l.column = f, l.order = "desc") : (l.active = !0, l.column = f, l.order = "asc");
    };
    return Ce(() => {
      u = new br(c.area.value);
    }), Bs(() => {
      u.update();
    }), Ns(() => {
      u.destroy();
    }), (f, m) => (_(), g("div", Ku, [
      r(e).view === "list" || i.value.length ? (_(), g("div", Yu, [
        a("div", {
          onClick: m[0] || (m[0] = (p) => v("basename")),
          class: "vuefinder__explorer__sort-button vuefinder__explorer__sort-button--name vf-sort-button"
        }, [
          Z(b(r(n)("Name")) + " ", 1),
          _e(j(Jt, {
            direction: l.order
          }, null, 8, ["direction"]), [
            [je, l.active && l.column === "basename"]
          ])
        ]),
        i.value.length ? N("", !0) : (_(), g("div", {
          key: 0,
          onClick: m[1] || (m[1] = (p) => v("file_size")),
          class: "vuefinder__explorer__sort-button vuefinder__explorer__sort-button--size vf-sort-button"
        }, [
          Z(b(r(n)("Size")) + " ", 1),
          _e(j(Jt, {
            direction: l.order
          }, null, 8, ["direction"]), [
            [je, l.active && l.column === "file_size"]
          ])
        ])),
        i.value.length ? N("", !0) : (_(), g("div", {
          key: 1,
          onClick: m[2] || (m[2] = (p) => v("last_modified")),
          class: "vuefinder__explorer__sort-button vuefinder__explorer__sort-button--date vf-sort-button"
        }, [
          Z(b(r(n)("Date")) + " ", 1),
          _e(j(Jt, {
            direction: l.order
          }, null, 8, ["direction"]), [
            [je, l.active && l.column === "last_modified"]
          ])
        ])),
        i.value.length ? (_(), g("div", {
          key: 2,
          onClick: m[3] || (m[3] = (p) => v("path")),
          class: "vuefinder__explorer__sort-button vuefinder__explorer__sort-button--path vf-sort-button"
        }, [
          Z(b(r(n)("Filepath")) + " ", 1),
          _e(j(Jt, {
            direction: l.order
          }, null, 8, ["direction"]), [
            [je, l.active && l.column === "path"]
          ])
        ])) : N("", !0)
      ])) : N("", !0),
      a("div", Xu, [
        j(nu, {
          ref_key: "dragImage",
          ref: s,
          count: r(c).getCount()
        }, null, 8, ["count"])
      ]),
      a("div", {
        ref: r(c).scrollBarContainer,
        class: le(["vf-explorer-scrollbar-container vuefinder__explorer__scrollbar-container", [{ "grid-view": r(e).view === "grid" }, { "search-active": i.value.length }]])
      }, [
        a("div", {
          ref: r(c).scrollBar,
          class: "vuefinder__explorer__scrollbar"
        }, null, 512)
      ], 2),
      a("div", {
        ref: r(c).area,
        class: "vuefinder__explorer__selector-area vf-explorer-scrollbar vf-selector-area min-h-32",
        onContextmenu: m[4] || (m[4] = tt((p) => r(e).emitter.emit("vf-contextmenu-show", { event: p, items: r(c).getSelected() }), ["self", "prevent"]))
      }, [
        r(e).loadingIndicator === "linear" && r(e).fs.loading ? (_(), g("div", Zu)) : N("", !0),
        i.value.length ? (_(!0), g(ye, { key: 1 }, $e(d(), (p, h) => (_(), K(Fn, {
          item: p,
          index: h,
          dragImage: s.value,
          class: "vf-item vf-item-list"
        }, {
          default: J(() => [
            a("div", Qu, [
              a("div", Ju, [
                j(Vn, {
                  type: p.type,
                  small: r(e).compactListView
                }, null, 8, ["type", "small"]),
                a("span", ef, b(p.basename), 1)
              ]),
              a("div", tf, b(p.path), 1)
            ])
          ]),
          _: 2
        }, 1032, ["item", "index", "dragImage"]))), 256)) : N("", !0),
        r(e).view === "list" && !i.value.length ? (_(!0), g(ye, { key: 2 }, $e(d(), (p, h) => (_(), K(Fn, {
          item: p,
          index: h,
          dragImage: s.value,
          class: "vf-item vf-item-list",
          draggable: "true",
          key: p.path
        }, {
          default: J(() => [
            a("div", nf, [
              a("div", sf, [
                j(Vn, {
                  type: p.type,
                  small: r(e).compactListView
                }, null, 8, ["type", "small"]),
                a("span", of, b(p.basename), 1)
              ]),
              a("div", rf, b(p.file_size ? r(e).filesize(p.file_size) : ""), 1),
              a("div", af, b(r(Go)(p.last_modified)), 1)
            ])
          ]),
          _: 2
        }, 1032, ["item", "index", "dragImage"]))), 128)) : N("", !0),
        r(e).view === "grid" && !i.value.length ? (_(!0), g(ye, { key: 3 }, $e(d(!1), (p, h) => (_(), K(Fn, {
          item: p,
          index: h,
          dragImage: s.value,
          class: "vf-item vf-item-grid",
          draggable: "true"
        }, {
          default: J(() => [
            a("div", null, [
              a("div", lf, [
                (p.mime_type ?? "").startsWith("image") && r(e).showThumbnails ? (_(), g("img", {
                  src: "data:image/png;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==",
                  class: "vuefinder__explorer__item-thumbnail lazy",
                  "data-src": r(e).requester.getPreviewUrl(r(e).fs.adapter, p),
                  alt: p.basename,
                  key: p.path
                }, null, 8, cf)) : (_(), K(Vn, {
                  key: 1,
                  type: p.type
                }, null, 8, ["type"])),
                !((p.mime_type ?? "").startsWith("image") && r(e).showThumbnails) && p.type !== "dir" ? (_(), g("div", df, b(o(p.extension)), 1)) : N("", !0)
              ]),
              a("span", uf, b(r(Xn)(p.basename)), 1)
            ])
          ]),
          _: 2
        }, 1032, ["item", "index", "dragImage"]))), 256)) : N("", !0)
      ], 544),
      j(Ud)
    ]));
  }
}, vf = ["href", "download"], _f = ["onClick"], mf = {
  __name: "ContextMenu",
  setup(t) {
    const e = re("ServiceContainer"), n = O(null), o = O([]), s = O(""), i = St({
      active: !1,
      items: [],
      positions: {
        left: 0,
        top: 0
      }
    });
    e.emitter.on("vf-context-selected", (d) => {
      o.value = d;
    });
    const c = (d) => d.link(e, o), u = (d) => {
      e.emitter.emit("vf-contextmenu-hide"), d.action(e, o);
    };
    e.emitter.on("vf-search-query", ({ newQuery: d }) => {
      s.value = d;
    }), e.emitter.on("vf-contextmenu-show", ({ event: d, items: v, target: f = null }) => {
      if (i.items = e.contextMenuItems.filter((m) => m.show(e, {
        searchQuery: s.value,
        items: v,
        target: f
      })), s.value)
        if (f)
          e.emitter.emit("vf-context-selected", [f]);
        else
          return;
      else !f && !s.value ? e.emitter.emit("vf-context-selected", []) : v.length > 1 && v.some((m) => m.path === f.path) ? e.emitter.emit("vf-context-selected", v) : e.emitter.emit("vf-context-selected", [f]);
      l(d);
    }), e.emitter.on("vf-contextmenu-hide", () => {
      i.active = !1;
    });
    const l = (d) => {
      const v = e.dragSelect.area.value, f = e.root.getBoundingClientRect(), m = v.getBoundingClientRect();
      let p = d.clientX - f.left, h = d.clientY - f.top;
      i.active = !0, vt(() => {
        var I;
        const w = (I = n.value) == null ? void 0 : I.getBoundingClientRect();
        let x = (w == null ? void 0 : w.height) ?? 0, M = (w == null ? void 0 : w.width) ?? 0;
        p = m.right - d.pageX + window.scrollX < M ? p - M : p, h = m.bottom - d.pageY + window.scrollY < x ? h - x : h, i.positions = {
          left: p + "px",
          top: h + "px"
        };
      });
    };
    return (d, v) => _e((_(), g("ul", {
      ref_key: "contextmenu",
      ref: n,
      style: fn(i.positions),
      class: "vuefinder__context-menu"
    }, [
      (_(!0), g(ye, null, $e(i.items, (f) => (_(), g("li", {
        class: "vuefinder__context-menu__item",
        key: f.title
      }, [
        f.link ? (_(), g("a", {
          key: 0,
          class: "vuefinder__context-menu__link",
          target: "_blank",
          href: c(f),
          download: c(f),
          onClick: v[0] || (v[0] = (m) => r(e).emitter.emit("vf-contextmenu-hide"))
        }, [
          a("span", null, b(f.title(r(e).i18n)), 1)
        ], 8, vf)) : (_(), g("div", {
          key: 1,
          class: "vuefinder__context-menu__action",
          onClick: (m) => u(f)
        }, [
          a("span", null, b(f.title(r(e).i18n)), 1)
        ], 8, _f))
      ]))), 128))
    ], 4)), [
      [je, i.active]
    ]);
  }
}, pf = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  class: "h-5 w-5",
  viewBox: "0 0 24 24"
};
function hf(t, e) {
  return _(), g("svg", pf, e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4m0 5c0 2.21-3.582 4-8 4s-8-1.79-8-4"
    }, null, -1)
  ]));
}
const Yo = { render: hf }, gf = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  "stroke-width": "2",
  class: "h-5 w-5 stroke-slate-500 cursor-pointer",
  viewBox: "0 0 24 24"
};
function bf(t, e) {
  return _(), g("svg", gf, e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0"
    }, null, -1)
  ]));
}
const wf = { render: bf }, yf = { class: "vuefinder__status-bar__wrapper" }, kf = { class: "vuefinder__status-bar__space" }, xf = { class: "vuefinder__status-bar__space-container" }, Sf = { class: "vuefinder__status-bar__space-icon" }, $f = { class: "vuefinder__status-bar__space-text" }, Cf = { class: "vuefinder__status-bar__actions" }, Ef = ["disabled"], Tf = ["title"], Af = {
  __name: "Statusbar",
  setup(t) {
    const e = re("ServiceContainer"), { t: n } = e.i18n, { setStore: o } = e.storage, s = e.dragSelect, i = O("");
    e.emitter.on("vf-search-query", ({ newQuery: v }) => {
      i.value = v;
    });
    const c = ze(() => {
      const v = e.selectButton.multiple ? s.getSelected().length > 0 : s.getSelected().length === 1;
      return e.selectButton.active && v;
    }), u = ze(() => {
      var f, m;
      const v = (m = (f = e.fs) == null ? void 0 : f.data) == null ? void 0 : m.used_space;
      return typeof v == "number" ? v.toFixed(2) : "0.00";
    }), l = ze(() => {
      var f, m;
      const v = (m = (f = e.fs) == null ? void 0 : f.data) == null ? void 0 : m.total_space;
      return typeof v == "number" ? v.toFixed(2) : "0.00";
    });
    ze(() => {
      var m, p, h, w;
      const v = (p = (m = e.fs) == null ? void 0 : m.data) == null ? void 0 : p.used_space, f = (w = (h = e.fs) == null ? void 0 : h.data) == null ? void 0 : w.total_space;
      return typeof v == "number" && typeof f == "number" && f !== 0 ? (v / f * 100).toFixed(2) : "0.00";
    });
    const d = ze(() => `Used ${u.value}Mb out of ${l.value}Mb (${formatedUsedSpacePercentage.value}%)`);
    return (v, f) => (_(), g("div", yf, [
      a("div", kf, [
        a("div", xf, [
          a("span", Sf, [
            j(r(Yo))
          ]),
          a("span", $f, b(d.value), 1)
        ])
      ]),
      a("div", Cf, [
        r(e).selectButton.active ? (_(), g("button", {
          key: 0,
          class: le(["vf-btn py-0 vf-btn-primary", { disabled: !c.value }]),
          disabled: !c.value,
          onClick: f[0] || (f[0] = (m) => r(e).selectButton.click(r(s).getSelected(), m))
        }, b(r(n)("Select")), 11, Ef)) : N("", !0),
        a("span", {
          class: "vuefinder__status-bar__about",
          title: r(n)("About"),
          onClick: f[1] || (f[1] = (m) => r(e).modal.open(Fo))
        }, [
          j(r(wf))
        ], 8, Tf)
      ])
    ]));
  }
}, Mf = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  "stroke-width": "1.5",
  class: "text-neutral-500 fill-sky-500 stroke-gray-100/50 dark:stroke-slate-700/50 dark:fill-slate-500",
  viewBox: "0 0 24 24"
};
function Df(t, e) {
  return _(), g("svg", Mf, e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M3.75 9.776q.168-.026.344-.026h15.812q.176 0 .344.026m-16.5 0a2.25 2.25 0 0 0-1.883 2.542l.857 6a2.25 2.25 0 0 0 2.227 1.932H19.05a2.25 2.25 0 0 0 2.227-1.932l.857-6a2.25 2.25 0 0 0-1.883-2.542m-16.5 0V6A2.25 2.25 0 0 1 6 3.75h3.879a1.5 1.5 0 0 1 1.06.44l2.122 2.12a1.5 1.5 0 0 0 1.06.44H18A2.25 2.25 0 0 1 20.25 9v.776"
    }, null, -1)
  ]));
}
const Xo = { render: Df }, Of = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "currentColor",
  class: "h-5 w-5",
  viewBox: "0 0 24 24"
};
function Lf(t, e) {
  return _(), g("svg", Of, e[0] || (e[0] = [
    a("path", {
      fill: "none",
      d: "M0 0h24v24H0z"
    }, null, -1),
    a("path", { d: "M12 2c5.523 0 10 4.477 10 10s-4.477 10-10 10S2 17.523 2 12 6.477 2 12 2m3.6 5.2a1 1 0 0 0-1.4.2L12 10.333 9.8 7.4a1 1 0 1 0-1.6 1.2l2.55 3.4-2.55 3.4a1 1 0 1 0 1.6 1.2l2.2-2.933 2.2 2.933a1 1 0 0 0 1.6-1.2L13.25 12l2.55-3.4a1 1 0 0 0-.2-1.4" }, null, -1)
  ]));
}
const Vf = { render: Lf }, Ff = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  "stroke-linecap": "round",
  "stroke-linejoin": "round",
  "stroke-width": "2",
  viewBox: "0 0 24 24"
};
function If(t, e) {
  return _(), g("svg", Ff, e[0] || (e[0] = [
    a("path", {
      stroke: "none",
      d: "M0 0h24v24H0z"
    }, null, -1),
    a("path", { d: "M15 12H9M12 9v6" }, null, -1)
  ]));
}
const Zo = { render: If }, Hf = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  "stroke-linecap": "round",
  "stroke-linejoin": "round",
  "stroke-width": "2",
  viewBox: "0 0 24 24"
};
function Rf(t, e) {
  return _(), g("svg", Hf, e[0] || (e[0] = [
    a("path", {
      stroke: "none",
      d: "M0 0h24v24H0z"
    }, null, -1),
    a("path", { d: "M9 12h6" }, null, -1)
  ]));
}
const Qo = { render: Rf };
function Jo(t, e) {
  const n = t.findIndex((o) => o.path === e.path);
  n > -1 ? t[n] = e : t.push(e);
}
const Bf = { class: "vuefinder__folder-loader-indicator" }, Uf = {
  key: 1,
  class: "vuefinder__folder-loader-indicator--icon"
}, er = {
  __name: "FolderLoaderIndicator",
  props: /* @__PURE__ */ ur({
    adapter: {
      type: String,
      required: !0
    },
    path: {
      type: String,
      required: !0
    }
  }, {
    modelValue: {},
    modelModifiers: {}
  }),
  emits: ["update:modelValue"],
  setup(t) {
    const e = t, n = re("ServiceContainer"), { t: o } = n.i18n, s = Ps(t, "modelValue"), i = O(!1);
    Oe(
      () => s.value,
      () => {
        var l;
        return ((l = c()) == null ? void 0 : l.folders.length) || u();
      }
    );
    function c() {
      return n.treeViewData.find((l) => l.path === e.path);
    }
    const u = () => {
      i.value = !0, n.requester.send({
        url: "",
        method: "get",
        params: {
          q: "subfolders",
          adapter: e.adapter,
          path: e.path
        }
      }).then((l) => {
        Jo(n.treeViewData, { path: e.path, ...l });
      }).catch((l) => {
      }).finally(() => {
        i.value = !1;
      });
    };
    return (l, d) => {
      var v;
      return _(), g("div", Bf, [
        i.value ? (_(), K(r(_s), {
          key: 0,
          class: "vuefinder__folder-loader-indicator--loading"
        })) : (_(), g("div", Uf, [
          s.value && ((v = c()) != null && v.folders.length) ? (_(), K(r(Qo), {
            key: 0,
            class: "vuefinder__folder-loader-indicator--minus"
          })) : N("", !0),
          s.value ? N("", !0) : (_(), K(r(Zo), {
            key: 1,
            class: "vuefinder__folder-loader-indicator--plus"
          }))
        ]))
      ]);
    };
  }
}, Nf = { class: "vuefinder__treesubfolderlist__item-content" }, Pf = ["onClick"], qf = ["title", "onClick"], zf = { class: "vuefinder__treesubfolderlist__item-icon" }, jf = { class: "vuefinder__treesubfolderlist__subfolder" }, Gf = {
  __name: "TreeSubfolderList",
  props: {
    adapter: {
      type: String,
      required: !0
    },
    path: {
      type: String,
      required: !0
    }
  },
  setup(t) {
    const e = re("ServiceContainer"), n = O([]), o = t, s = O(null);
    Ce(() => {
      o.path === o.adapter + "://" && Ne(s.value, {
        scrollbars: {
          theme: "vf-theme-dark dark:vf-theme-light"
        }
      });
    });
    const i = ze(() => {
      var c;
      return ((c = e.treeViewData.find((u) => u.path === o.path)) == null ? void 0 : c.folders) || [];
    });
    return (c, u) => {
      const l = fr("TreeSubfolderList", !0);
      return _(), g("ul", {
        ref_key: "parentSubfolderList",
        ref: s,
        class: "vuefinder__treesubfolderlist__container"
      }, [
        (_(!0), g(ye, null, $e(i.value, (d, v) => (_(), g("li", {
          key: d.path,
          class: "vuefinder__treesubfolderlist__item"
        }, [
          a("div", Nf, [
            a("div", {
              class: "vuefinder__treesubfolderlist__item-toggle",
              onClick: (f) => n.value[d.path] = !n.value[d.path]
            }, [
              j(er, {
                adapter: t.adapter,
                path: d.path,
                modelValue: n.value[d.path],
                "onUpdate:modelValue": (f) => n.value[d.path] = f
              }, null, 8, ["adapter", "path", "modelValue", "onUpdate:modelValue"])
            ], 8, Pf),
            a("div", {
              class: "vuefinder__treesubfolderlist__item-link",
              title: d.path,
              onClick: (f) => r(e).emitter.emit("vf-fetch", { params: { q: "index", adapter: o.adapter, path: d.path } })
            }, [
              a("div", zf, [
                r(e).fs.path === d.path ? (_(), K(r(Xo), { key: 0 })) : (_(), K(r(kn), { key: 1 }))
              ]),
              a("div", {
                class: le(["vuefinder__treesubfolderlist__item-text", {
                  "vuefinder__treesubfolderlist__item-text--active": r(e).fs.path === d.path
                }])
              }, b(d.basename), 3)
            ], 8, qf)
          ]),
          a("div", jf, [
            _e(j(l, {
              adapter: o.adapter,
              path: d.path
            }, null, 8, ["adapter", "path"]), [
              [je, n.value[d.path]]
            ])
          ])
        ]))), 128))
      ], 512);
    };
  }
}, Wf = {
  __name: "TreeStorageItem",
  props: {
    storage: {
      type: String,
      required: !0
    }
  },
  setup(t) {
    const e = re("ServiceContainer"), { setStore: n } = e.storage, o = O(!1);
    function s(i) {
      i === e.fs.adapter ? o.value = !o.value : (e.emitter.emit("vf-search-exit"), e.emitter.emit("vf-fetch", { params: { q: "index", adapter: i } }), n("adapter", i));
    }
    return (i, c) => (_(), g(ye, null, [
      a("div", {
        onClick: c[2] || (c[2] = (u) => s(t.storage)),
        class: "vuefinder__treestorageitem__header"
      }, [
        a("div", {
          class: le(["vuefinder__treestorageitem__info", t.storage === r(e).fs.adapter ? "vuefinder__treestorageitem__info--active" : ""])
        }, [
          a("div", {
            class: le(["vuefinder__treestorageitem__icon", t.storage === r(e).fs.adapter ? "vuefinder__treestorageitem__icon--active" : ""])
          }, [
            j(r(Yo))
          ], 2),
          a("div", null, b(t.storage), 1)
        ], 2),
        a("div", {
          class: "vuefinder__treestorageitem__loader",
          onClick: c[1] || (c[1] = tt((u) => o.value = !o.value, ["stop"]))
        }, [
          j(er, {
            adapter: t.storage,
            path: t.storage + "://",
            modelValue: o.value,
            "onUpdate:modelValue": c[0] || (c[0] = (u) => o.value = u)
          }, null, 8, ["adapter", "path", "modelValue"])
        ])
      ]),
      _e(j(Gf, {
        adapter: t.storage,
        path: t.storage + "://",
        class: "vuefinder__treestorageitem__subfolder"
      }, null, 8, ["adapter", "path"]), [
        [je, o.value]
      ])
    ], 64));
  }
}, Kf = { class: "vuefinder__folder-indicator" }, Yf = { class: "vuefinder__folder-indicator--icon" }, Xf = {
  __name: "FolderIndicator",
  props: {
    modelValue: {},
    modelModifiers: {}
  },
  emits: ["update:modelValue"],
  setup(t) {
    const e = Ps(t, "modelValue");
    return (n, o) => (_(), g("div", Kf, [
      a("div", Yf, [
        e.value ? (_(), K(r(Qo), {
          key: 0,
          class: "vuefinder__folder-indicator--minus"
        })) : N("", !0),
        e.value ? N("", !0) : (_(), K(r(Zo), {
          key: 1,
          class: "vuefinder__folder-indicator--plus"
        }))
      ])
    ]));
  }
}, Zf = { class: "vuefinder__treeview__header" }, Qf = { class: "vuefinder__treeview__pinned-label" }, Jf = { class: "vuefinder__treeview__pin-text text-nowrap" }, ev = {
  key: 0,
  class: "vuefinder__treeview__pinned-list"
}, tv = { class: "vuefinder__treeview__pinned-item" }, nv = ["onClick"], sv = ["title"], ov = ["onClick"], rv = { key: 0 }, av = { class: "vuefinder__treeview__no-pinned" }, lv = { class: "vuefinder__treeview__storage" }, iv = {
  __name: "TreeView",
  setup(t) {
    const e = re("ServiceContainer"), { t: n } = e.i18n, { getStore: o, setStore: s } = e.storage, i = O(190), c = O(o("pinned-folders-opened", !0));
    Oe(c, (v) => s("pinned-folders-opened", v));
    const u = (v) => {
      e.pinnedFolders = e.pinnedFolders.filter((f) => f.path !== v.path), e.storage.setStore("pinned-folders", e.pinnedFolders);
    }, l = (v) => {
      const f = v.clientX, m = v.target.parentElement, p = m.getBoundingClientRect().width;
      m.classList.remove("transition-[width]"), m.classList.add("transition-none");
      const h = (x) => {
        i.value = p + x.clientX - f, i.value < 50 && (i.value = 0, e.showTreeView = !1), i.value > 50 && (e.showTreeView = !0);
      }, w = () => {
        const x = m.getBoundingClientRect();
        i.value = x.width, m.classList.add("transition-[width]"), m.classList.remove("transition-none"), window.removeEventListener("mousemove", h), window.removeEventListener("mouseup", w);
      };
      window.addEventListener("mousemove", h), window.addEventListener("mouseup", w);
    }, d = O(null);
    return Ce(() => {
      Ne(d.value, {
        overflow: {
          x: "hidden"
        },
        scrollbars: {
          theme: "vf-theme-dark dark:vf-theme-light"
        }
      });
    }), Oe(e.fs.data, (v, f) => {
      const m = v.files.filter((p) => p.type === "dir");
      Jo(e.treeViewData, { path: e.fs.path, folders: m.map((p) => ({
        adapter: p.storage,
        path: p.path,
        basename: p.basename
      })) });
    }), (v, f) => (_(), g(ye, null, [
      a("div", {
        onClick: f[0] || (f[0] = (m) => r(e).showTreeView = !r(e).showTreeView),
        class: le(["vuefinder__treeview__overlay", r(e).showTreeView ? "vuefinder__treeview__backdrop" : "hidden"])
      }, null, 2),
      a("div", {
        style: fn(r(e).showTreeView ? "min-width:100px;max-width:75%; width: " + i.value + "px" : "width: 0"),
        class: "vuefinder__treeview__container"
      }, [
        a("div", {
          ref_key: "treeViewScrollElement",
          ref: d,
          class: "vuefinder__treeview__scroll"
        }, [
          a("div", Zf, [
            a("div", {
              onClick: f[2] || (f[2] = (m) => c.value = !c.value),
              class: "vuefinder__treeview__pinned-toggle"
            }, [
              a("div", Qf, [
                j(r(Ko), { class: "vuefinder__treeview__pin-icon" }),
                a("div", Jf, b(r(n)("Pinned Folders")), 1)
              ]),
              j(Xf, {
                modelValue: c.value,
                "onUpdate:modelValue": f[1] || (f[1] = (m) => c.value = m)
              }, null, 8, ["modelValue"])
            ]),
            c.value ? (_(), g("ul", ev, [
              (_(!0), g(ye, null, $e(r(e).pinnedFolders, (m) => (_(), g("li", tv, [
                a("div", {
                  class: "vuefinder__treeview__pinned-folder",
                  onClick: (p) => r(e).emitter.emit("vf-fetch", { params: { q: "index", adapter: m.storage, path: m.path } })
                }, [
                  r(e).fs.path !== m.path ? (_(), K(r(kn), {
                    key: 0,
                    class: "vuefinder__treeview__folder-icon"
                  })) : N("", !0),
                  r(e).fs.path === m.path ? (_(), K(r(Xo), {
                    key: 1,
                    class: "vuefinder__treeview__open-folder-icon"
                  })) : N("", !0),
                  a("div", {
                    title: m.path,
                    class: le(["vuefinder__treeview__folder-name text-nowrap", {
                      "vuefinder__treeview__folder-name--active": r(e).fs.path === m.path
                    }])
                  }, b(m.basename), 11, sv)
                ], 8, nv),
                a("div", {
                  class: "vuefinder__treeview__remove-favorite",
                  onClick: (p) => u(m)
                }, [
                  j(r(Vf), { class: "vuefinder__treeview__remove-icon" })
                ], 8, ov)
              ]))), 256)),
              r(e).pinnedFolders.length ? N("", !0) : (_(), g("li", rv, [
                a("div", av, b(r(n)("No folders pinned")), 1)
              ]))
            ])) : N("", !0)
          ]),
          (_(!0), g(ye, null, $e(r(e).fs.data.storages, (m) => (_(), g("div", lv, [
            j(Wf, { storage: m }, null, 8, ["storage"])
          ]))), 256))
        ], 512),
        a("div", {
          onMousedown: l,
          class: le([(r(e).showTreeView, ""), "vuefinder__treeview__resize-handle"])
        }, null, 34)
      ], 4)
    ], 64));
  }
};
class cv {
  /**
   * 
   * @param {Item['title']} title 
   * @param {Item['action']} action 
   * @param {Item['link']} link
   * @param {Partial<SimpleItemOptions>} options 
   */
  constructor(e, n, o, s) {
    this.title = e, this.action = n, this.link = o, this.options = Object.assign(
      {
        needsSearchQuery: !1,
        target: "one"
      },
      s
    );
  }
  /**
   * @type {Item['show']}
   */
  show(e, n) {
    var s, i;
    const o = (c) => c.items.length > 1 && c.items.some((u) => {
      var l;
      return u.path === ((l = c.target) == null ? void 0 : l.path);
    }) ? "many" : c.target ? "one" : null;
    return !(this.options.needsSearchQuery !== !!n.searchQuery || this.options.target !== void 0 && this.options.target !== o(n) || this.options.targetType !== void 0 && this.options.targetType !== ((s = n.target) == null ? void 0 : s.type) || this.options.mimeType !== void 0 && this.options.mimeType !== ((i = n.target) == null ? void 0 : i.mime_type) || this.options.feature !== void 0 && !e.features.includes(this.options.feature) || this.options.show !== void 0 && !this.options.show(e, n));
  }
}
function Be(t, e) {
  return t.map((n) => new cv(n.title, n.action, n.link, {
    ...e,
    feature: n.key
  }));
}
const Te = {
  newfolder: {
    key: ue.NEW_FOLDER,
    title: ({ t }) => t("New Folder"),
    action: (t) => t.modal.open(Bo)
  },
  selectAll: {
    title: ({ t }) => t("Select All"),
    action: (t) => t.dragSelect.selectAll()
  },
  pinFolder: {
    title: ({ t }) => t("Pin Folder"),
    action: (t, e) => {
      t.pinnedFolders = t.pinnedFolders.concat(e.value), t.storage.setStore("pinned-folders", t.pinnedFolders);
    }
  },
  unpinFolder: {
    title: ({ t }) => t("Unpin Folder"),
    action: (t, e) => {
      t.pinnedFolders = t.pinnedFolders.filter((n) => !e.value.find((o) => o.path === n.path)), t.storage.setStore("pinned-folders", t.pinnedFolders);
    }
  },
  delete: {
    key: ue.DELETE,
    title: ({ t }) => t("Delete"),
    action: (t, e) => {
      t.modal.open(fs, { items: e });
    }
  },
  refresh: {
    title: ({ t }) => t("Refresh"),
    action: (t) => {
      t.emitter.emit("vf-fetch", { params: { q: "index", adapter: t.fs.adapter, path: t.fs.data.dirname } });
    }
  },
  preview: {
    key: ue.PREVIEW,
    title: ({ t }) => t("Preview"),
    action: (t, e) => t.modal.open(Wo, { adapter: t.fs.adapter, item: e.value[0] })
  },
  open: {
    title: ({ t }) => t("Open"),
    action: (t, e) => {
      t.emitter.emit("vf-search-exit"), t.emitter.emit("vf-fetch", {
        params: {
          q: "index",
          adapter: t.fs.adapter,
          path: e.value[0].path
        }
      });
    }
  },
  openDir: {
    title: ({ t }) => t("Open containing folder"),
    action: (t, e) => {
      t.emitter.emit("vf-search-exit"), t.emitter.emit("vf-fetch", {
        params: {
          q: "index",
          adapter: t.fs.adapter,
          path: e.value[0].dir
        }
      });
    }
  },
  download: {
    key: ue.DOWNLOAD,
    link: (t, e) => t.requester.getDownloadUrl(t.fs.adapter, e.value[0]),
    title: ({ t }) => t("Download"),
    action: () => {
    }
  },
  archive: {
    key: ue.ARCHIVE,
    title: ({ t }) => t("Archive"),
    action: (t, e) => t.modal.open(jo, { items: e })
  },
  unarchive: {
    key: ue.UNARCHIVE,
    title: ({ t }) => t("Unarchive"),
    action: (t, e) => t.modal.open(qo, { items: e })
  },
  rename: {
    key: ue.RENAME,
    title: ({ t }) => t("Rename"),
    action: (t, e) => t.modal.open(vs, { items: e })
  }
}, dv = [
  ...Be([Te.openDir], {
    needsSearchQuery: !0
  }),
  ...Be([Te.refresh, Te.selectAll, Te.newfolder], {
    target: null
  }),
  ...Be([Te.refresh, Te.archive, Te.delete], {
    target: "many"
  }),
  ...Be([Te.open], {
    targetType: "dir"
  }),
  ...Be([Te.unpinFolder], {
    targetType: "dir",
    show: (t, e) => t.pinnedFolders.findIndex((n) => {
      var o;
      return n.path === ((o = e.target) == null ? void 0 : o.path);
    }) !== -1
  }),
  ...Be([Te.pinFolder], {
    targetType: "dir",
    show: (t, e) => t.pinnedFolders.findIndex((n) => {
      var o;
      return n.path === ((o = e.target) == null ? void 0 : o.path);
    }) === -1
  }),
  ...Be([Te.preview, Te.download], {
    show: (t, e) => {
      var n;
      return ((n = e.target) == null ? void 0 : n.type) !== "dir";
    }
  }),
  ...Be([Te.rename], { numItems: "one" }),
  ...Be([Te.unarchive], {
    mimeType: "application/zip"
  }),
  ...Be([Te.archive], {
    show: (t, e) => {
      var n;
      return ((n = e.target) == null ? void 0 : n.mime_type) !== "application/zip";
    }
  }),
  ...Be([Te.delete], {})
], uv = { class: "vuefinder__main__content" }, fv = {
  __name: "VueFinder",
  props: {
    id: {
      type: String,
      default: "vf"
    },
    request: {
      type: [String, Object],
      required: !0
    },
    persist: {
      type: Boolean,
      default: !1
    },
    path: {
      type: String,
      default: ""
    },
    features: {
      type: [Array, Boolean],
      default: !0
    },
    debug: {
      type: Boolean,
      default: !1
    },
    theme: {
      type: String,
      default: "system"
    },
    locale: {
      type: String,
      default: null
    },
    maxHeight: {
      type: String,
      default: "600px"
    },
    maxFileSize: {
      type: String,
      default: "10mb"
    },
    fullScreen: {
      type: Boolean,
      default: !1
    },
    showTreeView: {
      type: Boolean,
      default: !1
    },
    pinnedFolders: {
      type: Array,
      default: []
    },
    showThumbnails: {
      type: Boolean,
      default: !0
    },
    selectButton: {
      type: Object,
      default(t) {
        return {
          active: !1,
          multiple: !1,
          click: (e) => {
          },
          ...t
        };
      }
    },
    onError: {
      type: Function,
      default: null
    },
    loadingIndicator: {
      type: String,
      default: "circular"
    },
    contextMenuItems: {
      type: Array,
      default: () => dv
    }
  },
  emits: ["select", "update:path"],
  setup(t, { emit: e }) {
    const n = e, o = t, s = Ia(o, re("VueFinderOptions"));
    vr("ServiceContainer", s);
    const { setStore: i } = s.storage, c = O(null);
    s.root = c;
    const u = s.dragSelect;
    _i(s);
    const l = (f) => {
      Object.assign(s.fs.data, f), u.clearSelection(), u.refreshSelection();
    };
    let d;
    s.emitter.on("vf-fetch-abort", () => {
      d.abort(), s.fs.loading = !1;
    }), s.emitter.on(
      "vf-fetch",
      ({
        params: f,
        body: m = null,
        onSuccess: p = null,
        onError: h = null,
        noCloseModal: w = !1
      }) => {
        ["index", "search"].includes(f.q) && (d && d.abort(), s.fs.loading = !0), d = new AbortController();
        const x = d.signal;
        s.requester.send({
          url: "",
          method: f.m || "get",
          params: f,
          body: m,
          abortSignal: x
        }).then((M) => {
          s.fs.adapter = M.adapter, s.persist && (s.fs.path = M.dirname, i("path", s.fs.path)), w || s.modal.close(), l(M), p && p(M);
        }).catch((M) => {
          console.error(M), h && h(M);
        }).finally(() => {
          ["index", "search"].includes(f.q) && (s.fs.loading = !1);
        });
      }
    );
    function v(f) {
      let m = {};
      f && f.includes("://") && (m = {
        adapter: f.split("://")[0],
        path: f
      }), s.emitter.emit("vf-fetch", {
        params: { q: "index", adapter: s.fs.adapter, ...m },
        onError: o.onError ?? ((p) => {
          p.message && s.emitter.emit("vf-toast-push", {
            label: p.message,
            type: "error"
          });
        })
      });
    }
    return Ce(() => {
      v(s.fs.path), Oe(
        () => o.path,
        (f) => {
          v(f);
        }
      ), u.onSelect((f) => {
        n("select", f);
      }), Oe(
        () => s.fs.data.dirname,
        (f) => {
          n("update:path", f);
        }
      );
    }), (f, m) => (_(), g("div", {
      class: "vuefinder",
      ref_key: "root",
      ref: c,
      tabindex: "0"
    }, [
      a("div", {
        class: le(r(s).theme.actualValue)
      }, [
        a("div", {
          class: le([
            r(s).fullScreen ? "vuefinder__main__fixed" : "vuefinder__main__relative",
            "vuefinder__main__container"
          ]),
          style: fn(
            r(s).fullScreen ? "" : "max-height: " + t.maxHeight + ";height: " + t.maxHeight
          ),
          onMousedown: m[0] || (m[0] = (p) => r(s).emitter.emit("vf-contextmenu-hide")),
          onTouchstart: m[1] || (m[1] = (p) => r(s).emitter.emit("vf-contextmenu-hide"))
        }, [
          j(Bc),
          j(Rd),
          a("div", uv, [
            j(iv),
            j(ff)
          ]),
          j(Af)
        ], 38),
        j(_r, { name: "fade" }, {
          default: J(() => [
            r(s).modal.visible ? (_(), K(Us(r(s).modal.type), { key: 0 })) : N("", !0)
          ]),
          _: 1
        }),
        j(mf)
      ], 2)
    ], 512));
  }
}, kv = {
  /**
   * @param {import('vue').App} app
   * @param options
   */
  install(t, e = {}) {
    e.i18n = e.i18n ?? {};
    let [n] = Object.keys(e.i18n);
    e.locale = e.locale ?? n ?? "en", t.provide("VueFinderOptions", e), t.component("VueFinder", fv);
  }
};
export {
  cv as SimpleContextMenuItem,
  dv as contextMenuItems,
  kv as default
};
