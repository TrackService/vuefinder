...
const pa = () => {
  ...
  const { ct: c, rt: d, lt: u, it: i, ut: f, ft: m } = r || {}, v = dn(() => o && s(!0), {
    ...
    if (!Hn(x)) {
      ...
      let k = !1, T = !1;
      if (ie(x, (R) => {
        const { attributeName: L, target: q, type: M, oldValue: B, addedNodes: P, removedNodes: J } = R, oe = M === "attributes", se = M === "childList", me = n === q, K = oe && L, E = K && rs(q, L || ""), H = Ut(E) ? E : null, N = K && B !== H, A = Ys(y, L) && N;
        if (e && (se || !me)) {
          const G = oe && N, z = G && i && qn(q, i), Q = (z ? !F(q, L, B, H) : !oe || G) && !U(R, !!z, n, r);
          ie(P, (ae) => ge($, ae)), ie(J, (ae) => ge($, ae)), T = T || Q;
        }
        !e && me && N && !F(q, L, B, H) && (ge(C, L), k = k || A);
...
const Lo = (n, e, s) => {
  ...
  const m = `[${it}]`, v = `[${Qe}]`, h = ["id", "class", "style", "open", "wrap", "cols", "rows"], { vt: _, ht: g, U: y, gt: S, bt: O, L: I, wt: D, yt: x, St: F, Ot: U } = n, $ = (A) => tt(A, "direction") === "rtl", C = {
    ...
    const A = T && T.V(n, e, C, k, s).W, z = !(D && I) && ls(g, it, sn), Y = !I && x(Kr), Q = Y && Ie(S), ae = Q && U(), fe = F(Co, z), ce = Y && A && A()[0], Te = fn(y), te = cs(y);
    ...
    S(A, G) {
      const [z] = A, [Y] = G;
      return [Ot(qe(z), qe(Y)).reduce((Q, ae) => (Q[ae] = z[ae] || Y[ae], Q), {})];
    }
  }), q = (A) => {
    const G = $(_);
    re(A, {
    ...
    return q(Y), z && !G && (A ? r : L)(Y), Y;
  }, J = (A, G, z) => {
    const Y = {
      Et: G
    };
    return q(Y), G && !z && L(Y), Y;
  }, [oe, se] = O ? ga(g, M) : [], me = !I && Lo(g, B, {
    _t: !0
  }), [K, E] = Rs(g, !1, J, {
    rt: h,
    ct: h
  ...
  return [() => {
    H && H.observe(g);
    const A = me && me(), G = oe && oe(), z = K(), Y = k.K((Q) => {
      Q ? L({
        zt: Q
      }) : N();
    });
    return () => {
      H && H.disconnect(), A && A(), G && G(), u && u(), z(), Y();
    };
  }, ({ It: A, At: G, Dt: z }) => {
    const Y = {}, [Q] = A("update.ignoreMutation"), [ae, fe] = A("update.attributes"), [ce, Te] = A("update.elementEvents"), [te, ye] = A("update.debounce"), De = Te || fe, xe = G || z, Se = (be) => Be(Q) && Q(be);
    if (De) {
      d && d(), u && u();
    ...
    if (xe) {
      ...
      be && re(Y, J(be[0], be[1], xe)), we && re(Y, M(we[0], xe)), pe && re(Y, P(pe[0], xe));
    }
    return q(Y), Y;
  }, C];
  ...
  const o = "--os-viewport-percent", c = "--os-scroll-percent", d = "--os-scroll-direction", { Z: u } = Xe(), { scrollbars: i } = u(), { slot: f } = i, { vt: m, ht: v, U: h, Mt: _, gt: g, wt: y, L: S } = e, { scrollbars: O } = _ ? {} : n, { slot: I } = O || {}, D = [], x = [], F = [], U = Vo([m, v, h], () => S && y ? m : v, f, I), $ = (K) => {
    if (Dt) {
      ...
      return {
        Rt: (z) => {
          ...
          return H[0] === ce[0] && H[1] === ce[1] || (A(), H = ce, E = z.kt.animate({
            clear: ["left"],
            transform: ce
          }, {
            timeline: N
    ...
    const { Vt: K, Lt: E } = s, H = (N, A) => lo(0, 1, N / (N + A) || 0);
    return {
      x: H(E.x, K.x),
      y: H(E.y, K.y)
    };
    ...
    if (!Dt) {
      const { Tt: K } = s, E = Cs(K, Ie(g)), H = (N) => (A) => [A.Ut, {
        [c]: zn(N) + ""
      }];
      R(x, H(E.x)), R(F, H(E.y));
    }
  }, B = () => {
    const { Tt: K } = s, E = Ln(K), H = (N) => (A) => [A.Ut, {
      [d]: N ? "0" : "1"
    }];
    R(x, H(E.x)), R(F, H(E.y)), Dt && (x.forEach(C.x.Rt), F.forEach(C.y.Rt));
  }, P = () => {
    if (S && !y) {
      const { Vt: K, Tt: E } = s, H = Ln(E), N = Cs(E, Ie(g)), A = (G) => {
    ...
    const E = K ? "x" : "y", N = yt(`${Ue} ${K ? sa : oa}`), A = yt(To), G = yt(us), z = {
      Ut: N,
      Pt: A,
      kt: G
    }, Y = C[E];
    return ge(K ? x : F, z), ge(D, [Le(N, A), Le(A, G), Z(St, N), Y && Y.Rt(z), r(z, L, K)]), z;
  }, oe = Z(J, !0), se = Z(J, !1), me = () => (Le(U, x[0].Ut), Le(U, F[0].Ut), Z(Re, D));
  return oe(), se(), [{
    Nt: q,
    qt: M,
    Bt: B,
  ...
  const { ht: u, U: i, L: f, gt: m, Kt: v, Ot: h } = e, { Ut: _, Pt: g, kt: y } = o, [S, O] = bt(333), [I, D] = bt(444), x = ($) => {
    ...
    const $ = "pointerup pointercancel lostpointercapture", C = `client${d ? "X" : "Y"}`, k = d ? gn : bn, T = d ? "left" : "top", R = d ? "w" : "h", L = d ? "x" : "y", q = (B, P) => (J) => {
      const { Vt: oe } = s, se = kt(g)[R] - kt(y)[R], K = P * J / se * oe[L];
      We(m, {
        [L]: B + K
      });
    }, M = [];
    return ve(g, "pointerdown", (B) => {
      const P = wt(B.target, `.${us}`) === y, J = P ? y : g, oe = n.scrollbars, se = oe[P ? "dragScroll" : "clickScroll"], { button: me, isPrimary: K, pointerType: E } = B, { pointers: H } = oe;
      if (me === 0 && K && se && (H || []).includes(E)) {
        Re(M), D();
        const A = !P && (B.shiftKey || se === "instant"), G = Z(Dn, y), z = Z(Dn, g), Y = (ue, $e) => (ue || G())[T] - ($e || z())[T], Q = Rn(Dn(m)[k]) / kt(m)[R] || 1, ae = q(Ie(m)[L], 1 / Q), fe = B[C], ce = G(), Te = z(), te = ce[k], ye = Y(ce, Te) + te / 2, De = fe - Te[T], xe = P ? 0 : De - ye, Se = (ue) => {
          Re(pe), J.releasePointerCapture(ue.pointerId);
        }, be = P || A, we = h(), pe = [ve(v, $, Se), ve(v, "selectstart", (ue) => Gn(ue), {
          H: !1
        ...
        if (J.setPointerCapture(B.pointerId), A)
          ae(xe);
        else if (!P) {
          const ue = Ht(fa);
          if (ue) {
  ...
  const _ = ["mouse", "pen"], g = (E) => _.includes(E.pointerType), [y, S] = bt(), [O, I] = bt(100), [D, x] = bt(100), [F, U] = bt(() => h), [$, C] = ka(n, o, r, xa(e, o, r, (E) => g(E) && oe())), { ht: k, Qt: T, wt: R } = o, { jt: L, Nt: q, qt: M, Bt: B, Ft: P } = $, J = (E, H) => {
    if (U(), E)
      L(Ls);
    else {
      const N = Z(L, Ls, !0);
  ...
  return [() => Z(Re, ge(K, C())), ({ It: E, Dt: H, Zt: N, tn: A }) => {
    ...
    })) : se(!0)), xe && L(ta, zt), be && (L(m), L(Se, !0), m = Se), Et && !Me && se(!0), $e && (u = ue === "move", i = ue === "leave", f = ue === "never", J(f, !0)), Nt && L(ia, qt), Tt && L(la, !!ut), En) {
      const ot = jt(Pt.x, te.x, !0), At = jt(Pt.y, te.y, !1);
      L(aa, !(ot && At));
    }
    Pe && (M(), q(), P(), Q && B(), L(Ds, !ye.x, !0), L(Ds, !ye.y, !1), L(na, ce && !R));
  }, {}, $];
}, $a = (n) => {
  const e = Xe(), { Z: s, P: r } = e, { elements: o } = s(), { padding: c, viewport: d, content: u } = o, i = cn(n), f = i ? {} : n, { elements: m } = f, { padding: v, viewport: h, content: _ } = m || {}, g = i ? n : f.target, y = vo(g), S = g.ownerDocument, O = S.documentElement, I = () => S.defaultView || Ve, D = Z(wa, [g]), x = Z(Vo, [g]), F = Z(yt, ""), U = Z(D, F, d), $ = Z(x, F, u), C = (te) => {
  ...
  }, k = U(h), T = k === g, R = T && y, L = !T && $(_), q = !T && k === L, M = R ? O : k, B = R ? M : g, P = !T && x(F, c, v), J = !q && L, oe = [J, M, P, B].map((te) => cn(te) && !Vt(te) && te), se = (te) => te && Ys(oe, te), me = !se(M) && C(M) ? M : g, K = R ? O : M, H = {
    vt: g,
    ht: B,
    U: M,
    ln: P,
    ...
    Ot: () => un(K, Qe, Xr, !0)
  }, { vt: N, ht: A, ln: G, U: z, bt: Y } = H, Q = [() => {
    ze(A, [it, On]), ze(N, On), y && ze(O, [On, it]);
  }];
  let ae = Nn([Y, z, G, A, N].find((te) => te && !se(te)));
  const fe = R ? N : Y || z, ce = Z(Re, Q);
  return [H, () => {
    ...
    return Je(A, it, T ? "" : Gr), Je(G, Kn, ""), Je(z, Qe, ""), Je(Y, As, ""), T || (Je(z, Se, be || "-1"), y && Je(O, Ts, "")), Le(fe, ae), Le(A, G), Le(G || A, !T && z), Le(z, Y), ge(Q, [we, () => {
      const pe = Pn(), ue = se(z), $e = ue && pe === z ? N : pe, Me = xe($e);
      ze(G, Kn), ze(Y, As), ze(z, Qe), y && ze(O, Ts), be ? Je(z, Se, be) : ze(z, Se), se(Y) && De(Y), ue && De(z), se(G) && De(G), Wn($e), Me();
    }]), r && !T && (as(z, Qe, Eo), ge(Q, Z(ze, z, Qe))), Wn(!T && y && ye === N && te.top === te ? z : ye), we(), ae = 0, ce;
  }, ce];
  ...
  const s = Xe(), { ht: r, ln: o, U: c, L: d, Qt: u, gt: i, wt: f, St: m, un: v } = n, { P: h } = s, _ = f && d, g = Z(Gs, 0), y = {
    ...
    D(!0);
    const N = Ie(i), A = m(Zr, !0), G = ve(u, xt, (fe) => {
      const ce = Ie(i);
  ...
  }, [U, $] = Fe(O, Z(cs, c)), [C, k] = Fe(O, Z(fn, c)), [T, R] = Fe(O), [L] = Fe(I), [q, M] = Fe(O), [B] = Fe(I), [P] = Fe({
    i: (E, H) => wn(E, H, S),
    o: {}
  }, () => Nr(c) ? tt(c, S) : {}), [J, oe] = Fe({
    ...
    const H = (A) => [at, vt, xt].map((G) => me(G, A)), N = H(!0).concat(H()).join(" ");
    m(N), m(qe(E).map((A) => me(E[A], A === "x")).join(" "), !0);
  };
  return ({ It: E, Zt: H, fn: N, Dt: A }, { dn: G }) => {
    const { dt: z, Ht: Y, Ct: Q, _t: ae, zt: fe } = H || {}, ce = se && se.V(n, e, N, s, E), { Y: Te, W: te, J: ye } = ce || {}, [De, xe] = da(E, s), [Se, be] = E("overflow"), we = $t(Se.x), pe = $t(Se.y), ue = z || G || Y || Q || fe || xe;
    ...
    const [qt, Nt] = Bt, [ut, Tt] = Et, [Pt, Sn] = Me, [$n, Cn] = $e, [Pe, En] = L({
      x: ut.w > 0,
      y: ut.h > 0
    }), zt = we && pe && (Pe.x || Pe.y) || we && Pe.x && !Pe.y || pe && Pe.y && !Pe.x, jt = G || Q || fe || Cn || Sn || Nt || Tt || be || xe || ue, ot = ua(Pe, Se), [At, Mt] = B(ot.k), [Gt, er] = P(A), ps = Q || ae || er || En || A, [tr, nr] = ps ? J(x(Gt), A) : oe();
  ...
  const d = Is(e, {}), [u, i, f, m, v] = Aa(n), [h, _, g] = ba(m, f, d, (x) => {
    ...
    const B = I(q), P = I(M), J = B || P || !os(T) || R;
    return c = !0, J && r(x, {
      Zt: q,
      tn: M
    }), J;
  ...
  if (e && !d) {
    ...
    const i = [], f = {}, m = (T) => {
      ...
      const { dt: M, Ct: B, xt: P, Ht: J, Et: oe, _t: se } = L, { nn: me, sn: K, en: E, cn: H } = q;
      I("updated", [k, {
        updateHints: {
          sizeChanged: !!M,
          directionChanged: !!B,
      ...
      state() {
        const { gn: T, bn: R } = F(), { F: L } = T, { Lt: q, Vt: M, k: B, rn: P, ln: J, _n: oe, Tt: se } = R;
        return re({}, {
          overflowEdge: q,
          overflowAmount: M,
          overflowStyle: B,
      ...
      elements() {
        const { vt: T, ht: R, ln: L, U: q, bt: M, gt: B, Qt: P } = U.wn, { Xt: J, Gt: oe } = U.yn, se = (K) => {
  ...
  setup(n) {
    ...
    return Ee(async () => {
      S = new pr({
        ...
        locale: r,
        onBeforeFileAdded(M, B) {
          if (B[M.id] != null) {
            const J = O(M.id);
            h.value[J].status === o.PENDING && (_.value = S.i18n("noDuplicates", { fileName: M.name })), h.value = h.value.filter((oe) => oe.id !== M.id);
          }
          return h.value.push({
            id: M.id,
            name: M.name,
        ...
        limit: 5,
        timeout: 0,
        getResponseError(M, B) {
          let P;
          try {
            P = JSON.parse(M).message;
          } catch {
            P = s("Cannot parse server response.");
          }
          return new Error(P);
        }
      }), S.on("restriction-failed", (M, B) => {
        const P = h.value[O(M.id)];
        C(P), _.value = B.message;
      }), S.on("upload", async () => {
        const M = e.request.getToken ? await e.request.getToken() : null, B = R();
        M && (B.headers = {
          ...B.headers,
          Authorization: `Bearer ${M}`
        }), S.setMeta({ ...B.body });
        const P = S.getPlugin("XHRUpload");
        P.opts.method = B.method, P.opts.endpoint = B.url + "?" + new URLSearchParams(B.params), P.opts.headers = B.headers, delete B.headers["Content-Type"], g.value = !0, h.value.forEach((J) => {
          J.status !== o.DONE && (J.percent = null, J.status = o.UPLOADING, J.statusName = s("Pending upload"));
        });
      }), S.on("upload-progress", (M, B) => {
        const P = Math.floor(B.bytesUploaded / B.bytesTotal * 100);
        h.value[O(M.id)].percent = `${P}%`;
      }), S.on("upload-success", (M) => {
        const B = h.value[O(M.id)];
        B.status = o.DONE, B.statusName = s("Done"), e.emitter.emit("vf-fetch", {
          params: {
            q: "index",
            adapter: e.fs.adapter,
            path: e.fs.data.dirname
      ...
      }), S.on("upload-error", (M, B) => {
        const P = h.value[O(M.id)];
        P.percent = null, P.status = o.ERROR, B.isNetworkError ? P.statusName = s(
          "Network Error, Unable establish connection to the server or interrupted."
        ) : P.statusName = B ? B.message : s("Unknown Error");
      }), S.on("error", (M) => {
      ...
      function L(M, B) {
        B.isFile && B.file((P) => M(B, P)), B.isDirectory && B.createReader().readEntries((P) => {
          P.forEach((J) => {
            L(M, J);
          });
        });
      ...
      v.value.addEventListener("drop", (M) => {
        M.preventDefault(), y.value = !1;
        const B = /^[/\\](.+)/;
        [...M.dataTransfer.items].forEach((P) => {
          P.kind === "file" && L((J, oe) => {
            const se = B.exec(J.fullPath);
            I(oe, se[1]);
          }, P.webkitGetAsEntry());
        });
      });
              ...
              (p(!0), b(ke, null, Ce(h.value, (M) => (p(), b("div", {
                ...
                l("div", Ai, [
                  ...
                  l("div", {
                    ...
                    ee(w(M.statusName) + " ", 1),
                    M.status === c.value.QUEUE_ENTRY_STATUS.UPLOADING ? (p(), b("b", Li, w(M.percent), 1)) : j("", !0)
                  ], 2)
                ]),
                l("button", {
                  type: "button",
