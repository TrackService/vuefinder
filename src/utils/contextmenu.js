...
export class SimpleItem {
  ...
  show(app, ctx) {
    ...
    const actualTarget = (ctx) => {
      if (
        ctx.items.length > 1 &&
        ctx.items.some((item) => item.path === ctx.target?.path)
      ) {
        return "many";
      }
    ...
    if (
      ...
      this.options.target !== actualTarget(ctx)
    ) {
      return false;
    }
    if (
      this.options.targetType !== undefined &&
      this.options.targetType !== ctx.target?.type
    ) {
      return false;
    }
    if (
      this.options.mimeType !== undefined &&
      this.options.mimeType !== ctx.target?.mime_type
    ) {
      return false;
    }
    if (
      this.options.feature !== undefined &&
      !app.features.includes(this.options.feature)
    ) {
      return false;
    }
...
const templateMap = {
  ...
  unpinFolder: {
    ...
    action: (app, selectedItems) => {
      app.pinnedFolders = app.pinnedFolders.filter(
        (fav) => !selectedItems.value.find((item) => item.path === fav.path)
      );
      app.storage.setStore("pinned-folders", app.pinnedFolders);
    },
  },
  ...
  download: {
    ...
    link: (app, selectedItems) =>
      app.requester.getDownloadUrl(app.fs.adapter, selectedItems.value[0]),
    title: ({ t }) => t("Download"),
    // action: () => {},
    action: (app, selectedItems) => {
  ...
  archive: {
    ...
    title: ({ t }) => t("Archive"),
    action: (app, selectedItems) =>
      app.modal.open(ModalArchive, { items: selectedItems }),
  },
  unarchive: {
    key: FEATURES.UNARCHIVE,
    title: ({ t }) => t("Unarchive"),
    action: (app, selectedItems) =>
      app.modal.open(ModalUnarchive, { items: selectedItems }),
  },
  rename: {
    key: FEATURES.RENAME,
    title: ({ t }) => t("Rename"),
    action: (app, selectedItems) =>
      app.modal.open(ModalRename, { items: selectedItems }),
  },
};
...
export const menuItems = [
  ...
  ...itemBundle(
    [templateMap.refresh, templateMap.selectAll, templateMap.newfolder],
    {
      target: null,
    }
  ),
  ...itemBundle(
    [templateMap.refresh, templateMap.archive, templateMap.delete],
    {
      target: "many",
    }
  ),
  ...
  ...itemBundle([templateMap.unpinFolder], {
    targetType: "dir",
    show: (app, ctx) =>
      app.pinnedFolders.findIndex((item) => item.path === ctx.target?.path) !==
      -1,
  }),
