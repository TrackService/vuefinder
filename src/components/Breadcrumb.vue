  ...
  <div class="vuefinder__breadcrumb__container">
    ...
    >
      ...
      <div class="vuefinder__breadcrumb__list">
        <div
          v-if="app.fs.hiddenBreadcrumbs.length"
      ...
      >
        <div v-for="(item, index) in app.fs.breadcrumbs" :key="index">
          ...
          <span
            @dragover="
              index === app.fs.breadcrumbs.length - 1 || handleDragOver($event)
            "
            @dragleave="
              index === app.fs.breadcrumbs.length - 1 || handleDragLeave($event)
            "
            @drop="
              index === app.fs.breadcrumbs.length - 1 ||
                handleDropZone($event, index)
            "
    ...
    <div
      v-show="app.fs.showHiddenBreadcrumbs"
      class="vuefinder__breadcrumb__hidden-dropdown"
    >
      <div
        v-for="(item, index) in app.fs.hiddenBreadcrumbs"
        :key="index"
        @dragover="handleDragOver($event)"
        @dragleave="handleDragLeave($event)"
      ...
      >
        <div class="vuefinder__breadcrumb__hidden-item-content">
          <span
            ><FolderSVG class="vuefinder__breadcrumb__hidden-item-icon"
          /></span>
  ...
  nextTick(() => {
    for (let i = children.length - 1; i >= 0; i--) {
      if (
        ...
        breadcrumbContainerWidth.value - 40
      ) {
        break;
      }
...
const handleHiddenBreadcrumbsClick = (item) => {
  app.emitter.emit("vf-fetch", {
    params: { q: "index", adapter: app.fs.adapter, path: item.path },
  });
