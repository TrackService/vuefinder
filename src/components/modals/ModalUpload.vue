    ...
    <div>
      ...
      <div class="vuefinder__upload-modal__content">
        ...
        <div class="vuefinder__upload-modal__file-list vf-scrollbar">
          ...
          >
            <span
              class="vuefinder__upload-modal__file-icon"
            ...
            <div class="vuefinder__upload-modal__file-info">
              ...
              >
                ...
                <b
                  class="ml-auto"
                  v-if="
                    entry.status === definitions.QUEUE_ENTRY_STATUS.UPLOADING
                  "
                  >{{ entry.percent }}</b
                >
              </div>
...
function upload() {
  ...
  if (
    !queue.value.filter((entry) => entry.status !== QUEUE_ENTRY_STATUS.DONE)
      .length
  ) {
...
onMounted(async () => {
  ...
  uppy.on("upload", async () => {
    const token = app.requester.getToken
      ? await app.requester.getToken()
      : null;
